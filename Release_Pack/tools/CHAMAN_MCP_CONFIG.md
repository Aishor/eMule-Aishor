...
> [!IMPORTANT]
> En la configuración de CHAMAN, debes usar la **ruta absoluta** de donde hayas extraído la carpeta portable de eMule-Aishor.

**Ruta del ejecutable Python (entorno virtual):**
```
[RUTA_PORTABLE]\_env\Scripts\python.exe
```

**Ruta del script MCP:**
```
[RUTA_PORTABLE]\tools\emule_mcp_server.py
```

**Configuración típica para CHAMAN** (ejemplo asumiendo extracción en `C:\eMule`):

```json
{
  "mcpServers": {
    "emule": {
      "command": "C:\\eMule\\_env\\Scripts\\python.exe",
      "args": [
        "C:\\eMule\\tools\\emule_mcp_server.py"
      ],
      "env": {
...
> - Ajusta `[RUTA_PORTABLE]` según tu ubicación real
...
3. Usa ruta absoluta completa: `[RUTA_PORTABLE]\_env\Scripts\python.exe`
...
1. Abrir terminal en la carpeta raíz de eMule-Aishor
2. Ejecutar:
```batch
_env\Scripts\python.exe tools\emule_mcp_server.py
```
