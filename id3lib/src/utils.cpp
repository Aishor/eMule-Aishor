...
#if (defined(ID3_ICONV_FORMAT_UTF16BE) && defined(ID3_ICONV_FORMAT_UTF16) &&   \
     defined(ID3_ICONV_FORMAT_UTF8) && defined(ID3_ICONV_FORMAT_ASCII))
#include <errno.h>
#include <iconv.h>
...
dami::String oldconvert(dami::String data, ID3_TextEnc sourceEnc,
...
#define ID3LIB_BUFSIZ 1024
  ...
  do {
    errno = 0;
    size_t nconv =
        iconv(cd, &source_str, &source_size, &target_str, &target_size);
    if (nconv == (size_t)-1 && errno != EINVAL && errno != E2BIG) {
      // errno is probably EILSEQ here, which means either an invalid byte
      // sequence or a valid but unconvertible byte sequence
      return target;
    }
...
String dami::convert(String data, ID3_TextEnc sourceEnc,
                     ID3_TextEnc targetEnc) {
  String target;
