...
CPPgDisplay::CPPgDisplay()
    : CPropertyPage(CPPgDisplay::IDD), m_eSelectFont(sfServer) {}
...
void CPPgDisplay::LoadSettings() {
  ...
  CheckDlgButton(IDC_SHOWRATEONTITLE,
                 static_cast<UINT>(thePrefs.showRatesInTitle));
  CheckDlgButton(IDC_DISABLEKNOWNLIST,
                 static_cast<UINT>(thePrefs.m_bDisableKnownClientList));
  CheckDlgButton(IDC_DISABLEQUEUELIST,
                 static_cast<UINT>(thePrefs.m_bDisableQueueList));
  CheckDlgButton(IDC_STORESEARCHES,
                 static_cast<UINT>(thePrefs.IsStoringSearchesEnabled()));
  CheckDlgButton(IDC_SHOWCATINFO,
                 static_cast<UINT>(thePrefs.ShowCatTabInfos()));
  CheckDlgButton(IDC_SHOWDWLPERCENT,
                 static_cast<UINT>(thePrefs.GetUseDwlPercentage()));
  CheckDlgButton(IDC_CLEARCOMPL,
                 static_cast<UINT>(thePrefs.GetRemoveFinishedDownloads()));
  CheckDlgButton(IDC_SHOWTRANSTOOLBAR,
                 static_cast<UINT>(thePrefs.IsTransToolbarEnabled()));
  CheckDlgButton(IDC_DISABLEHIST,
                 static_cast<UINT>(thePrefs.GetUseAutocompletion()));

#ifdef HAVE_WIN7_SDK_H
  if (thePrefs.GetWindowsVersion() >= _WINVER_7_)
    CheckDlgButton(IDC_WIN7TASKBARGOODIES,
                   static_cast<UINT>(thePrefs.IsWin7TaskbarGoodiesEnabled()));
  else
#endif
...
BOOL CPPgDisplay::OnInitDialog() {
  ...
  CSpinButtonCtrl *pSpinCtrl =
      static_cast<CSpinButtonCtrl *>(GetDlgItem(IDC_TOOLTIPDELAY_SPIN));
  if (pSpinCtrl)
...
BOOL CPPgDisplay::OnApply() {
  ...
  thePrefs.depth3D =
      static_cast<CSliderCtrl *>(GetDlgItem(IDC_3DDEPTH))->GetPos();
...
#ifdef HAVE_WIN7_SDK_H
  thePrefs.m_bShowWin7TaskbarGoodies =
      IsDlgButtonChecked(IDC_WIN7TASKBARGOODIES) != 0;
  ...
  if (thePrefs.m_bDisableKnownClientList !=
      (IsDlgButtonChecked(IDC_DISABLEKNOWNLIST) != 0)) {
    thePrefs.m_bDisableKnownClientList =
        IsDlgButtonChecked(IDC_DISABLEKNOWNLIST) != 0;
    if (thePrefs.m_bDisableKnownClientList)
      bListDisabled = true;
    else
  ...
  if (thePrefs.m_bDisableQueueList !=
      (IsDlgButtonChecked(IDC_DISABLEQUEUELIST) != 0)) {
  ...
  thePrefs.m_iToolDelayTime =
      (i > MAX_TOOLTIP_DELAY_SEC) ? MAX_TOOLTIP_DELAY_SEC : i;
  ...
  if ((IsDlgButtonChecked(IDC_SHOWTRANSTOOLBAR) != 0) !=
      thePrefs.IsTransToolbarEnabled()) {
    thePrefs.m_bWinaTransToolbar = !thePrefs.m_bWinaTransToolbar;
    theApp.emuledlg->transferwnd->ResetTransToolbar(
        thePrefs.m_bWinaTransToolbar);
...
void CPPgDisplay::Localize() {
  if (m_hWnd) {
    ...
    SetDlgItemText(IDC_HYPERTEXT_FONT_HINT,
                   GetResString(IDS_HYPERTEXT_FONT_HINT));
    SetDlgItemText(IDC_SELECT_HYPERTEXT_FONT,
                   GetResString(IDS_SELECT_FONT) + _T("..."));
    ...
    // Actualizar el texto de ayuda para las fuentes (ahora incluye Shift para
    // listas)
    CString strFontHint =
...
// NOTE: Can't use 'lCustData' for a structure which would hold that static
// members, because 's_pfnChooseFontHook' will be needed *before* WM_INITDIALOG
// (which would give as the 'lCustData').
static LPCFHOOKPROC s_pfnChooseFontHook = NULL;
static CPPgDisplay *s_pThis = NULL;

UINT_PTR CALLBACK CPPgDisplay::ChooseFontHook(HWND hdlg, UINT uiMsg,
  ...
  case WM_COMMAND:
    if (LOWORD(wParam) == psh3 /*Apply*/ && HIWORD(wParam) == BN_CLICKED) {
      ...
      if (pDlg != NULL) {
        ...
        else if (s_pThis->m_eSelectFont == sfList)
...
void CPPgDisplay::OnHelp() { theApp.ShowHelp(eMule_FAQ_Preferences_Display); }

BOOL CPPgDisplay::OnCommand(WPARAM wParam, LPARAM lParam) {
  return (wParam == ID_HELP) ? OnHelpInfo(NULL)
