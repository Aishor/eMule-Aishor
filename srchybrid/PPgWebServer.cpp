...
// create RSA 2048 key
int KeyCreate(mbedtls_pk_context *key, mbedtls_ctr_drbg_context *ctr_drbg,
              LPCTSTR output_file) {
  LPCTSTR pmsg = NULL;
...
int write_certificate(mbedtls_x509write_cert *crt, LPCTSTR output_file,
  ...
  int ret = mbedtls_x509write_crt_pem(crt, output_buf, sizeof output_buf, f_rng,
                                      p_rng);
  return ret ? ret : write_buffer(output_file, output_buf);
}
...
int CertCreate(const struct options &opt) {
  ...
  ret = mbedtls_x509write_crt_set_serial_raw(
      &crt, (unsigned char *)&opt.serial,
      1 + static_cast<size_t>(opt.serial > 0xff));
  if (ret) {
  ...
  ret = write_certificate(&crt, opt.cert_file, mbedtls_ctr_drbg_random,
                          &ctr_drbg);
  if (ret)
...
CPPgWebServer::CPPgWebServer()
    : CPropertyPage(CPPgWebServer::IDD), m_generating(), m_bNewCert(),
      m_bModified(), m_icoBrowse() {}
...
BOOL CPPgWebServer::OnInitDialog() {
  ...
  AddBuddyButton(GetDlgItem(IDC_TMPLPATH)->m_hWnd,
                 ::GetDlgItem(m_hWnd, IDC_TMPLBROWSE));
  InitAttachedBrowseButton(::GetDlgItem(m_hWnd, IDC_TMPLBROWSE), m_icoBrowse);

  AddBuddyButton(GetDlgItem(IDC_CERTPATH)->m_hWnd,
                 ::GetDlgItem(m_hWnd, IDC_CERTBROWSE));
  InitAttachedBrowseButton(::GetDlgItem(m_hWnd, IDC_CERTBROWSE), m_icoBrowse);

  AddBuddyButton(GetDlgItem(IDC_KEYPATH)->m_hWnd,
                 ::GetDlgItem(m_hWnd, IDC_KEYBROWSE));
...
void CPPgWebServer::LoadSettings() {
  ...
  GetDlgItem(IDC_WSUPNP)
      ->EnableWindow(thePrefs.IsUPnPEnabled() && thePrefs.GetWSIsEnabled());
  ...
  CheckDlgButton(IDC_WS_ALLOWHILEVFUNC,
                 static_cast<UINT>(thePrefs.GetWebAdminAllowedHiLevFunc()));
  CheckDlgButton(IDC_WSENABLEDLOW,
                 static_cast<UINT>(thePrefs.GetWSIsLowUserEnabled()));
...
BOOL CPPgWebServer::OnApply() {
  if (m_bModified) {
    ...
    thePrefs.m_bAllowAdminHiLevFunc =
        IsDlgButtonChecked(IDC_WS_ALLOWHILEVFUNC) != 0;

    thePrefs.m_bWebUseUPnP = IsDlgButtonChecked(IDC_WSUPNP) != 0;
    // add the port to existing mapping without having eMule restarting (if all
    // conditions are met)
    if (bUPnP != (thePrefs.m_bWebUseUPnP && bWSIsEnabled) &&
        thePrefs.IsUPnPEnabled() && theApp.m_pUPnPFinder != NULL)
      theApp.m_pUPnPFinder->GetImplementation()->LateEnableWebServerPort(
          bUPnP ? 0 : thePrefs.GetWSPort());
...
void CPPgWebServer::Localize() {
  if (m_hWnd) {
    ...
    SetDlgItemText(IDC_WSTIMEOUTLABEL,
                   GetResString(IDS_WEB_SESSIONTIMEOUT) + _T(':'));
...
void CPPgWebServer::SetUPnPState() {
  GetDlgItem(IDC_WSUPNP)
      ->EnableWindow(thePrefs.IsUPnPEnabled() &&
                     IsDlgButtonChecked(IDC_WSENABLED));
}

void CPPgWebServer::OnChangeHTTPS() {
  BOOL bEnable =
      IsDlgButtonChecked(IDC_WSENABLED) && IsDlgButtonChecked(IDC_WEB_HTTPS);
...
void CPPgWebServer::OnEnChangeWSEnabled() {
  ...
  GetDlgItem(IDC_WSPASSLOW)
      ->EnableWindow(bIsWIEnabled && IsDlgButtonChecked(IDC_WSENABLEDLOW));
  SetUPnPState();
  SetTmplButtonState();
  OnChangeHTTPS();
...
void CPPgWebServer::OnReloadTemplates() { theApp.webserver->ReloadTemplates(); }
...
void CPPgWebServer::OnGenerateCertificate() {
  ...
  opt.serial =
      _byteswap_ushort(rand() & 0xfff); // avoid repeated serials (kind of)
  if (!opt.serial)
...
void CPPgWebServer::SetTmplButtonState() {
  ...
  GetDlgItem(IDC_WSRELOADTMPL)
      ->EnableWindow(IsDlgButtonChecked(IDC_WSENABLED) &&
                     (buffer.CompareNoCase(thePrefs.GetTemplate()) == 0));
}
