...
#ifndef WINVER
#define WINVER                                                                 \
  0x0502 // 0x0502 == Windows Server 2003, Windows XP (same as VS2005-MFC)
#endif
...
#ifndef WINVER
#define WINVER                                                                 \
  0x0400 // 0x0400 == Windows 98 and Windows NT 4.0 (because of
...
#ifndef _WIN32_IE
#define _WIN32_IE                                                              \
  0x0560 // 0x0560 == Internet Explorer 5.6 -> Comctl32.dll v5.8 (same as MFC
         // internally used)
#endif

#endif // HAVE_VISTA_SDK

#define _ATL_CSTRING_EXPLICIT_CONSTRUCTORS // Makes certain CString constructors
                                           // explicit, preventing any
                                           // unintentional conversions
#define _ATL_EX_CONVERSION_MACROS_ONLY // Disable old ATL 3.0 string conversion
                                       // macros
...
#define _CONVERSION_DONT_USE_THREAD_LOCALE // for consistency with C-RTL/MFC the
                                           // ATL thread locale support has to
                                           // get disabled
...
// Disable some warnings which get fired with /W4 or /Wall for Windows/MFC/ATL
// headers

#pragma warning(disable : 4061) // enumerate in switch of enum is not explicitly
                                // handled by a case label
#pragma warning(disable : 4062) // enumerate in switch of enum is not handled
#pragma warning(                                                               \
    disable : 4191) // 'type cast' : unsafe conversion from <this> to <that>
#if _MSC_VER < 1400
#pragma warning(disable                                                        \
                : 4217) // <func>: member template functions cannot be used for
                        // copy-assignment or copy-construction
#endif
#pragma warning(disable : 4263) // <func> member function does not override any
                                // base class virtual member function
#pragma warning(disable                                                        \
                : 4264) // <func>: no override available for virtual member
                        // function from base <class>; function is hidden
#pragma warning(disable : 4265) // <class>: class has virtual functions, but
                                // destructor is not virtual
#if _MSC_VER >= 1400
#pragma warning(disable                                                        \
                : 4266) // no override available for virtual member function
                        // from base <class>; function is hidden
#endif
#if _MSC_VER >= 1400
#pragma warning(                                                               \
    disable                                                                    \
    : 4365) // conversion from 'int' to 'UINT', signed/unsigned mismatch
#endif
#if _MSC_VER >= 1900
#pragma warning(                                                               \
    disable                                                                    \
...
#if _MSC_VER < 1400
#pragma warning(disable                                                        \
                : 4529) // forming a pointer-to-member requires explicit use of
                        // the address-of operator ('&') and a qualified name
#endif
#if _MSC_VER >= 1400
#pragma warning(                                                               \
    disable                                                                    \
    : 4571) // Informational: catch(...) semantics changed since Visual C++ 7.1;
            // structured exceptions (SEH) are no longer caught
#endif
#pragma warning(disable                                                        \
                : 4619) // #pragma warning : there is no warning number <n>
#pragma warning(                                                               \
    disable : 4625) // <class> : copy constructor could not be generated because
                    // a base class copy constructor is inaccessible
#pragma warning(                                                               \
    disable : 4626) // <class> : assignment operator could not be generated
                    // because a base class copy constructor is inaccessible
#pragma warning(                                                               \
    disable : 4640) // construction of local static object is not thread-safe
#pragma warning(disable : 4668) // <name>  is not defined as a preprocessor
                                // macro, replacing with '0' for '#if/#elif'
#pragma warning(disable : 4710) // function not inlined
#pragma warning(                                                               \
    disable : 4711) // function <func> selected for automatic inline expansion
#if _MSC_VER >= 1400
#pragma warning(disable : 4738) // storing 32-bit float result in memory,
                                // possible loss of performance
#endif
#ifdef _M_ARM64
#pragma warning(disable : 4746) // volatile access of '<expression>' is subject
                                // to /volatile:[iso|ms] setting
#endif
...
#pragma warning(                                                               \
    disable                                                                    \
    : 5220) // a non-static data member with a volatile qualified type no longer
            // implies that compiler generated copy/move constructors and
...
// _CRT_SECURE_CPP_OVERLOAD_STANDARD_NAMES - Overloads all standard string
// functions (e.g. strcpy) with "_s" functions if, and only if, the size of the
// output buffer is known at compile time (so, if it is a static array). If
// there is a buffer overflow during runtime, it will throw an exception.
//
#ifndef _CRT_SECURE_CPP_OVERLOAD_STANDARD_NAMES
#define _CRT_SECURE_CPP_OVERLOAD_STANDARD_NAMES 1
#endif

// _CRT_SECURE_CPP_OVERLOAD_STANDARD_NAMES_COUNT - This is a cool CRT feature
// but does not make sense for our code. With our existing code we could get
// exceptions which are not justifiable because we explicitly terminate all our
// string buffers. This define could be enabled for debug builds though.
//
...
// Windows XP compatibility requires 'inet_addr' and 'WSAAsyncGetHostByName'
// (warning C4996)
