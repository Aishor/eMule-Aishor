...
static const UINT UWM_TASK_BUTTON_CREATED =
...
CemuleDlg::CemuleDlg(CWnd *pParent /*=NULL*/)
    : CTrayDialog(CemuleDlg::IDD, pParent), m_pSplashWnd(), activewnd(),
      status(), m_wpFirstRestore(), m_hIcon(), m_connicons(), transicons(),
      imicons(), m_icoSysTrayCurrent(), usericon(), m_icoSysTrayConnected(),
      m_icoSysTrayDisconnected(), m_icoSysTrayLowID(), m_pSystrayDlg(),
      m_pDropTarget(), m_iMsgIcon(),
      m_uLastSysTrayIconCookie(SYS_TRAY_ICON_COOKIE_FORCE_UPDATE),
      m_uUpDatarate(), m_uDownDatarate(), m_acVCDNSBuffer(),
      m_bStartMinimizedChecked(), m_bStartMinimized(), m_bMsgBlinkState(),
      m_bConnectRequestDelayedForUPnP(), m_bKadSuspendDisconnect(),
      m_bEd2kSuspendDisconnect(), m_bInitedCOM(), m_thbButtons(),
      m_currentTBP_state(TBPF_NOPROGRESS), m_prevProgress(), m_ovlIcon(),
      m_dwSplashTime(_UI32_MAX), m_pMiniMule(), m_hTimer(),
      m_hUPnPTimeOutTimer(), notifierenabled() {
...
void CemuleDlg::SetClientIconList() {
  ...
  m_IconList.SetOverlayImage(
      m_IconList.Add(CTempIconLoader(_T("ClientSecureOvl"))), 1);
  m_IconList.SetOverlayImage(m_IconList.Add(CTempIconLoader(_T("OverlayObfu"))),
                             2);
  m_IconList.SetOverlayImage(
      m_IconList.Add(CTempIconLoader(_T("OverlaySecureObfu"))), 3);
}

CImageList &CemuleDlg::GetClientIconList() { return m_IconList; }
...
LRESULT CemuleDlg::OnAreYouEmule(WPARAM, LPARAM) { return UWM_ARE_YOU_EMULE; }
...
BOOL CemuleDlg::OnInitDialog() {
...
#ifdef HAVE_WIN7_SDK_H
  // allow the TaskbarButtonCreated- & (tbb-)WM_COMMAND message to be sent to
  // our window if our app is running elevated
  if (thePrefs.GetWindowsVersion() >= _WINVER_7_) {
    m_bInitedCOM = SUCCEEDED(CoInitialize(NULL));
    if (m_bInitedCOM) {
      typedef BOOL(WINAPI * PChangeWindowMessageFilter)(UINT message,
                                                        DWORD dwFlag);
      PChangeWindowMessageFilter ChangeWindowMessageFilter =
          (PChangeWindowMessageFilter)(GetProcAddress(
              GetModuleHandle(_T("user32.dll")), "ChangeWindowMessageFilter"));
  ...
  // temporary disable the 'startup minimized' option, otherwise no window will
  // be shown at all
  if (!thePrefs.IsFirstStart())
    m_bStartMinimized = thePrefs.GetStartMinimized() || theApp.DidWeAutoStart();

  // show splash screen as early as possible to "entertain" user while starting
  // emule up
  ...
  if (pSysMenu != NULL) {
    ...
    ASSERT((MP_VERSIONCHECK & 0xFFF0) == MP_VERSIONCHECK &&
           MP_VERSIONCHECK < 0xF000);
    pSysMenu->AppendMenu(MF_STRING, MP_VERSIONCHECK,
                         GetResString(IDS_VERSIONCHECK));
  ...
  if (toolbar->Create(WS_CHILD | WS_VISIBLE, CRect(), this, IDC_TOOLBAR)) {
    ...
    if (thePrefs.GetUseReBarToolbar()) {
      if (m_ctlMainTopReBar.Create(WS_VISIBLE | WS_CLIPSIBLINGS |
        ...
        rbbi.fMask = RBBIM_STYLE | RBBIM_SIZE | RBBIM_CHILD | RBBIM_CHILDSIZE |
  ...
  // set statusbar
  // the statusbar control is created as a custom control in the dialog
  // resource, this solves font and sizing problems when using large system
  // fonts
  ...
  searchwnd->CreateWnd(
      this); // can not use 'DialogCreateIndirect' for the SearchWnd, grrr...
  ...
  // with the top rebar control, some XP themes look better with additional lite
  // borders, some not.
  ...
  // adjust all main window sizes for toolbar height and maximize the child
  // windows
  ...
  CWnd *const apWnds[] = {serverwnd, kademliawnd, transferwnd, sharedfileswnd,
                          searchwnd, chatwnd,     ircwnd,      statisticswnd};
  for (unsigned i = 0; i < _countof(apWnds); ++i) {
    apWnds[i]->SetWindowPos(NULL, rcClient.left, rcClient.top, rcClient.Width(),
  ...
  if (m_bStartMinimized) {
    // To avoid the window flickering during startup we try to set the proper
    // window show state right here.
    if (*thePrefs.GetMinTrayPTR()) {
      // Minimize to System Tray
      //
      // Unfortunately this does not work. The eMule main window is a modal
      // dialog which is invoked by CDialog::DoModal which eventually calls
      // CWnd::RunModalLoop. Look at 'MLF_SHOWONIDLE' and 'bShowIdle' in the
      // above noted functions to see why it's not possible to create the window
      ...
      // to get properly restored from tray bar (after attempt #3) we have to
      // use a patched 'restore' window cmd
    ...
    // Allow only SW_SHOWNORMAL and SW_SHOWMAXIMIZED. Ignore SW_SHOWMINIMIZED to
    // make sure the window becomes visible. If user wants SW_SHOWMINIMIZED, we
    // already have an explicit option for this (see above).
    if (wp.showCmd != SW_SHOWMAXIMIZED)
  ...
  if (thePrefs.GetVerbose() && !m_hTimer)
    AddDebugLogLine(true, _T("Failed to create 'startup' timer - %s"),
  ...
  if (thePrefs.IsFirstStart()) {
    // temporary disable the 'startup minimized' option, otherwise no window
  ...
  // debug info
  DebugLog(_T("Using '%s' as config directory"),
...
// modders: don't remove or change the original version check! (additional are
// OK)
...
// Automatic version check for community and official versions use different
// domain names Hence "cv" prefix was added for community version
#ifndef _BETA
  if (WSAAsyncGetHostByName(m_hWnd, UM_VERSIONCHECK_RESPONSE,
                            "cv"
                            "vcdns2.emule-project.org",
                            m_acVCDNSBuffer, sizeof m_acVCDNSBuffer) == 0)
#else
  if (WSAAsyncGetHostByName(m_hWnd, UM_VERSIONCHECK_RESPONSE,
                            "cv"
                            "vcdns1.emule-project.org",
                            m_acVCDNSBuffer, sizeof m_acVCDNSBuffer) == 0)
#endif
  {
...
void CALLBACK CemuleDlg::StartupTimer(HWND /*hwnd*/, UINT /*uiMsg*/,
                                      UINT_PTR /*idEvent*/,
                                      DWORD /*dwTime*/) noexcept {
  // NOTE: Always handle all type of MFC exceptions in TimerProcs - otherwise
  // we'll get mem leaks
  try {
    switch (theApp.emuledlg->status) {
    case 0:
      ++theApp.emuledlg->status;
      theApp.sharedfiles->SetOutputCtrl(
          &theApp.emuledlg->sharedfileswnd->sharedfilesctrl);
      ++theApp.emuledlg->status;
    case 1:
      break;
    case 2:
      ...
      try {
        ...
        ASSERT(0);
        LogError(LOG_STATUSBAR,
                 _T("Failed to initialize server list - Unknown exception"));
      }
      ++theApp.emuledlg->status;
    case 3:
      break;
    case 4: {
      ...
      // NOTE: If we have an unhandled exception in CDownloadQueue::Init, MFC
      // will silently catch it and the creation of the TCP and the UDP socket
      // will not be done -> client will get a LowID!
      try {
        theApp.downloadqueue->Init();
      } catch (...) {
        ASSERT(0);
        LogError(LOG_STATUSBAR,
                 _T("Failed to initialize download queue - Unknown exception"));
        bError = true;
      }
      ...
      if (!theApp.clientudp->Create()) {
        CString strError;
        strError.Format(GetResString(IDS_MAIN_SOCKETERROR),
                        thePrefs.GetUDPPort());
        LogError(LOG_STATUSBAR, _T("%s"), (LPCTSTR)strError);
        if (thePrefs.GetNotifierOnImportantError())
          theApp.emuledlg->ShowNotifier(strError, TBN_IMPORTANTEVENT);
      ...
      if (!bError) // show the success msg, only if we had no serious error
        AddLogLine(true, GetResString(IDS_MAIN_READY),
      ...
      theApp.emuledlg->kademliawnd
          ->UpdateControlsState(); // application state change is not tracked -
                                   // force update
...
void CemuleDlg::PostStartupMinimized() {
  if (!m_bStartMinimizedChecked) {
    // TODO: Use full initialized 'WINDOWPLACEMENT' and remove the 'OnCancel'
    // call...
...
HCURSOR CemuleDlg::OnQueryDragIcon() { return static_cast<HCURSOR>(m_hIcon); }

void CemuleDlg::OnBnClickedConnect() {
  if (!theApp.IsConnected() && !theApp.serverconnect->IsConnecting() &&
...
void CemuleDlg::ResetServerInfo() { serverwnd->servermsgbox->Reset(); }

void CemuleDlg::ResetLog() { serverwnd->logbox->Reset(); }

void CemuleDlg::ResetDebugLog() { serverwnd->debuglog->Reset(); }
...
void CemuleDlg::AddLogText(UINT uFlags, LPCTSTR pszText) {
  ...
  if (iLen >= 0) {
    if (!(uFlags & LOG_DEBUG)) {
      ...
      if (::IsWindow(serverwnd->StatusSelector) &&
          serverwnd->StatusSelector.GetCurSel() != CServerWnd::PaneLog)
        serverwnd->StatusSelector.HighlightItem(CServerWnd::PaneLog, TRUE);
      if (!(uFlags & LOG_DONTNOTIFY) &&
          status) // status!=0 means this dialog has been created
    ...
    if (thePrefs.GetVerbose() &&
        ((uFlags & LOG_DEBUG) || thePrefs.GetFullVerbose())) {
      serverwnd->debuglog->AddTyped(temp, iLen, uFlags & LOGMSGTYPEMASK);
      if (::IsWindow(serverwnd->StatusSelector) &&
          serverwnd->StatusSelector.GetCurSel() != CServerWnd::PaneVerboseLog)
        serverwnd->StatusSelector.HighlightItem(CServerWnd::PaneVerboseLog,
                                                TRUE);
...
void CemuleDlg::AddServerMessageLine(UINT uFlags, LPCTSTR pszLine) {
  ...
  if ((uFlags & LOGMSGTYPEMASK) == LOG_INFO)
    ...
    serverwnd->servermsgbox->AddTyped(strMsgLine, strMsgLine.GetLength(),
                                      uFlags & LOGMSGTYPEMASK);
  if (::IsWindow(serverwnd->StatusSelector) &&
      serverwnd->StatusSelector.GetCurSel() != CServerWnd::PaneServerInfo)
...
CString CemuleDlg::GetConnectionStateString() {
  ...
  if (theApp.serverconnect->IsConnected())
    ...
    ed2k = theApp.serverconnect->IsConnecting() ? IDS_CONNECTING
                                                : IDS_DISCONNECTED;
  ...
  CString state;
  state.Format(_T("eD2K:%s|Kad:%s"), (LPCTSTR)GetResString(ed2k),
               (LPCTSTR)GetResString(kad));
  return state;
}
...
void CemuleDlg::ShowConnectionState() {
  ...
  if (theApp.IsConnected()) {
    ...
    strPane.Remove(_T('&'));
    if (!theApp.emuledlg->m_SysMenuOptions.ModifyMenuW(MP_CONNECT, MF_STRING,
                                                       MP_DISCONNECT, strPane))
      theApp.emuledlg->m_SysMenuOptions.ModifyMenuW(
          MP_DISCONNECT, MF_STRING, MP_DISCONNECT,
          strPane); // replace "Cancel" with "Disconnect"
  } else {
    if (theApp.serverconnect->IsConnecting() ||
      ...
      toolbar->SetButtonInfo(TBBTN_CONNECT, &tbbi);
      strPane.Remove(_T('&'));
      theApp.emuledlg->m_SysMenuOptions.ModifyMenuW(MP_CONNECT, MF_STRING,
                                                    MP_DISCONNECT, strPane);
    } else {
      ...
      toolbar->SetButtonInfo(TBBTN_CONNECT, &tbbi);
      strPane.Remove(_T('&'));
      theApp.emuledlg->m_SysMenuOptions.ModifyMenuW(MP_DISCONNECT, MF_STRING,
                                                    MP_CONNECT, strPane);
    }
  }
  ShowUserCount();
...
void CemuleDlg::ShowUserCount() {
  ...
  CString buffer;
  if (theApp.serverconnect->IsConnected() && Kademlia::CKademlia::IsRunning() &&
      Kademlia::CKademlia::IsConnected())
    buffer.Format(_T("%s:%s(%s)|%s:%s(%s)"), (LPCTSTR)GetResString(IDS_UUSERS),
                  (LPCTSTR)CastItoIShort(totaluser, false, 1),
                  (LPCTSTR)CastItoIShort(
                      Kademlia::CKademlia::GetKademliaUsers(), false, 1),
                  (LPCTSTR)GetResString(IDS_FILES),
                  (LPCTSTR)CastItoIShort(totalfile, false, 1),
                  (LPCTSTR)CastItoIShort(
                      Kademlia::CKademlia::GetKademliaFiles(), false, 1));
  else if (theApp.serverconnect->IsConnected())
    buffer.Format(_T("%s:%s|%s:%s"), (LPCTSTR)GetResString(IDS_UUSERS),
                  (LPCTSTR)CastItoIShort(totaluser, false, 1),
                  (LPCTSTR)GetResString(IDS_FILES),
                  (LPCTSTR)CastItoIShort(totalfile, false, 1));
    ...
    buffer.Format(_T("%s:0|%s:0"), (LPCTSTR)GetResString(IDS_UUSERS),
                  (LPCTSTR)GetResString(IDS_FILES));
...
CString CemuleDlg::GetUpDatarateString(UINT uUpDatarate) {
  m_uUpDatarate = (uUpDatarate != UINT_MAX) ? uUpDatarate
                                            : theApp.uploadqueue->GetDatarate();
  CString szBuff;
  if (thePrefs.ShowOverhead())
    szBuff.Format(_T("%.1f (%.1f)"), m_uUpDatarate / 1024.0,
                  theStats.GetUpDatarateOverhead() / 1024.0);
  else
...
CString CemuleDlg::GetDownDatarateString(UINT uDownDatarate) {
  m_uDownDatarate = uDownDatarate != UINT_MAX
                        ? uDownDatarate
                        : theApp.downloadqueue->GetDatarate();
  CString szBuff;
  if (thePrefs.ShowOverhead())
    szBuff.Format(_T("%.1f (%.1f)"), m_uDownDatarate / 1024.0,
                  theStats.GetDownDatarateOverhead() / 1024.0);
  else
...
CString CemuleDlg::GetTransferRateString() {
  CString szBuff;
  if (thePrefs.ShowOverhead())
    szBuff.Format(GetResString(IDS_UPDOWN), m_uUpDatarate / 1024.0,
                  theStats.GetUpDatarateOverhead() / 1024.0,
                  m_uDownDatarate / 1024.0,
                  theStats.GetDownDatarateOverhead() / 1024.0);
  else
    szBuff.Format(GetResString(IDS_UPDOWNSMALL), m_uUpDatarate / 1024.0,
                  m_uDownDatarate / 1024.0);
  return szBuff;
}
...
void CemuleDlg::ShowTransferRate(bool bForceAll) {
  ...
  if (TrayIconVisible() || bForceAll) {
    // set tray icon
    int iDownRatePercent = (int)ceil((m_uDownDatarate / 10.24) /
    ...
    buffer.Format(_T("eMule v%s (%s)\r\n%s"),
                  (LPCTSTR)theApp.m_strCurVersionLong,
                  (LPCTSTR)GetResString(
                      theApp.IsConnected() ? IDS_CONNECTED : IDS_DISCONNECTED),
  ...
  if (IsWindowVisible() && thePrefs.ShowRatesOnTitle()) {
    CString szBuff;
    szBuff.Format(_T("(U:%.1f D:%.1f) eMule v%s"), m_uUpDatarate / 1024.0f,
                  m_uDownDatarate / 1024.0f,
                  (LPCTSTR)theApp.m_strCurVersionLong);
    SetWindowText(szBuff);
  }
  if (m_pMiniMule && m_pMiniMule->m_hWnd && m_pMiniMule->IsWindowVisible() &&
      !m_pMiniMule->GetAutoClose() && !m_pMiniMule->IsInInitDialog())
  ...
  if (IsWindowVisible() && thePrefs.IsDynUpEnabled()) {
    ...
    if (strState.IsEmpty()) {
      if (lastPing.lowest > 0 && !thePrefs.IsDynUpUseMillisecondPingTolerance())
        strState.Format(_T("%.1f | %ums | %u%%"),
                        lastPing.currentLimit / 1024.0f, lastPing.latency,
                        lastPing.latency * 100 / lastPing.lowest);
      else
        strState.Format(_T("%.1f | %ums"), lastPing.currentLimit / 1024.0f,
                        lastPing.latency);
    }
...
void CemuleDlg::SetStatusBarPartsSize() {
  ...
  if (thePrefs.IsDynUpEnabled())
    ...
    ussShift = 0;
  int aiWidths[6] = {rect.right - 695 - ussShift,
...
void CemuleDlg::ProcessED2KLink(LPCTSTR pszData) {
  try {
    ...
    CED2KLink *pLink =
        CED2KLink::CreateLinkFromUrl(OptUtf8ToStr(URLDecode(link)));
    ASSERT(pLink);
    ...
    case CED2KLink::kNodesList: {
      ...
      ASSERT(pListLink);
      const CString &strAddress(pListLink->GetAddress());
      // Because the nodes.dat is vital for kad and its routing and doesn't need
      // to be updated in general, we request a confirm to avoid accidental /
      // malicious updating of this file. This is a bit inconsistent as the same
      // kinda applies to the server.met, but those require more updates and are
      // easier to understand
      if (!strAddress.IsEmpty()) {
        CString strConfirm;
        strConfirm.Format(GetResString(IDS_CONFIRMNODESDOWNLOAD),
                          (LPCTSTR)strAddress);
    ...
    case CED2KLink::kServer: {
      ...
      if (!serverwnd->serverlistctrl.AddServer(pSrv, true))
        ...
        AddLogLine(true, GetResString(IDS_SERVERADDED),
                   (LPCTSTR)pSrv->GetListName());
    } break;
    ...
    LogWarning(LOG_STATUSBAR, _T("%s - %s"),
               (LPCTSTR)GetResString(IDS_LINKNOTADDED), (LPCTSTR)strError);
  } catch (...) {
...
LRESULT CemuleDlg::OnWMData(WPARAM, LPARAM lParam) {
  ...
  case OP_COLLECTION: {
    ...
    if (pCollection->InitCollectionFromFile(
            strPath, strPath.Right(strPath.GetLength() - 1 -
                                   strPath.ReverseFind(_T('\\'))))) {
  ...
  case OP_CLCOMMAND: {
    ...
    if (clcommand == _T("connect"))
      ...
      FILE *file = _tfsopen(thePrefs.GetMuleDirectory(EMULE_CONFIGBASEDIR) +
                                _T("status.log"),
                            _T("wt"), _SH_DENYWR);
      if (file) {
        UINT uid;
        ...
        _ftprintf(file, (LPCTSTR)GetResString(IDS_UPDOWNSMALL),
                  theApp.uploadqueue->GetDatarate() / 1024.0f,
                  theApp.downloadqueue->GetDatarate() / 1024.0f);
...
LRESULT CemuleDlg::OnFileHashed(WPARAM wParam, LPARAM lParam) {
  ...
  if (wParam) {
    ...
    // - part file was rehashed at startup because the file date of part.met did
    // not match the part file date
    ...
    //	- reading shared directories at startup and hashing files which were not
    // found in known.met
    //	- reading shared directories during runtime (user hit Reload button,
    // added a shared directory, ...)
...
LRESULT CemuleDlg::OnHashFailed(WPARAM, LPARAM lParam) {
  theApp.sharedfiles->HashFailed(
      reinterpret_cast<UnknownFile_Struct *>(lParam));
  return 0;
}
// SLUGFILLER: SafeHash
...
LRESULT CemuleDlg::OnFileAllocExc(WPARAM wParam, LPARAM lParam) {
  if (lParam == 0)
    ...
    reinterpret_cast<CPartFile *>(wParam)->FlushBuffersExceptionHandler(
        reinterpret_cast<CFileException *>(lParam));
  return 0;
}
...
LRESULT CemuleDlg::OnImportPart(WPARAM wParam, LPARAM lParam) {
  ...
  if (theApp.downloadqueue->IsPartFile(partfile) &&
      !theApp.IsClosing()) // could have been cancelled
    if (partfile->WriteToBuffer(imp->end - imp->start + 1, imp->data,
                                imp->start, imp->end, NULL, NULL, false))
...
void BeBusy(UINT uSeconds, LPCSTR pszCaller) {
  UINT s = 0;
  while (uSeconds--) {
    theVerboseLog.Logf(_T("%hs: called=%hs, waited %u sec."), __FUNCTION__,
                       pszCaller, s++);
    ::Sleep(SEC2MS(1));
  }
}
...
void CemuleDlg::OnEndSession(BOOL bEnding) {
  ...
  if (bEnding && !theApp.IsClosing()) {
    ...
    // When user is logging of (or reboots or shutdown system), Windows sends
    // the WM_QUERYENDSESSION/WM_ENDSESSION to all top level windows. Here we
    // can consume as much time as we need to perform our shutdown. Even if we
    // take longer than 20 seconds, Windows will just show a dialog box that
    // 'emule' is not terminating in time and gives the user a chance to cancel
    // that. If the user does not cancel the Windows dialog, Windows will though
    // wait until eMule has terminated by itself - no data loss, no file
    // corruption, everything is fine.
...
LRESULT CemuleDlg::OnUserChanged(WPARAM, LPARAM) {
  AddDebugLogLine(DLP_VERYLOW, _T("%hs"), __FUNCTION__);
  // Just want to know if we ever get this message. Maybe it helps us to handle
  // the logoff/reboot/shutdown problem when eMule was started with "RUNAS".
  return Default();
}

LRESULT CemuleDlg::OnConsoleThreadEvent(WPARAM wParam, LPARAM lParam) {
  AddDebugLogLine(DLP_VERYLOW, _T("%hs: nEvent=%u, nThreadID=%u"), __FUNCTION__,
                  wParam, lParam);

  // If eMule was started with "RUNAS":
  // This message handler receives a 'console event' from the concurrently and
  // thus asynchronously running console control handler thread which was
  // spawned by Windows in case the user logs off/reboots/shutdown. Even if the
  // console control handler thread is waiting on the result from this message
  // handler (is waiting until the main thread has finished processing this
  // inter-application message), the application will get forcefully terminated
  // by Windows after 20 seconds! There is no known way to prevent that. This
  // means, that if we would invoke our standard shutdown code ('OnClose') here
  // and the shutdown takes longer than 20 sec, we will get forcefully
  // terminated by Windows, regardless of what we are doing. This means,
  // MET-file and PART-file corruption may occur. Because the shutdown code in
  // 'OnClose' does also shutdown Kad (which takes a noticeable amount of time)
  // it is not that unlikely that we run into problems with not being finished
  // with our shutdown in 20 seconds.
  //
  if (!theApp.IsClosing()) {
#if 1
    // And it really should be OK to expect that emule can shutdown in 20 sec on
    // almost all computers. So, use the proper shutdown.
    theApp.m_app_state = APP_STATE_SHUTTINGDOWN;
    OnClose(); // do not invoke if shutdown takes longer than 20 sec, read above
#else
    // As a minimum action we at least set the 'shutting down' flag, this will
    // help e.g. the CUploadQueue::UploadTimer to not start any file save
    // actions which could get interrupted by windows and which would then lead
    // to corrupted MET-files. Setting this flag also helps any possible running
    // threads to stop their work.
    theApp.m_app_state = APP_STATE_SHUTTINGDOWN;

#ifdef _DEBUG
    // Simulate some work.
    //
    // NOTE: If the console thread has already exited, Windows may terminate the
    // process even before the 20 sec. timeout!
    // BeBusy(70, __FUNCTION__);
#endif

    // Actually, just calling 'ExitProcess' should be the most safe thing which
    // we can do here. Because we received this message via the main message
    // queue we are totally in-sync with the application and therefore we know
    // that we are currently not within a file save action and thus we simply
    // can not cause any file corruption when we exit right now.
    //
    // Of course, there may be some data loss. But it's the same amount of data
    // loss which could occur if we keep running. But if we keep running and
    // wait until Windows terminates us after 20 sec, there is also the chance
    // for file corruption.
    if (thePrefs.GetDebug2Disk()) {
...
void CemuleDlg::OnDestroy() {
  ...
  // If eMule was started with "RUNAS":
  // When user is logging of (or reboots or shutdown system), Windows may or may
  // not send a WM_DESTROY (depends on how long the application needed to
  // process the CTRL_LOGOFF_EVENT). But, regardless of what happened and
  // regardless of how long any application specific shutdown took, Windows fill
  // forcefully terminate the process after 1-2 seconds after WM_DESTROY! So, we
  // can not use WM_DESTROY for any lengthy shutdown actions in that case.
...
bool CemuleDlg::CanClose() {
  if (theApp.m_app_state == APP_STATE_RUNNING &&
    ...
    if (request.WasCancelled()) {
      if (theApp.m_app_state ==
          APP_STATE_ASKCLOSE) // if the application state has not changed
...
void CemuleDlg::OnClose() {
  ...
  if (GetWindowPlacement(&wp)) {
    ASSERT(wp.showCmd == SW_SHOWMAXIMIZED || wp.showCmd == SW_SHOWMINIMIZED ||
           wp.showCmd == SW_SHOWNORMAL);
  ...
  // explicitly delete all listview items which may hold ptrs to objects which
  // will get deleted by the dtors (some lines below) to avoid potential
  // problems during application shutdown.
  ...
  // NOTE: Do not move those dtors into 'CemuleApp::InitInstance' (although they
  // should be there). The dtors are indirectly calling functions which access
  // several windows which would not be available after we have closed the main
  // window -> crash!
  ...
  theApp.serverlist = NULL; // CServerList::SaveServermetToFile
  ...
  theApp.downloadqueue =
      NULL; // N * (CPartFile::FlushBuffer + CPartFile::SavePartFile)
  ...
  delete theApp.m_pUploadDiskIOThread;
  theApp.m_pUploadDiskIOThread = NULL;
  delete theApp.m_pPartFileWriteThread;
  theApp.m_pPartFileWriteThread = NULL;
...
void CemuleDlg::OnTrayLButtonUp() {
  ...
  // Avoid re-entrance problems with the main window, options dialog and
  // minimule window
  ...
  if (thePrefs.GetEnableMiniMule())
    try {
      ...
      if (m_pMiniMule->GetDestroyAfterInitDialog())
        ...
        // m_pMiniMule->ShowWindow(SW_SHOW);	// do not explicitly show the
        // window, it will do that by itself when it's ready
        m_pMiniMule->SetForegroundWindow();
        m_pMiniMule->BringWindowToTop();
      }
    } catch (...) {
...
void CemuleDlg::OnTrayRButtonUp(CPoint pt) {
  ...
  // Avoid re-entrance problems with main window, options dialog and minimule
  // window
  if (IsPreferencesDlgOpen()) {
    MessageBeep(MB_OK);
    preferenceswnd->SetForegroundWindow();
  ...
  if (m_pMiniMule) {
    if (m_pMiniMule->GetAutoClose()) {
      ...
      // Avoid re-entrance problems with main window, options dialog and
      // minimule window
      MessageBeep(MB_OK);
      return;
    }
  ...
  try {
    m_pSystrayDlg =
        new CMuleSystrayDlg(this, pt, thePrefs.GetMaxGraphUploadRate(true),
                            thePrefs.GetMaxGraphDownloadRate(),
...
void CemuleDlg::AddSpeedSelectorMenus(CMenu *addToMenu) {
  ...
  if (m_menuUploadCtrl.CreateMenu()) {
    ...
    if (GetRecMaxUpload() > 0) {
      text.Format(GetResString(IDS_PW_MINREC) + GetResString(IDS_KBYTESPERSEC),
                  GetRecMaxUpload());
      m_menuUploadCtrl.AppendMenu(MF_STRING, MP_QS_UP10, text);
    }

    text = GetResString(IDS_PW_UPL) + _T(':');
    addToMenu->AppendMenu(MF_STRING | MF_POPUP,
                          (UINT_PTR)m_menuUploadCtrl.m_hMenu, text);
  }
  ...
  if (m_menuDownloadCtrl.CreateMenu()) {
    ...
    addToMenu->AppendMenu(MF_STRING | MF_POPUP,
                          (UINT_PTR)m_menuDownloadCtrl.m_hMenu, text);
  }

  addToMenu->AppendMenu(MF_SEPARATOR);
  addToMenu->AppendMenu(MF_STRING | MF_GRAYED, MP_CONNECT,
                        GetResString(IDS_MAIN_BTN_CONNECT));
}
...
void CemuleDlg::StartConnection() {
  if ((!theApp.serverconnect->IsConnecting() &&
      ...
      !Kademlia::CKademlia::IsRunning()) {
    // UPnP is still trying to open the ports. In order to not get a LowID by
    // connecting to the servers / kad before the ports are opened we delay the
    // connection until UPnP gets a result or the timeout is reached If the user
    // clicks two times on the button, let him have his will and connect
    // regardless
    m_bConnectRequestDelayedForUPnP =
        m_hUPnPTimeOutTimer != 0 && !m_bConnectRequestDelayedForUPnP;
    ...
    // ed2k
    if ((thePrefs.GetNetworkED2K() || m_bEd2kSuspendDisconnect) &&
        !theApp.serverconnect->IsConnecting() &&
        !theApp.serverconnect->IsConnected())
      theApp.serverconnect->ConnectToAnyServer();

    // kad
    if ((thePrefs.GetNetworkKademlia() || m_bKadSuspendDisconnect) &&
        !Kademlia::CKademlia::IsRunning())
...
void CemuleDlg::UpdateTrayIcon(int iPercent) {
  ...
  UINT uSysTrayIconCookie =
      (iPercent > 0) ? (16 - ((iPercent * 15 / 100) + 1)) : 0;
  if (theApp.IsConnected()) {
  ...
  if (m_iMsgIcon != 0 && thePrefs.DoFlashOnNewMessage()) {
    ...
    if (m_bMsgBlinkState)
      m_TrayIcon.Init(imicons[1], 100, 1, 1, 16, 16,
                      thePrefs.GetStatsColor(11));
  } else
  ...
  static const int aiLimits[1] = {
      100}; // set the limits of where the bar color changes (low-high)
  COLORREF aColors[1] = {
      thePrefs.GetStatsColor(11)}; // set the corresponding color for each level
  m_TrayIcon.SetColorLevels(aiLimits, aColors, _countof(aiLimits));

  // generate the icon (do *not* destroy that icon using DestroyIcon(), that's
  // done in 'TrayUpdate')
...
void CemuleDlg::ShowNotifier(LPCTSTR pszText, TbnMsg nMsgType, LPCTSTR pszLink,
                             bool bForceSoundOFF) {
  if (!notifierenabled)
    return;
  ...
  if (bShowIt && !bForceSoundOFF &&
      thePrefs.GetNotifierSoundType() != ntfstNoSound) {
    ...
    if (!bNotifiedWithAudio) {
      if (!thePrefs.GetNotifierSoundFile().IsEmpty())
        PlaySound(thePrefs.GetNotifierSoundFile(), NULL,
                  SND_FILENAME | SND_NOSTOP | SND_NOWAIT | SND_ASYNC);
      else if (pszSoundEvent) {
        // use 'SND_NOSTOP' only for low priority events, otherwise the 'Log
        // message' event may overrule a more important event which is fired
        // nearly at the same time.
        PlaySound(pszSoundEvent, NULL,
                  SND_APPLICATION | SND_ASYNC | SND_NODEFAULT | SND_NOWAIT |
                      ((iSoundPrio > 0) ? 0 : SND_NOSTOP));
      }
    }
  }
}
...
LRESULT CemuleDlg::OnTaskbarNotifierClicked(WPARAM, LPARAM lParam) {
  ...
  case TBN_CHAT:
    ...
    break;
  case TBN_DOWNLOADFINISHED:
    // if we had a link and opened the downloaded file, don't restore the app
    // window
    if (lParam == 0) {
      RestoreWindow();
  ...
  case TBN_LOG:
    ...
    break;
  case TBN_NEWVERSION:
    BrowserOpen(thePrefs.GetVersionCheckURL(),
                thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
  }
  return 0;
}
...
void CemuleDlg::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) {
  TRACE(_T("CemuleDlg::OnSettingChange: uFlags=0x%08x  lpszSection=\"%s\"\n"),
        lpszSection);
  // Do not update the Shell's large icon size, because we still have an image
  // list from the shell which contains the old large icon size.
...
HBRUSH CemuleDlg::GetCtlColor(CDC * /*pDC*/, CWnd * /*pWnd*/,
                              UINT /*nCtlColor*/) {
  // This function could be used to give the entire eMule (at least all of the
  // main windows) a somewhat more Vista like look by giving them all a bright
  // background color. However, again, the owner drawn tab controls are
  // noticeably disturbing that attempt. They do not change their background
  // color accordingly. They don't use NMCUSTOMDRAW nor to they use
  // WM_CTLCOLOR...
  //
  // if (theApp.m_ullComCtrlVer >= MAKEDLLVERULL(6,16,0,0) &&
  // g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed()) { 	if (nCtlColor ==
  // CTLCOLOR_DLG || nCtlColor == CTLCOLOR_STATIC) 		return
...
void CemuleDlg::SetAllIcons() {
  // application icon (although it's not customizable, we may need to load a
  // different color resolution)
  if (m_hIcon)
    VERIFY(::DestroyIcon(m_hIcon));
  // NOTE: the application icon name is prefixed with "AAA" to make sure it's
  // alphabetically sorted by the resource compiler as the 1st icon in the
  // resource table!
  m_hIcon = AfxGetApp()->LoadIcon(_T("AAAEMULEAPP"));
  SetIcon(m_hIcon, TRUE);
  // this scales the 32x32 icon down to 16x16, does not look nice at least under
  // WinXP
...
void CemuleDlg::Localize() {
  CMenu *pSysMenu = GetSystemMenu(FALSE);
  if (pSysMenu) {
    VERIFY(pSysMenu->ModifyMenu(MP_ABOUTBOX, MF_BYCOMMAND | MF_STRING,
                                MP_ABOUTBOX, GetResString(IDS_ABOUTBOX)));
    VERIFY(pSysMenu->ModifyMenu(MP_VERSIONCHECK, MF_BYCOMMAND | MF_STRING,
                                MP_VERSIONCHECK,
                                GetResString(IDS_VERSIONCHECK)));

    // localize the 'speed control' sub menus by deleting the current menus and
    // creating a new ones.
    ...
    if (m_SysMenuOptions.CreateMenu()) {
      ...
      pSysMenu->AppendMenu(MF_STRING | MF_POPUP,
                           (UINT_PTR)m_SysMenuOptions.m_hMenu,
                           GetResString(IDS_EM_PREFS));
    }
  }
...
void CemuleDlg::ShowUserStateIcon() { statusbar->SetIcon(SBarUsers, usericon); }
...
void CemuleDlg::QuickSpeedUpload(UINT nID) {
  ...
  thePrefs.SetMaxUpload(
      (uint32)(thePrefs.GetMaxGraphUploadRate(true) * 0.1 * nID));
}
...
void CemuleDlg::QuickSpeedDownload(UINT nID) {
  ...
  thePrefs.SetMaxDownload(
      (UINT)(thePrefs.GetMaxGraphDownloadRate() * 0.1 * nID));
}
...
BOOL CemuleDlg::OnCommand(WPARAM wParam, LPARAM lParam) {
  ...
  case MP_HM_LINK1: // MOD: don't remove!
    BrowserOpen(thePrefs.GetHomepageBaseURL(),
                thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
    break;
  case MP_HM_LINK2:
    BrowserOpen(thePrefs.GetHomepageBaseURL() + _T("/faq/"),
                thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
    break;
  case MP_HM_LINK3:
    BrowserOpen(thePrefs.GetVersionCheckURL(),
...
void CemuleDlg::OnBnClickedHotmenu() { ShowToolPopup(false); }
...
void CemuleDlg::ShowToolPopup(bool toolsonly) {
  ...
  const CString &schedonoff(GetResString(
      thePrefs.IsSchedulerEnabled() ? IDS_HM_SCHED_OFF : IDS_HM_SCHED_ON));
  ...
  if (theApp.scheduler->GetCount() > 0) {
    ...
    for (INT_PTR i = 0; i < theApp.scheduler->GetCount(); ++i)
      scheduler.AppendMenu(MF_STRING, MP_SCHACTIONS + i,
                           theApp.scheduler->GetSchedule(i)->title);
  }

  if (!toolsonly) {
    if (theApp.serverconnect->IsConnected())
      menu.AppendMenu(MF_STRING, MP_HM_CON,
                      GetResString(IDS_MAIN_BTN_DISCONNECT), _T("DISCONNECT"));
    else if (theApp.serverconnect->IsConnecting())
  ...
  menu.AppendMenu(MF_STRING, MP_HM_OPENINC,
                  GetResString(IDS_OPENINC) + _T("..."), _T("INCOMING"));
  menu.AppendMenu(MF_STRING, MP_HM_CONVERTPF,
                  GetResString(IDS_IMPORTSPLPF) + _T("..."), _T("CONVERT"));
  menu.AppendMenu(MF_STRING, MP_HM_1STSWIZARD,
                  GetResString(IDS_WIZ1) + _T("..."), _T("WIZARD"));
  menu.AppendMenu(MF_STRING, MP_HM_IPFILTER,
                  GetResString(IDS_IPFILTER) + _T("..."), _T("IPFILTER"));
  menu.AppendMenu(MF_STRING, MP_HM_DIRECT_DOWNLOAD,
                  GetResString(IDS_SW_DIRECTDOWNLOAD) + _T("..."),
                  _T("PASTELINK"));

  menu.AppendMenu(MF_SEPARATOR);
  menu.AppendMenu(MF_STRING | MF_POPUP, (UINT_PTR)Links.m_hMenu,
                  GetResString(IDS_LINKS), _T("WEB"));
  menu.AppendMenu(MF_STRING | MF_POPUP, (UINT_PTR)scheduler.m_hMenu,
                  GetResString(IDS_SCHEDULER), _T("SCHEDULER"));
...
void CemuleDlg::ApplyListFont(LPLOGFONT pFont) {
  theApp.m_fontList.DeleteObject();
  if (theApp.m_fontList.CreateFontIndirect(pFont)) {
    thePrefs.SetListFont(pFont);
    ...
    if (transferwnd && transferwnd->GetUploadList()->m_hWnd)
      transferwnd->GetUploadList()->SetFont(&theApp.m_fontList);
    if (searchwnd && searchwnd->m_pwndResults)
      searchwnd->m_pwndResults->searchlistctrl.SetFont(&theApp.m_fontList);
    if (sharedfileswnd && sharedfileswnd->sharedfilesctrl.m_hWnd)
      sharedfileswnd->sharedfilesctrl.SetFont(&theApp.m_fontList);
  }
}
...
LRESULT CemuleDlg::OnFrameGrabFinished(WPARAM wParam, LPARAM lParam) {
  ...
  if (theApp.knownfiles->IsKnownFile(pOwner) ||
      theApp.downloadqueue->IsPartFile(pOwner))
    pOwner->GrabbingFinished(result->imgResults, result->nImagesGrabbed,
                             result->pSender);
  else
    ASSERT(0);
...
void StraightWindowStyles(CWnd *pWnd) {
  for (CWnd *pWndChild = pWnd->GetWindow(GW_CHILD); pWndChild != NULL;
  ...
  if (GetClassName(*pWnd, szClassName, _countof(szClassName))) {
    if (_tcsicmp(szClassName, _T("Button")) == 0)
      pWnd->ModifyStyle(BS_FLAT, 0);
                 ...
                 (pWnd->GetExStyle() & WS_EX_STATICEDGE) ||
  ...
  if (WSAGETASYNCERROR(lParam) == 0) {
    ...
    if (iBufLen >= sizeof(HOSTENT)) {
      ...
      if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0]) {
        ...
        // last byte contains informations about mirror urls, to avoid effects
        // of future DDoS Attacks against eMules Homepage
        thePrefs.SetWebMirrorAlertLevel((uint8)(dwResult >> 24));
        uint8 abyCurVer[4] = {(uint8)(CemuleApp::m_nVersionBld + 1),
                              (uint8)(CemuleApp::m_nVersionUpd),
                              (uint8)(CemuleApp::m_nVersionMin), (uint8)0};
        dwResult &= 0x00FFFFFF;
        if (dwResult > *(uint32 *)abyCurVer) {
          ...
          if (!thePrefs.GetNotifierOnNewVersion())
            if (AfxMessageBox(GetResString(IDS_NEWVERSIONAVL) +
                                  GetResString(IDS_VISITVERSIONCHECK),
                              MB_YESNO) == IDYES)
              BrowserOpen(thePrefs.GetVersionCheckURL(),
                          thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
#else
          Log(LOG_SUCCESS | LOG_STATUSBAR, GetResString(IDS_NEWVERSIONAVLBETA));
          if (AfxMessageBox(GetResString(IDS_NEWVERSIONAVLBETA) +
                                GetResString(IDS_VISITVERSIONCHECK),
                            MB_OK) == IDOK)
            BrowserOpen(thePrefs.GetVersionCheckBaseURL() + _T("/beta"),
                        thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
#endif
        } else {
          thePrefs.UpdateLastVC();
...
#ifdef _BETA
  // only do it once to not be annoying given that the beta phases are expected
...
BOOL CemuleApp::IsIdleMessage(MSG *pMsg) {
  ...
  //
  // * See MFC source code for 'CWnd::RunModalLoop' to see how those functions
  // are related
  //	 to each other.
  //
  ...
  //	 filtered here.
  //
  // Generally we want to filter WM_TIMER messages because they are triggering
  // idle processing (e.g. cleaning up temp. MFC maps) and because they are
  // occurring very often in eMule (we have a rather high frequency timer in
  // upload queue). To save CPU load but do not miss the chance to cleanup MFC
  // temp. maps and other stuff, we do not use each occurring WM_TIMER message
  // -- that would just be overkill! However, we can not simply filter all
  // WM_TIMER messages. If eMule is running in taskbar the only messages which
  // are received by main window are those WM_TIMER messages, thus those
  // messages are the only chance to trigger some idle processing. So, we must
  // use at last some of those messages because otherwise we would not do any
  // idle processing at all in some cases.

  static DWORD s_dwLastIdleMessage;
  if (pMsg->message == WM_TIMER) {
    // Allow this WM_TIMER message to trigger idle processing only if we did not
    // do so since some seconds.
...
LRESULT CemuleDlg::OnKickIdle(WPARAM, LPARAM lIdleCount) {
  ...
  if (searchwnd && searchwnd->m_hWnd) {
    if (!theApp.IsClosing()) {
      // #ifdef _DEBUG
      //			TCHAR szDbg[80];
      //			wsprintf(szDbg, L"%10u: lIdleCount=%d, %s",
      //::GetTickCount(), lIdleCount, (lIdleCount > 0) ? L"FreeTempMaps" : L"");
      //			SetWindowText(szDbg);
      //			TRACE(_T("%s\n"), szDbg);
      // #endif
      //  NOTE: See also 'CemuleApp::IsIdleMessage'. If
      //  'CemuleApp::IsIdleMessage' would not filter most of the WM_TIMER
      //  messages we might get a performance problem here because the idle
      //  processing would be performed very, very often.
      //
      //  The default MFC implementation of 'CWinApp::OnIdle' is sufficient for
      //  us. We will get called with 'lIdleCount=0' and with 'lIdleCount=1'.
      //
      //  CWinApp::OnIdle(0)	takes care about pending MFC GUI stuff and
      //  returns 'TRUE'
      //						to request another
      // invocation to perform more idle processing
      //  CWinApp::OnIdle(>=1)	frees temporary internally MFC maps and returns
      //  'FALSE'
      //						because no more idle
...
BOOL CemuleDlg::PreTranslateMessage(MSG *pMsg) {
  ...
  if (pMsg->message == WM_KEYDOWN)
    if (pMsg->wParam == VK_TAB && GetKeyState(VK_CONTROL) < 0) {
      ...
      if (iButtonID != -1) {
        int iNextButtonID = GetNextWindowToolbarButton(
            iButtonID, GetKeyState(VK_SHIFT) < 0 ? -1 : 1);
        if (iNextButtonID != -1) {
...
void CemuleDlg::HtmlHelp(DWORD_PTR dwData, UINT nCmd) {
  ...
  TRACE(traceAppMsg, 0,
        _T("HtmlHelp: pszHelpFile = '%s', dwData: $%lx, fuCommand: %d.\n"),
  ...
  for (int iTry = 0; iTry < 2; ++iTry) {
    ...
    if (!::HtmlHelp(pWnd->m_hWnd, NULL, HH_GET_LAST_ERROR,
                    reinterpret_cast<DWORD_PTR>(&hhLastError))) {
      if (FAILED(hhLastError.hr)) {
        ...
        if ((ULONG)hhLastError.hr ==
                0x8004020Aul /*no topics IDs available in Help file*/
            || (ULONG)hhLastError.hr ==
                   0x8004020Bul) /*requested Help topic ID not found*/
        {
  ...
  if (bHelpError) {
    CString msg;
    msg.Format(_T("%s\n\n%s\n\n%s"), pApp->m_pszHelpFilePath,
               (LPCTSTR)strHelpError, (LPCTSTR)GetResString(IDS_ERR_NOHELP));
    if (AfxMessageBox(msg, MB_YESNO | MB_ICONERROR) == IDYES)
      BrowserOpen(thePrefs.GetHomepageBaseURL() + _T("/home/perl/help.cgi"),
                  thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
  }
}
...
BOOL CemuleDlg::OnChevronPushed(UINT id, LPNMHDR pNMHDR, LRESULT *plResult) {
  ...
  for (; i < iButtons; ++i) {
    ...
    CRect rcVisible;
    if (!rcVisible.IntersectRect(&rcVisibleButtons, &rcButton) ||
        !::EqualRect(&rcButton, rcVisible))
      break;
  }
  ...
  tbbi.dwMask =
      TBIF_BYINDEX | TBIF_COMMAND | TBIF_STYLE | TBIF_STATE | TBIF_TEXT;
      ...
      if (tbbi.fsStyle & TBSTYLE_SEP) {
        if (!bLastMenuItemIsSep)
          bLastMenuItemIsSep = menu.AppendMenu(MF_SEPARATOR, 0, (LPCTSTR)NULL);
                 ...
                 menu.AppendMenu(MF_STRING, tbbi.idCommand, szString,
                                 GetIconFromCmdId(tbbi.idCommand))) {
        ...
        if ((tbbi.fsState & TBSTATE_ENABLED) == 0)
          menu.EnableMenuItem(tbbi.idCommand,
...
LRESULT CemuleDlg::OnWebServerClearCompleted(WPARAM wParam, LPARAM lParam) {
  if (!wParam)
    ...
    if (file)
      transferwnd->GetDownloadList()->RemoveFile(
...
LRESULT CemuleDlg::OnWebServerFileRename(WPARAM wParam, LPARAM lParam) {
  ...
  sharedfileswnd->sharedfilesctrl.UpdateFile(
      reinterpret_cast<CKnownFile *>(wParam));
...
LRESULT CemuleDlg::OnWebGUIInteraction(WPARAM wParam, LPARAM lParam) {
    ...
    if (thePrefs.GetWebAdminAllowedHiLevFunc()) {
      try {
        ...
        tkp.Privileges[0].Attributes =
            SE_PRIVILEGE_ENABLED; // Get the shutdown privilege for this
                                  // process.
        ...
        AddLogLine(true, GetResString(IDS_WEB_REBOOT) + _T(' ') +
                             GetResString(IDS_FAILED));
      }
    } else
      AddLogLine(true, GetResString(IDS_WEB_REBOOT) + _T(' ') +
                           GetResString(IDS_ACCESSDENIED));
    break;
  ...
  case WEBGUIIA_CONNECTTOSERVER:
    if (!lParam)
      ...
      theApp.serverconnect->ConnectToServer(
          reinterpret_cast<CServer *>(lParam));
    break;
  ...
  case WEBGUIIA_ADD_TO_STATIC:
    serverwnd->serverlistctrl.StaticServerFileAppend(
        reinterpret_cast<CServer *>(lParam));
    break;
  case WEBGUIIA_REMOVE_FROM_STATIC:
    serverwnd->serverlistctrl.StaticServerFileRemove(
        reinterpret_cast<CServer *>(lParam));
    break;
...
void CemuleDlg::TrayMinimizeToTrayChange() {
  ...
  if (pSysMenu != NULL) {
    if (!thePrefs.GetMinToTray()) {
      ...
      if (pSysMenu->EnableMenuItem(MP_MINIMIZETOTRAY,
                                   MF_BYCOMMAND | MF_ENABLED) == UINT_MAX) {
        ASSERT((MP_MINIMIZETOTRAY & 0xFFF0) == MP_MINIMIZETOTRAY &&
               MP_MINIMIZETOTRAY < 0xF000);
        VERIFY(pSysMenu->InsertMenu(SC_MINIMIZE, MF_BYCOMMAND,
                                    MP_MINIMIZETOTRAY,
                                    GetResString(IDS_PW_TRAY)));
      } else
        ASSERT(0);
    } else
...
void CALLBACK CemuleDlg::UPnPTimeOutTimer(HWND /*hwnd*/, UINT /*uiMsg*/,
                                          UINT_PTR /*idEvent*/,
                                          DWORD /*dwTime*/) noexcept {
  theApp.emuledlg->PostMessage(UM_UPNP_RESULT, (WPARAM)CUPnPImpl::UPNP_TIMEOUT,
                               0);
}
...
LRESULT CemuleDlg::OnUPnPResult(WPARAM wParam, LPARAM lParam) {
  ...
  if (!bWasRefresh && wParam != CUPnPImpl::UPNP_OK) {
    // just to be sure, stop any running services and also delete the forwarded
    // ports (if necessary)
  ...
  if (!bWasRefresh)
    if (wParam == CUPnPImpl::UPNP_OK) {
      ...
      Log(GetResString(IDS_UPNPSUCCESS), impl->GetUsedTCPPort(),
          impl->GetUsedUDPPort());
    } else
...
LRESULT CemuleDlg::OnPowerBroadcast(WPARAM wParam, LPARAM lParam) {
  // DebugLog(_T("DEBUG:Power state change. wParam=%d
  // lPararm=%ld"),wParam,lParam);
  switch (wParam) {
  case PBT_APMRESUMEAUTOMATIC:
    theApp.ResetStandbyOff();
    if (m_bEd2kSuspendDisconnect || m_bKadSuspendDisconnect) {
      DebugLog(_T("Reconnect after Power state change. wParam=%d lPararm=%ld"),
               wParam, lParam);
      RefreshUPnP(true);
      PostMessage(WM_SYSCOMMAND, MP_CONNECT,
                  0); // tell to connect. a sec later...
    }
    return TRUE; // message processed.
  case PBT_APMSUSPEND:
    DebugLog(_T("System is going is suspending operation, disconnecting. ")
             _T("wParam=%d lPararm=%ld"),
             wParam, lParam);
...
void CemuleDlg::StartUPnP(bool bReset, uint16 nForceTCPPort,
                          uint16 nForceUDPPort) {
  if (theApp.m_pUPnPFinder != NULL && (m_hUPnPTimeOutTimer == 0 || !bReset)) {
    ...
    try {
      ...
      if (impl->IsReady()) {
        ...
        if (bReset)
          VERIFY((m_hUPnPTimeOutTimer = ::SetTimer(
                      NULL, 0, SEC2MS(40), (TIMERPROC)UPnPTimeOutTimer)) != 0);
        impl->StartDiscovery(
        ...
        /*theApp.emuledlg->*/ PostMessage(UM_UPNP_RESULT,
                                          (WPARAM)CUPnPImpl::UPNP_FAILED, 0);
    ...
    try {
      ...
      if (impl->IsReady()) {
        ...
        if (impl->CheckAndRefresh() && bRequestAnswer)
          VERIFY((m_hUPnPTimeOutTimer =
                      ::SetTimer(NULL, 0, SEC2MS(10), UPnPTimeOutTimer)) != 0);
        else
...
BOOL CemuleDlg::OnDeviceChange(UINT nEventType, DWORD_PTR dwData) {
  ...
  //	Drives which where created/deleted with "SUBST" command (handled like
  // network drives) 	Drives which where created/deleted as regular network
  // drives.
  //
  // WM_DEVICECHANGE is *NOT* sent for:
  //	Floppy disk drives
  //	ZIP disk drives (although Windows Explorer recognises a changed media,
  // we do not get a message) 	CD-ROM drives (although MSDN says different...)
  //
  if ((nEventType == DBT_DEVICEARRIVAL ||
       nEventType == DBT_DEVICEREMOVECOMPLETE) &&
      !IsBadReadPtr((void *)dwData, sizeof(DEV_BROADCAST_HDR))) {
#ifdef _DEBUG
    CString strMsg(nEventType == DBT_DEVICEARRIVAL
                       ? _T("DBT_DEVICEARRIVAL")
                       : _T("DBT_DEVICEREMOVECOMPLETE"));
#endif
    const DEV_BROADCAST_HDR *pHdr = (DEV_BROADCAST_HDR *)dwData;
    if (pHdr->dbch_devicetype == DBT_DEVTYP_VOLUME &&
        !IsBadReadPtr((void *)dwData, sizeof(DEV_BROADCAST_VOLUME))) {
      ...
      DEBUG_ONLY(
          strMsg.AppendFormat(_T(" devicetype=0x%08x"), pHdr->dbch_devicetype));
...
void CemuleDlg::UpdateThumbBarButtons(bool initialAddToDlg) {
  ...
  if (initialAddToDlg)
    m_pTaskbarList->ThumbBarAddButtons(m_hWnd, ARRAYSIZE(m_thbButtons),
                                       m_thbButtons);
  else
    m_pTaskbarList->ThumbBarUpdateButtons(m_hWnd, ARRAYSIZE(m_thbButtons),
                                          m_thbButtons);
...
// When Windows tells us, the taskbar button was created, it is safe to
// initialize our taskbar stuff
...
// Overlay icon looks rather annoying than useful, so it's disabled by default
// for the common user and can be enabled by ini setting only (Ornis)
...
void CemuleDlg::UpdateStatusBarProgress() {
  if (m_pTaskbarList && thePrefs.IsWin7TaskbarGoodiesEnabled()) {
    ...
    float finishedsize =
        theApp.emuledlg->transferwnd->GetDownloadList()->GetFinishedSize();
    ...
    if (globalSize == 0) {
      ...
      else if (theStats.m_dwOverallStatus &
               STATE_DOWNLOADING) // something downloading
      ...
      if (overallProgress != m_prevProgress) {
        ...
        m_pTaskbarList->SetProgressValue(
            m_hWnd, (ULONGLONG)(overallProgress * 100), 100);
      }
    }
    ...
    if (thePrefs.IsShowUpDownIconInTaskbar()) {
      bool bUp =
          theApp.emuledlg->transferwnd->GetUploadList()->GetItemCount() > 0;
      ...
      if (m_ovlIcon != newicon) {
        m_ovlIcon = newicon;
        m_pTaskbarList->SetOverlayIcon(m_hWnd, m_ovlIcon,
                                       _T("eMule Up/Down Indicator"));
      }
    }
  }
}
...
void CemuleDlg::SetTaskbarIconColor() {
  ...
  if (thePrefs.IsRunningAeroGlassTheme()) {
    ...
    if (hDWMAPI) {
      ...
      (FARPROC &)pfnDwmGetColorizationColor =
      ...
      if (pfnDwmGetColorizationColor != NULL) {
        ...
        if (pfnDwmGetColorizationColor(&dwGlassColor, &bOpaque) == S_OK) {
          ...
          if (byAlpha < 200 && !bOpaque) {
            // on transparent themes we can never figure out what exact color is
            // shown (if we could in real time?), but given that a color is
            // blended against the background, it is a good guess that a bright
            // speedbar will be the best solution in most cases
            bTransparent = true;
          }
        }
      }
    ...
    if (g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed()) {
      ...
      VERIFY(ptmpWnd->Create(_T("STATIC"), _T("Tmp"), 0, CRect(0, 0, 10, 10),
                             this, 1235));
      VERIFY(g_xpStyle.SetWindowTheme(ptmpWnd->GetSafeHwnd(),
                                      L"TrayNotifyHoriz", NULL) == S_OK);
      HTHEME hTheme =
          g_xpStyle.OpenThemeData(ptmpWnd->GetSafeHwnd(), L"TrayNotify");
      if (hTheme != NULL) {
        if (g_xpStyle.GetThemeColor(hTheme, TNP_BACKGROUND, 0,
                                    TMT_FILLCOLORHINT, &cr) != S_OK)
          ASSERT(0);
  ...
  uint16 iBrightness =
      (uint16)sqrt(((iRed * iRed * 0.241f) + (iGreen * iGreen * 0.691f) +
                    (iBlue * iBlue * 0.068f)));
  ASSERT(iBrightness <= 255);
  bBrightTaskbarIconSpeed = iBrightness < 132;
  DebugLog(_T("Taskbar Notifier Color: R:%u G:%u B:%u, Brightness: %u, ")
           _T("Transparent: %s"),
           iRed, iGreen, iBlue, iBrightness,
           bTransparent ? _T("Yes") : _T("No"));
  thePrefs.SetStatsColor(11, ((bBrightTaskbarIconSpeed || bTransparent)
                                  ? RGB(255, 255, 255)
                                  : RGB(0, 0, 0)));
}