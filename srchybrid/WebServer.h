...
private:
  ...
  static void _GetPlainResString(CString &rstrOut, UINT nID,
                                 bool noquote = true);
  static int _GzipCompress(Bytef *dest, uLongf *destLen, const Bytef *source,
                           uLong sourceLen, int level);
  static CString _LoadTemplate(const CString &sAll,
                               const CString &sTemplateName);
  ...
  static bool _IsSessionAdmin(const ThreadData &Data,
                              const CString &strSsessionID);
  static CString _GetPermissionDenied();
  static CString _GetDownloadGraph(const ThreadData &Data,
                                   const CString &filehash);
  static void _InsertCatBox(CString &Out, int preselect, LPCTSTR boxlabel,
                            bool jump, bool extraCats, const CString &sSession,
                            const CString &sFileHash, bool ed2kbox = false);
  ...
  static void _MakeTransferList(CString &Out, CWebServer *pThis,
                                const ThreadData &Data, void *FilesArray,
                                void *UploadArray, bool bAdmin);
  ...
  static void _GetClientversionImage(const CUpDownClient &client,
                                     TCHAR pSoft[2]);

  bool _GetIsTempDisabled() const { return m_bIsTempDisabled; } // never used

  // comparators for quick sort
  static int AFX_CDECL _DownloadCmp(void *prm, void const *pv1,
                                    void const *pv2);
