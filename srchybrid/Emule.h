...
#define PORTTESTURL                                                            \
  _T("https://porttest.emule-project.net/")                                    \
  _T("connectiontest.php?tcpport=%i&udpport=%i&lang=%i")
...
public:
  ...
  void IgnoreClipboardLinks(const CString &strLinks) {
  ...
  uint32 GetID();
  uint32 GetED2KPublicIP() const; // return current (valid) public IP or 0 if
                                  // unknown (ignore KAD connection)
  uint32
  GetPublicIP() const; // return current (valid) public IP or 0 if unknown
  void SetPublicIP(const uint32 dwIP);
  void ResetStandByIdleTimer();

  // because nearly all icons we are loading are 16x16, the default size is
  // specified as 16 and not as 32 nor LR_DEFAULTSIZE
  HICON LoadIcon(LPCTSTR lpszResourceName, int cx = 16, int cy = 16,
                 UINT uFlags = LR_DEFAULTCOLOR) const;
  ...
  CString GetSkinFileItem(LPCTSTR lpszResourceName,
...
public:
  // because nearly all icons we are loading are 16x16, the default size is
  // specified as 16 and not as 32 nor LR_DEFAULTSIZE
  explicit CTempIconLoader(LPCTSTR pszResourceID, int cx = 16, int cy = 16,
                           UINT uFlags = LR_DEFAULTCOLOR);
  explicit CTempIconLoader(UINT uResourceID, int cx = 16, int cy = 16,
                           UINT uFlags = LR_DEFAULTCOLOR);
  ~CTempIconLoader();
