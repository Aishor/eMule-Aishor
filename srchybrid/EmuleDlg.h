...
public:
  ...
  void ShowNotifier(LPCTSTR pszText, TbnMsg nMsgType, LPCTSTR pszLink = NULL,
                    bool bForceSoundOFF = false);
  ...
  void ApplyListFont(LPLOGFONT pFont);
  ...
  void StartUPnP(bool bReset = true, uint16 nForceTCPPort = 0,
                 uint16 nForceUDPPort = 0);
...
protected:
  ...
  HICON m_icoSysTrayConnected;    // do not use this icon for anything but the
                                  // system tray!!!
  HICON m_icoSysTrayDisconnected; // do not use this icon for anything but the
                                  // system tray!!!
  HICON m_icoSysTrayLowID; // do not use this icon for anything but the system
                           // tray!!!
  ...
  // Startup Timer
  UINT_PTR m_hTimer;
  static void CALLBACK StartupTimer(HWND hwnd, UINT uiMsg, UINT_PTR idEvent,
                                    DWORD dwTime) noexcept;

  // UPnP TimeOutTimer
  UINT_PTR m_hUPnPTimeOutTimer;
  static void CALLBACK UPnPTimeOutTimer(HWND hwnd, UINT uiMsg, UINT_PTR idEvent,
                                        DWORD dwTime) noexcept;
