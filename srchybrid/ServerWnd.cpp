...
CServerWnd::CServerWnd(CWnd *pParent /*=NULL*/)
    : CResizableDialog(CServerWnd::IDD, pParent), icon_srvlist(), m_cfDef(),
      m_cfBold(), m_pacServerMetURL(), debug() {
...
BOOL CServerWnd::OnInitDialog() {
  ...
  // tricky window update problem! If that style is not specified there are
  // troubles with right clicking into the control for the very first time!?
#define LOG_PANE_RICHEDIT_STYLES                                               \
  ...
  if (servermsgbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_SERVMSG)) {
    ...
    servermsgbox->SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                              MAKELONG(3, 3));
    ...
    // Enlace a GitHub (clickeable)
    servermsgbox->AppendText(GetResString(IDS_PROJECT_GITHUB_TEXT));
    servermsgbox->AppendHyperLink(GetResString(IDS_PROJECT_GITHUB_URL), NULL,
    ...
    // Enlace Ko-fi (clickeable)
    servermsgbox->AppendText(GetResString(IDS_KOFI_TEXT));
    servermsgbox->AppendHyperLink(GetResString(IDS_KOFI_URL), NULL,
                                  GetResString(IDS_KOFI_URL), NULL);
    servermsgbox->AppendText(_T("\n"));

    // Enlace PayPal (clickeable)
    servermsgbox->AppendText(GetResString(IDS_PAYPAL_TEXT));
    servermsgbox->AppendHyperLink(GetResString(IDS_PAYPAL_URL), NULL,
                                  GetResString(IDS_PAYPAL_URL), NULL);
    servermsgbox->AppendText(_T("\n\n"));
  }
  ...
  if (logbox->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_LOGBOX)) {
    ...
    logbox->SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                        MAKELONG(3, 3));
  ...
  if (debuglog->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_DEBUG_LOG)) {
    ...
    debuglog->SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                          MAKELONG(3, 3));
    if (theApp.m_fontLog.m_hObject)
  ...
  ti.iImage = 1;
  VERIFY(StatusSelector.InsertItem(StatusSelector.GetItemCount(), &ti) ==
         PaneServerInfo);
  ...
  AddAnchor(IDC_LOGRESET,
            MIDDLE_RIGHT); // avoid resizing GUI glitches with the tab control
                           // by adding this control as the last one (Z-order)
  // The resizing of those log controls (rich edit controls) works 'better' when
  // added as last anchors (?)
  ...
  // Set the tab control to the bottom of the z-order. This solves a lot of
  // strange repainting problems with the rich edit controls (the log panes).
  ::SetWindowPos(StatusSelector, HWND_BOTTOM, 0, 0, 0, 0,
                 SWP_NOACTIVATE | SWP_NOCOPYBITS | SWP_NOMOVE | SWP_NOSIZE);
  ...
  if (thePrefs.GetRestoreLastLogPane()) {
    if (thePrefs.GetLastLogPaneID() >= 0 &&
        thePrefs.GetLastLogPaneID() < StatusSelector.GetItemCount()) {
  ...
  m_MyInfo.SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
                       MAKELONG(3, 3));
  ...
  if (thePrefs.GetUseAutocompletion()) {
    ...
    if (m_pacServerMetURL->Bind(::GetDlgItem(m_hWnd, IDC_SERVERMETURL),
                                ACO_UPDOWNKEYDROPSLIST | ACO_AUTOSUGGEST |
                                    ACO_FILTERPREFIXES))
      m_pacServerMetURL->LoadList(thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) +
                                  SERVERMET_STRINGS_PROFILE);
  ...
  m_wndSplitter.CreateWnd(WS_CHILD | WS_VISIBLE, rcSpl, this,
                          IDC_SPLITTER_SERVER);
...
bool CServerWnd::UpdateServerMetFromURL(const CString &strURL) {
  ...
  // add entered URL to LRU list even if it's not yet known whether we can
  // download from this URL (it's just more convenient this way)
  ...
  if (dlgDownload.DoModal() != IDOK) {
    LogError(LOG_STATUSBAR, GetResString(IDS_ERR_FAILEDDOWNLOADMET),
             (LPCTSTR)strURL);
    return false;
  }
...
bool CServerWnd::AddServer(uint16 nPort, const CString &strAddress,
                           const CString &strName, bool bShowErrorMB) {
  ...
  if (!serverlistctrl.AddServer(toadd, true)) {
    CServer *pFoundServer = theApp.serverlist->GetServerByAddress(
        toadd->GetAddress(), toadd->GetPort());
    if (pFoundServer == NULL && toadd->GetIP() != 0)
      pFoundServer =
          theApp.serverlist->GetServerByIPTCP(toadd->GetIP(), toadd->GetPort());
    if (pFoundServer) {
      static TCHAR const _aszServerPrefix[] = _T("Server");
      if (_tcsnicmp(toadd->GetListName(), _aszServerPrefix,
                    _countof(_aszServerPrefix) - 1) != 0) {
...
void CServerWnd::OnBnClickedUpdateServerMetFromUrl() {
  ...
  if (strURL.Trim().IsEmpty()) {
    if (thePrefs.addresses_list.IsEmpty())
      ...
      for (POSITION pos = thePrefs.addresses_list.GetHeadPosition();
           pos != NULL;)
...
void CServerWnd::OnBnClickedResetLog() {
  ...
  if (cur_sel == PaneServerInfo) {
    servermsgbox->Reset();
    // the statusbar does not contain any server log related messages, so it's
    // not cleared.
  }
}
...
void CServerWnd::UpdateLogTabSelection() {
  ...
  if (cur_sel == PaneVerboseLog) {
    ...
    if (debuglog->IsAutoScroll() &&
        (StatusSelector.GetItemState(cur_sel, TCIS_HIGHLIGHTED) &
         TCIS_HIGHLIGHTED))
...
BOOL CServerWnd::PreTranslateMessage(MSG *pMsg) {
  ...
  if (pMsg->message == WM_KEYDOWN) {
    // Don't handle Ctrl+Tab in this window. It will be handled by the main
    // window.
    ...
    case VK_DELETE:
      if (m_pacServerMetURL && m_pacServerMetURL->IsBound() &&
          pMsg->hwnd == GetDlgItem(IDC_SERVERMETURL)->m_hWnd)
    ...
    case VK_RETURN:
      ...
      if (pMsg->hwnd == GetDlgItem(IDC_SERVERMETURL)->m_hWnd) {
        if (m_pacServerMetURL && m_pacServerMetURL->IsBound()) {
          ...
          if (!strText.IsEmpty()) {
            SetDlgItemText(IDC_SERVERMETURL,
                           _T("")); // this seems to be the only chance to let
                                    // the drop-down list to disappear
            SetDlgItemText(IDC_SERVERMETURL, strText);
            static_cast<CEdit *>(GetDlgItem(IDC_SERVERMETURL))
...
bool CServerWnd::SaveServerMetStrings() {
  if (m_pacServerMetURL == NULL)
    return false;
  return m_pacServerMetURL->SaveList(
      thePrefs.GetMuleDirectory(EMULE_CONFIGDIR) + SERVERMET_STRINGS_PROFILE);
}
...
void CServerWnd::OnEnLinkServerBox(LPNMHDR pNMHDR, LRESULT *pResult) {
  ...
  if (pEnLink && pEnLink->msg == WM_LBUTTONDOWN) {
    CString strUrl;
    servermsgbox->GetTextRange(pEnLink->chrg.cpMin, pEnLink->chrg.cpMax,
                               strUrl);
...
void CServerWnd::ResetHistory() {
  if (m_pacServerMetURL != NULL) {
    GetDlgItem(IDC_SERVERMETURL)
        ->SendMessage(WM_KEYDOWN, VK_ESCAPE, 0x00510001);
    m_pacServerMetURL->Clear();
  }
}
...
void CServerWnd::OnSvrTextChange() {
  GetDlgItem(IDC_ADDSERVER)
      ->EnableWindow(GetDlgItem(IDC_IPADDRESS)->GetWindowTextLength());
  GetDlgItem(IDC_UPDATESERVERMETFROMURL)
      ->EnableWindow(GetDlgItem(IDC_SERVERMETURL)->GetWindowTextLength() > 0);
}
