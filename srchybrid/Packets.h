...
// Copyright (C)2002-2026 Merkur ( strEmail.Format("%s@%s", "devteam",
...
public:
  ...
  explicit Packet(CSafeMemFile &datafile, uint8 protocol = OP_EDONKEYPROT,
                  uint8 ucOpcode = 0x00);
  Packet(const CStringA &str, uint8 protocol, uint8 ucOpcode);
  Packet(uint8 in_opcode, uint32 in_size, uint8 protocol = OP_EDONKEYPROT,
         bool bFromPartFile = true);
  Packet(char *pPacketPart, uint32 nSize, bool bLast,
         bool bFromPartFile = true); // only used for split packets!
  ...
  virtual uint32 GetRealPacketSize() const {
    return (uint32)(size + PACKET_HEADER_SIZE);
  }
  // bool	IsSplitted() const					{ return
  // m_bSplitted; } bool	IsLastSplitted() const
  // { return m_bLastSplitted; }
  ...
  char *pBuffer;
  uint32 size;
  uint32 uStatsPayLoad; // only for statistics and co., not used within the
                        // class itself
  uint8 opcode;
  uint8 prot;
...
public:
  ...
  virtual void AttachPacket(char *pPacketData, UINT uPacketSize,
                            bool bFromPartFile = false);
  virtual char *DetachPacket();
...
public:
  ...
  CTag(uint8 uName, BYTE *pucAttachData,
       uint32 nSize); // data gets attached (and deleted later on)
  ...
  bool IsInt64(bool bOrInt32 = true) const {
    return m_uType == TAGTYPE_UINT64 || (bOrInt32 && IsInt());
  }
  ...
  const CString &GetStr() const {
    ASSERT(IsStr());
    return const_cast<CString &>(*m_pstrVal);
  }
  ...
  // CTag *CloneTag()							{ return
  // new CTag(*this); }

  bool WriteTagToFile(CFileDataIO &file,
                      EUTF8str eStrEncode = UTF8strNone) const; // old eD2K tags
  bool
  WriteNewEd2kTag(CFileDataIO &data,
                  EUTF8str eStrEncode = UTF8strNone) const; // new eD2K tags

  CString GetFullInfo(
      CString (*pfnDbgGetFileMetaTagName)(UINT uMetaTagID) = NULL) const;
