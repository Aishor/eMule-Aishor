...
bool CPreferences::m_bAdjustNTFSDaylightFileTime =
    false; //'true' causes rehashing in XP and above when DST switches on/off
...
CPreferences::~CPreferences() { delete prefsExt; }

LPCTSTR CPreferences::GetConfigFile() { return theApp.m_pszProfileName; }

void CPreferences::MovePreferences(EDefaultDirectory eSrc, LPCTSTR const sFile,
                                   const CString &dst) {
...
void CPreferences::Init() {
  ...
  //
  CFileFind ff;
  for (BOOL bFound = ff.FindFile(GetMuleDirectory(EMULE_EXECUTABLEDIR) +
                                 _T("eMule*.log"));
       bFound;) {
    bFound = ff.FindNextFile();
    if (!ff.IsDirectory() && !ff.IsSystem() && !ff.IsHidden())
      ::MoveFile(ff.GetFilePath(),
                 GetMuleDirectory(EMULE_LOGDIR) + ff.GetFileName());
  }
  ff.Close();
  ...
  if (!preffile ||
      fread(prefsExt, sizeof(Preferences_Ext_Struct), 1, preffile) != 1 ||
      ferror(preffile))
    SetStandardValues();
  else {
  ...
  CStdioFile sdirfile;
  if (sdirfile.Open(strFullPath,
                    CFile::modeRead | CFile::shareDenyWrite |
                        (bIsUnicodeFile ? CFile::typeBinary : 0))) {
    try {
      if (bIsUnicodeFile)
      ...
      while (sdirfile.ReadString(toadd)) {
        ...
        if (!toadd.IsEmpty()) {
          ...
          if (IsShareableDirectory(toadd) &&
              (m_bKeepUnavailableFixedSharedDirs || DirAccsess(toadd)))
  ...
  if (!::PathFileExists(GetMuleDirectory(EMULE_INCOMINGDIR)) &&
    ...
    CString strError;
    strError.Format(GetResString(IDS_ERR_CREATE_DIR),
                    (LPCTSTR)GetResString(IDS_PW_INCOMING),
                    (LPCTSTR)GetMuleDirectory(EMULE_INCOMINGDIR),
                    (LPCTSTR)GetErrorMessage(::GetLastError()));
    AfxMessageBox(strError, MB_ICONERROR);

    m_strIncomingDir = GetDefaultDirectory(
        EMULE_INCOMINGDIR, true); // will also try to create it if needed
    if (!::PathFileExists(GetMuleDirectory(EMULE_INCOMINGDIR))) {
      strError.Format(GetResString(IDS_ERR_CREATE_DIR),
                      (LPCTSTR)GetResString(IDS_PW_INCOMING),
                      (LPCTSTR)GetMuleDirectory(EMULE_INCOMINGDIR),
  ...
  if (!::PathFileExists(GetTempDir()) && !::CreateDirectory(GetTempDir(), 0)) {
    CString strError;
    strError.Format(GetResString(IDS_ERR_CREATE_DIR),
                    (LPCTSTR)GetResString(IDS_PW_TEMP), GetTempDir(),
                    (LPCTSTR)GetErrorMessage(::GetLastError()));
    AfxMessageBox(strError, MB_ICONERROR);

    tempdir[0] = GetDefaultDirectory(
        EMULE_TEMPDIR, true); // will also try to create it if needed;
  ...
  if (!::PathFileExists(GetMuleDirectory(EMULE_SKINDIR)) &&
      !::CreateDirectory(GetMuleDirectory(EMULE_SKINDIR), 0))
    m_strSkinProfileDir = GetDefaultDirectory(
        EMULE_SKINDIR, true); // will also try to create it if needed

  // Create 'toolbars' directory
  if (!::PathFileExists(GetMuleDirectory(EMULE_TOOLBARDIR)) &&
      !::CreateDirectory(GetMuleDirectory(EMULE_TOOLBARDIR), 0))
    m_sToolbarBitmapFolder = GetDefaultDirectory(
        EMULE_TOOLBARDIR, true); // will also try to create it if needed;
}
...
bool CPreferences::IsTempFile(const CString &rstrDirectory,
  ...
  // do not share a file from the temp directory, if it matches one of the
  // following patterns
  ...
  for (unsigned i = 0; i < _countof(_apszNotSharedExts); ++i) {
    ...
    if (_stscanf(strNameLower, _apszNotSharedExts[i], &uNum, &iChar) == 2 &&
        iChar == _T('|'))
      return true;
  }
...
uint64 CPreferences::GetMaxDownloadInBytesPerSec(bool dynamic) {
  // don't be a Lam3r :)
  uint64 maxup;
  if (dynamic && thePrefs.IsDynUpEnabled() &&
      theApp.uploadqueue->GetWaitingUserCount() > 0 &&
      theApp.uploadqueue->GetDatarate() > 0)
...
// -khaos--+++> A whole bunch of methods! Keep going until you reach the end
// tag.
void CPreferences::SaveStats(int bBackUp) {
  // This function saves all of the new statistics in my addon.  It is also used
  // to save backups for the Reset Stats function, and the Restore Stats
  // function (Which is actually LoadStats) bBackUp = 0: DEFAULT; save to
  // statistics.ini bBackUp = 1: Save to statbkup.ini, which is used to restore
  // after a reset bBackUp = 2: Save to statbkuptmp.ini, which is temporarily
  // created during a restore and then renamed to statbkup.ini
  ...
  // Save cumulative statistics to statistics.ini, going in the order they
  // appear in CStatisticsDlg::ShowStatistics. We do NOT SET the values in prefs
  // struct here.

  // Save Cum Down Data
  ini.WriteUInt64(_T("TotalDownloadedBytes"),
                  theStats.sessionReceivedBytes + GetTotalDownloaded());
  ...
  ini.WriteUInt64(_T("LostFromCorruption"),
                  cumLostFromCorruption + sesLostFromCorruption);
  ini.WriteUInt64(_T("SavedFromCompression"),
                  sesSavedFromCompression + cumSavedFromCompression);
  ...
  ini.WriteUInt64(_T("DownOverheadTotal"),
                  theStats.GetDownDataOverheadFileRequest() +
  ...
  ini.WriteUInt64(_T("DownOverheadFileReq"),
                  theStats.GetDownDataOverheadFileRequest() +
                      GetDownOverheadFileReq());
  ini.WriteUInt64(_T("DownOverheadSrcEx"),
                  theStats.GetDownDataOverheadSourceExchange() +
                      GetDownOverheadSrcEx());
  ini.WriteUInt64(_T("DownOverheadServer"),
                  theStats.GetDownDataOverheadServer() +
                      GetDownOverheadServer());
  ini.WriteUInt64(_T("DownOverheadKad"),
                  theStats.GetDownDataOverheadKad() + GetDownOverheadKad());

  ini.WriteUInt64(_T("DownOverheadTotalPackets"),
  ...
  ini.WriteUInt64(_T("DownOverheadFileReqPackets"),
                  theStats.GetDownDataOverheadFileRequestPackets() +
                      GetDownOverheadFileReqPackets());
  ini.WriteUInt64(_T("DownOverheadSrcExPackets"),
                  theStats.GetDownDataOverheadSourceExchangePackets() +
                      GetDownOverheadSrcExPackets());
  ini.WriteUInt64(_T("DownOverheadServerPackets"),
                  theStats.GetDownDataOverheadServerPackets() +
                      GetDownOverheadServerPackets());
  ini.WriteUInt64(_T("DownOverheadKadPackets"),
                  theStats.GetDownDataOverheadKadPackets() +
                      GetDownOverheadKadPackets());

  // Save Cumulative Upline Statistics
  ini.WriteUInt64(_T("TotalUploadedBytes"),
                  theStats.sessionSentBytes + GetTotalUploaded());
  ini.WriteInt(_T("UpSuccessfulSessions"),
               theApp.uploadqueue->GetSuccessfullUpCount() +
                   GetUpSuccessfulSessions());
  ini.WriteInt(_T("UpFailedSessions"),
               theApp.uploadqueue->GetFailedUpCount() + GetUpFailedSessions());
  ...
  ini.WriteUInt64(_T("UpOverheadTotal"),
                  theStats.GetUpDataOverheadFileRequest() +
                      theStats.GetUpDataOverheadSourceExchange() +
                      theStats.GetUpDataOverheadServer() +
                      theStats.GetUpDataOverheadKad() +
                      theStats.GetUpDataOverheadOther() + GetUpOverheadTotal());
  ini.WriteUInt64(_T("UpOverheadFileReq"),
                  theStats.GetUpDataOverheadFileRequest() +
                      GetUpOverheadFileReq());
  ini.WriteUInt64(_T("UpOverheadSrcEx"),
                  theStats.GetUpDataOverheadSourceExchange() +
                      GetUpOverheadSrcEx());
  ini.WriteUInt64(_T("UpOverheadServer"),
                  theStats.GetUpDataOverheadServer() + GetUpOverheadServer());
  ini.WriteUInt64(_T("UpOverheadKad"),
                  theStats.GetUpDataOverheadKad() + GetUpOverheadKad());

  ini.WriteUInt64(_T("UpOverheadTotalPackets"),
                  theStats.GetUpDataOverheadFileRequestPackets() +
  ...
  ini.WriteUInt64(_T("UpOverheadFileReqPackets"),
                  theStats.GetUpDataOverheadFileRequestPackets() +
                      GetUpOverheadFileReqPackets());
  ini.WriteUInt64(_T("UpOverheadSrcExPackets"),
                  theStats.GetUpDataOverheadSourceExchangePackets() +
                      GetUpOverheadSrcExPackets());
  ini.WriteUInt64(_T("UpOverheadServerPackets"),
                  theStats.GetUpDataOverheadServerPackets() +
                      GetUpOverheadServerPackets());
  ini.WriteUInt64(_T("UpOverheadKadPackets"),
                  theStats.GetUpDataOverheadKadPackets() +
                      GetUpOverheadKadPackets());
  ...
  // Overall Run Time
  ini.WriteInt(_T("ConnRunTime"),
               (UINT)((::GetTickCount() - theStats.starttime) / SEC2MS(1) +
                      GetConnRunTime()));

  // Number of Reconnects
  ini.WriteInt(_T("ConnNumReconnects"),
               GetConnNumReconnects() + theStats.reconnects -
                   static_cast<uint32>(theStats.reconnects > 0));

  // Average Connections
  if (theApp.serverconnect->IsConnected())
    ini.WriteInt(_T("ConnAvgConnections"),
                 (UINT)((theApp.listensocket->GetAverageConnections() +
                         cumConnAvgConnections) /
                        2));
  ...
  if (theApp.listensocket->GetMaxConnectionReached() > 0)
    ini.WriteInt(_T("ConnMaxConnLimitReached"),
                 theApp.listensocket->GetMaxConnectionReached() +
                     cumConnMaxConnLimitReached);

  // Time Stuff...
  ini.WriteInt(_T("ConnTransferTime"),
               GetConnTransferTime() + theStats.GetTransferTime());
  ini.WriteInt(_T("ConnUploadTime"),
               GetConnUploadTime() + theStats.GetUploadTime());
  ini.WriteInt(_T("ConnDownloadTime"),
               GetConnDownloadTime() + theStats.GetDownloadTime());
  ini.WriteInt(_T("ConnServerDuration"),
               GetConnServerDuration() + theStats.GetServerDuration());

  // Compare and Save Server Records
  uint32 servtotal, servfail, servuser, servfile, servlowiduser, servtuser,
      servtfile;
  float servocc;
  theApp.serverlist->GetStatus(servtotal, servfail, servuser, servfile,
                               servlowiduser, servtuser, servtfile, servocc);
...
void CPreferences::SetRecordStructMembers() {
  // The purpose of this function is to be called from
  // CStatisticsDlg::ShowStatistics() This was easier than making a bunch of
  // functions to interface with the record members of the prefs struct from
  // ShowStatistics.

  // This function is going to compare current values with previously saved
  // records, and if the current values are greater, the corresponding member of
  // prefs will be updated. We will not write to INI here, because this code is
  // going to be called a lot more often than SaveStats()  - Khaos
...
void CPreferences::SaveCompletedDownloadsStat() {
  ...
  CIni ini(GetMuleDirectory(EMULE_CONFIGDIR) + _T("statistics.ini"),
           _T("Statistics"));
...
void CPreferences::Add2SessionTransferData(UINT uClientID, UINT uClientPort,
                                           BOOL bFromPF, BOOL bUpDown,
                                           uint32 bytes, bool sentToFriend) {
  //	This function adds the transferred bytes to the appropriate variables,
  //	as well as to the totals for all clients. - Khaos
  //	PARAMETERS:
  //	uClientID - The identifier for which client software sent or received
  // this data, e.g. SO_EMULE 	uClientPort - The remote port of the client that
  // sent or received this data, e.g. 4662 	bFromPF - Applies only to
  // uploads. True is from partfile, False is from non-partfile. 	bUpDown
  // - True is Up, False is Down 	bytes - Number of bytes sent by the
  // client.  Subtract header before calling.
  ...
  if (bUpDown) {
    ...
    switch (uClientPort) {
      ...
      // A statistical analysis of all data sent from every single port/domain
      // is beyond the scope of this add-on.
    case 4662:
...
bool CPreferences::LoadStats(int loadBackUp) {
  ...
  // loadBackUp = 0: Load the stats normally like we used to do in
  // LoadPreferences loadBackUp = 1: Load the stats from statbkup.ini and create
  // a backup of the current stats.  Also, do not initialize session variables.
  const CString &sConfDir(GetMuleDirectory(EMULE_CONFIGDIR));
  ...
  case 1:
    ...
    if (!findBackUp.FindFile(sINI))
      return false;
    SaveStats(2); // Save our temp backup of current values to statbkuptmp.ini,
                  // we will be renaming it at the end of this function.
    break;
  case 0:
  default:
    // for transition...
  ...
  cumDownOverheadFileReqPackets =
      ini.GetUInt64(_T("DownOverheadFileReqPackets"));
  ...
  // Smart Load For Restores - Don't overwrite records that are greater than the
  // backed up ones
  if (loadBackUp == 1) {
    // Load records for servers / network
    if ((uint32)ini.GetInt(_T("SrvrsMostWorkingServers")) >
        cumSrvrsMostWorkingServers)
      cumSrvrsMostWorkingServers = ini.GetInt(_T("SrvrsMostWorkingServers"));

    if ((uint32)ini.GetInt(_T("SrvrsMostUsersOnline")) >
        cumSrvrsMostUsersOnline)
    ...
    if ((uint32)ini.GetInt(_T("SharedMostFilesShared")) >
        cumSharedMostFilesShared)
    ...
    // Check to make sure the backup of the values we just overwrote exists.  If
    // so, rename it to the backup file. This allows us to undo a restore, so to
    // speak, just in case we don't like the restored values...
    ...
    if (findBackUp.FindFile(sINIBackUp)) {
      ...
      ::MoveFile(sINIBackUp, sINI); // Rename our temporary backup to the normal
                                    // statbkup.ini filename.
    }

    // Since we know this is a restore, now we should call ShowStatistics to
    // update the data items to the new ones we just loaded. Otherwise user is
    // left waiting around for the tick counter to reach the next automatic
    // update (Depending on setting in prefs)
  ...
  if (!fileex || (stat_datetimeLastReset == 0 && totalDownloadedBytes == 0 &&
                  totalUploadedBytes == 0)) {
    time_t timeNow;
    time(&timeNow);
    stat_datetimeLastReset = timeNow;
...
CString CPreferences::GetStatsLastResetStr(bool formatLong) {
  ...
  if (GetStatsLastResetLng()) {
    ...
    if (statsReset) {
      _tcsftime(szDateReset, _countof(szDateReset),
                formatLong ? (LPCTSTR)GetDateTimeFormat() : _T("%c"),
                statsReset);
      if (*szDateReset)
...
bool CPreferences::Save() {
  ...
  FILE *preffile =
      _tfsopen(strPrefPath + stmp, _T("wb"), _SH_DENYWR); // keep contents
  if (preffile) {
    prefsExt->version = PREFFILE_VERSION;
    md4cpy(prefsExt->userhash, userhash);
    prefsExt->EmuleWindowPlacement = EmuleWindowPlacement;
    error =
        (fwrite(prefsExt, sizeof(Preferences_Ext_Struct), 1, preffile) != 1);
    error |= (fclose(preffile) != 0);
    if (!error)
      error = !MoveFileEx(strPrefPath + stmp, strPrefPath,
  ...
  CStdioFile file;
  if (file.Open(strSharesPath + stmp, CFile::modeCreate | CFile::modeWrite |
    ...
    try {
      ...
      file.Close();
      error |= (MoveFileEx(strSharesPath + stmp, strSharesPath,
                           MOVEFILE_REPLACE_EXISTING) == 0);
    } catch (CFileException *ex) {
      if (thePrefs.GetVerbose())
        AddDebugLogLine(true, _T("Failed to save %s%s"), (LPCTSTR)strSharesPath,
...
void CPreferences::SavePreferences() {
  ...
  ini.WriteInt(_T("SplitterbarPositionStat_HL"),
               splitterbarPositionStat_HL + 1);
  ini.WriteInt(_T("SplitterbarPositionStat_HR"),
               splitterbarPositionStat_HR + 1);
  ...
  ini.WriteBool(_T("IRCIgnoreEmuleAddFriendMsgs"),
                m_bIRCIgnoreEmuleAddFriendMsgs);
  ini.WriteBool(_T("IRCAllowEmuleAddFriend"), m_bIRCAllowEmuleAddFriend);
  ini.WriteBool(_T("IRCIgnoreEmuleSendLinkMsgs"),
                m_bIRCIgnoreEmuleSendLinkMsgs);
  ...
  ini.WriteBool(_T("DebugSourceExchange"),
                m_bDebugSourceExchange); // do *not* use the according 'Get...'
                                         // function here!
  ini.WriteBool(_T("LogBannedClients"),
                m_bLogBannedClients); // do *not* use the according 'Get...'
                                      // function here!
  ini.WriteBool(_T("LogRatingDescReceived"),
                m_bLogRatingDescReceived); // do *not* use the according
                                           // 'Get...' function here!
  ini.WriteBool(
      _T("LogSecureIdent"),
      m_bLogSecureIdent); // do *not* use the according 'Get...' function here!
  ini.WriteBool(
      _T("LogFilteredIPs"),
      m_bLogFilteredIPs); // do *not* use the according 'Get...' function here!
  ini.WriteBool(
      _T("LogFileSaving"),
      m_bLogFileSaving); // do *not* use the according 'Get...' function here!
  ini.WriteBool(
      _T("LogA4AF"),
      m_bLogA4AF); // do *not* use the according 'Get...' function here!
  ini.WriteBool(_T("LogUlDlEvents"), m_bLogUlDlEvents);
#if defined(_DEBUG) || defined(USE_DEBUG_DEVICE)
  // following options are for debugging or when using an external debug device
  // viewer only.
  ...
  ini.DeleteKey(
      _T("FileBufferSizePref")); // delete old 'file buff size' setting
  ...
  ini.WriteBool(_T("UseSimpleTimeRemainingcomputation"),
                m_bUseOldTimeRemaining);
  ...
  ini.WriteBool(_T("SecureIdent"),
                m_bUseSecureIdent); // change the name in future version to
  ...
  ini.WriteBool(_T("USSUseMillisecondPingTolerance"),
                m_bDynUpUseMillisecondPingTolerance);
  ini.WriteInt(_T("USSPingTolerance"), m_iDynUpPingTolerance);
  ini.WriteInt(
      _T("USSPingToleranceMilliseconds"),
      m_iDynUpPingToleranceMilliseconds); // EastShare - Add by TAHO, USS limit
  ...
  ini.WriteInt(_T("WinXPSP2OrHigher"),
               static_cast<int>(IsRunningXPSP2OrHigher()));
  ...
  // ini.WriteString(_T("NotifierMailEncryptCertName"),
  // m_email.sEncryptCertName);
  ...
  ini.WriteBool(_T("EnableSearchResultSpamFilter"),
                m_bEnableSearchResultFilter);
  ...
  // Section: "Statistics"
  //
  ini.WriteInt(_T("statsConnectionsGraphRatio"), statsConnectionsGraphRatio,
               _T("Statistics"));
  ...
  ini.WriteBool(_T("HasCustomTaskIconColor"), m_bHasCustomTaskIconColor,
                _T("Statistics"));
...
void CPreferences::ResetStatsColor(int index) {
  static const COLORREF defcol[15] = {
      RGB(0, 0, 64),           RGB(192, 192, 255), RGB(128, 255, 128),
      RGB(0, 210, 0),          RGB(0, 128, 0),     RGB(255, 128, 128),
      RGB(200, 0, 0),          RGB(140, 0, 0),     RGB(150, 150, 255),
      RGB(192, 0, 192),        RGB(255, 255, 128), RGB(0, 0, 0),
      /**/ RGB(255, 255, 255), RGB(255, 255, 255), RGB(255, 190, 190)};
...
void CPreferences::GetAllStatsColors(int iCount, LPDWORD pdwColors) {
  ...
  if (cnt > sizeof m_adwStatsColors)
    memset(&pdwColors[sizeof m_adwStatsColors], 0,
           cnt - sizeof m_adwStatsColors);
}
...
void CPreferences::IniCopy(const CString &si, const CString &di) {
  ...
  // Do NOT write empty settings, this will mess up reading of default settings
  // in case there were no settings available at all (fresh emule install)!
  if (!sValue.IsEmpty())
...
void CPreferences::LoadPreferences() {
  ...
  m_bBetaNaggingDone =
      (ini.GetString(_T("BetaVersionNotified"), _T("")) == strCurrVersion);
#endif
  ...
  if (m_strIncomingDir
          .IsEmpty()) // We want GetDefaultDirectory to also create the folder,
                      // so we have to know if we use the default or not
  ...
  for (int iPos = 0; iPos >= 0;) {
    ...
    if (!bDup && (::PathFileExists(sTmp) || ::CreateDirectory(sTmp, NULL)) ||
        tempdir.IsEmpty())
      tempdir.Add(sTmp);
  }
  ...
  if (maxGraphUploadRate == UNLIMITED) {
    ...
    if (nOldUploadCapacity == 16 && ini.GetInt(_T("MaxUpload"), 12) == 12) {
      // either this is a complete new install, or the prior version used the
      // default value in both cases, set the new default values
  ...
  maxconnections =
      ini.GetInt(_T("MaxConnections"), GetRecommendedMaxConnections());
  ...
  // 0 is a valid value for the UDP port setting, as it is used for disabling
  // it.
  int iPort = ini.GetInt(_T("UDPPort"), INT_MAX /*invalid port value*/);
  udpport = (iPort == INT_MAX) ? thePrefs.GetRandomUDPPort() : (uint16)iPort;

  nServerUDPPort = (uint16)ini.GetInt(
      _T("ServerUDPPort"), -1); // 0 = Don't use UDP port for servers, -1 = use
                                // a random port (for backward compatibility)
  ...
  MaxConperFive =
      ini.GetInt(_T("MaxConnectionsPerFiveSeconds"), GetDefaultMaxConperFive());
  ...
  // since the minimize to tray button is not working under Aero (at least not
  // at this point), we enable map the minimize to tray on the minimize button
  // by default if Aero is running
  if (IsRunningAeroGlassTheme())
  ...
  m_bResolveSharedShellLinks =
      ini.GetBool(_T("ResolveSharedShellLinks"), false);
  m_bKeepUnavailableFixedSharedDirs =
      ini.GetBool(_T("KeepUnavailableFixedSharedDirs"), false);
  m_strYourHostname = ini.GetString(_T("YourHostname"), _T(""));
  m_bImportParts = false; // enable on demand for the current session only

  // Barry - New properties...
  m_bAutoConnectToStaticServersOnly =
      ini.GetBool(_T("AutoConnectStaticOnly"), false);
  ...
  notifierConfiguration =
      ini.GetString(_T("NotifierConfiguration"),
                    GetMuleDirectory(EMULE_CONFIGDIR) + _T("Notifier.ini"));
  ...
  notifierSoundType =
      (ENotifierSoundType)ini.GetInt(_T("NotifierUseSound"), ntfstNoSound);
  ...
  m_strDateTimeFormat4Lists =
      ini.GetString(_T("DateTimeFormat4Lists"), _T("%c"));

  m_strIRCServer =
      ini.GetString(_T("DefaultIRCServerNew"), _T("ircchat.emule-project.net"));
  ...
  m_bIRCIgnorePingPongMessages =
      ini.GetBool(_T("IRCIgnorePingPongMessages"), false);
  m_bIRCIgnoreEmuleAddFriendMsgs =
      ini.GetBool(_T("IRCIgnoreEmuleAddFriendMsgs"), false);
  m_bIRCAllowEmuleAddFriend = ini.GetBool(_T("IRCAllowEmuleAddFriend"), true);
  m_bIRCIgnoreEmuleSendLinkMsgs =
      ini.GetBool(_T("IRCIgnoreEmuleSendLinkMsgs"), false);
  ...
  m_iDebugSearchResultDetailLevel =
      ini.GetInt(_T("DebugSearchResultDetailLevel"), 0);
#else
  ...
  m_bAutomaticArcPreviewStart =
      ini.GetBool(_T("AutoArchivePreviewStart"), true);
  ...
  m_bShowUpDownIconInTaskbar =
      ini.GetBool(_T("ShowUpDownIconInTaskbar"), false);
  ...
  m_iQueueSize =
      (INT_PTR)ini.GetInt(_T("QueueSizePref"), 50) * 100; // old setting
  m_iQueueSize = ini.GetInt(_T("QueueSize"), (int)m_iQueueSize);

  m_iCommitFiles =
      ini.GetInt(_T("CommitFiles"), 1); // 1 = "commit" on application shutdown;
                                        // 2 = "commit" on each file saving
  ...
  m_bShowActiveDownloadsBold =
      ini.GetBool(_T("ShowActiveDownloadsBold"), false);
  ...
  m_strTemplateFile =
      ini.GetString(_T("WebTemplateFile"),
                    GetMuleDirectory(EMULE_EXECUTABLEDIR) + _T("eMule.tmpl"));
  // if emule is using the default, check if the file is in the config folder,
  // as it used to be val prior version and might be wanted by the user when
  // switching to a personalized template
  if (m_strTemplateFile.Compare(GetMuleDirectory(EMULE_EXECUTABLEDIR) +
                                _T("eMule.tmpl")) == 0)
  ...
  messageFilter = ini.GetStringLong(_T("MessageFilter"),
                                    _T("fastest download speed|fastest eMule"));
  commentFilter = ini.GetStringLong(_T("CommentFilter"),
                                    _T("http://|https://|ftp://|www.|ftp."));
  commentFilter.MakeLower();
  filenameCleanups = ini.GetStringLong(
      _T("FilenameCleanups"), _T("http|www.|.com|.de|.org|.net|shared|powered|")
                              _T("sponsored|sharelive|filedonkey|"));
  m_iExtractMetaData =
      ini.GetInt(_T("ExtractMetaData"), 1); // 0=disable, 1=mp3, 2=MediaDet
  if (m_iExtractMetaData > 1)
    m_iExtractMetaData = 1;
  m_bAdjustNTFSDaylightFileTime =
      ini.GetBool(_T("AdjustNTFSDaylightFileTime"), false);
  m_bRearrangeKadSearchKeywords =
      ini.GetBool(_T("RearrangeKadSearchKeywords"), true);
  ...
  m_bUseOldTimeRemaining =
      ini.GetBool(_T("UseSimpleTimeRemainingcomputation"), false);
  ...
  if (m_sToolbarBitmapFolder
          .IsEmpty()) // We want GetDefaultDirectory to also create the folder,
                      // so we have to know if we use the default or not
    m_sToolbarBitmapFolder = GetDefaultDirectory(EMULE_TOOLBARDIR, true);
  else
    slosh(m_sToolbarBitmapFolder);
  m_nToolbarLabels = (EToolbarLabelType)ini.GetInt(
      _T("ToolbarLabels"), CMuleToolbarCtrl::GetDefaultLabelType());
  m_bReBarToolbar = ini.GetBool(_T("ReBarToolbar"), 1);
  m_sizToolbarIconSize.cx = m_sizToolbarIconSize.cy =
      ini.GetInt(_T("ToolbarIconSize"), 32);
  m_iStraightWindowStyles = ini.GetInt(_T("StraightWindowStyles"), 0);
  m_bUseSystemFontForMainControls =
      ini.GetBool(_T("UseSystemFontForMainControls"), 0);
  ...
  if (ini.GetBinary(_T("HyperTextFont"), &pData, &uSize) &&
      uSize == sizeof m_lfHyperText)
  ...
  if (ini.GetBinary(_T("LogTextFont"), &pData, &uSize) &&
      uSize == sizeof m_lfLogText)
  ...
  m_bDynUpUseMillisecondPingTolerance =
      ini.GetBool(_T("USSUseMillisecondPingTolerance"), false);
  m_iDynUpPingTolerance = ini.GetInt(_T("USSPingTolerance"), 500);
  m_iDynUpPingToleranceMilliseconds =
      ini.GetInt(_T("USSPingToleranceMilliseconds"), 200);
  ...
  m_bPreferRestrictedOverUser =
      ini.GetBool(_T("PreferRestrictedOverUser"), false);
  ...
  m_bPartiallyPurgeOldKnownFiles =
      ini.GetBool(_T("PartiallyPurgeOldKnownFiles"), true);

  m_email.bSendMail =
      IsRunningXPSP2OrHigher() && ini.GetBool(_T("NotifierSendMail"), false);
  ...
  // m_email.sEncryptCertName = ini.GetString(_T("NotifierMailEncryptCertName"),
  // _T(""));
  ...
  m_bEnableSearchResultFilter =
      ini.GetBool(_T("EnableSearchResultSpamFilter"), true);

  ///////////////////////////////////////////////////////////////////////////
  // Section: "Proxy"
  //
  proxy.bEnablePassword =
      ini.GetBool(_T("ProxyEnablePassword"), false, _T("Proxy"));
  ...
  m_strStatsExpandedTreeItems =
      ini.GetString(_T("statsExpandedTreeItems"),
                    _T("111000000100000110000010000011110000010010"));
  CString buffer;
  for (unsigned i = 0; i < _countof(m_adwStatsColors); ++i) {
    buffer.Format(_T("StatColor%u"), i);
    m_adwStatsColors[i] = 0;
    if (_stscanf(ini.GetString(buffer, _T("")), _T("%li"),
                 (long *)&m_adwStatsColors[i]) != 1)
      ResetStatsColor(i);
  }
  ...
  m_nLastWorkingImpl =
      ini.GetInt(_T("LastWorkingImplementation"), 1 /*MiniUPnPLib*/);
...
void CPreferences::SaveCats() {
  ...
  strCatIniFilePath.Format(_T("%s")
                           _T("Category.ini"),
                           (LPCTSTR)GetMuleDirectory(EMULE_CONFIGDIR));
  ...
  for (INT_PTR i = 0; i < catArr.GetCount(); ++i) {
    ...
    ini.WriteBool(_T("downloadInAlphabeticalOrder"),
                  cmap->downloadInAlphabeticalOrder != FALSE);
  ...
  for (INT_PTR i = 0; i <= iNumCategories; ++i) {
    ...
    if (i != 0) { // All category
      ...
      if (!IsShareableDirectory(newcat->strIncomingPath) ||
          (!::PathFileExists(newcat->strIncomingPath) &&
    ...
    newcat->prio =
        ini.GetInt(_T("a4afPriority"), PR_NORMAL); // ZZ:DownloadManager
    newcat->filter = ini.GetInt(_T("Filter"), 0);
    newcat->filterNeg = ini.GetBool(_T("FilterNegator"), FALSE);
    newcat->ac_regexpeval =
        ini.GetBool(_T("AutoCatAsRegularExpression"), FALSE);
    ...
    newcat->downloadInAlphabeticalOrder = ini.GetBool(
        _T("downloadInAlphabeticalOrder"), FALSE); // ZZ:DownloadManager
...
void CPreferences::UpdateLastVC() {
  ...
  versioncheckLastAutomatic =
      safe_mktime(CTime::GetCurrentTime().GetLocalTm(&tmTemp));
}
...
void CPreferences::SetNetworkKademlia(bool val) { networkkademlia = val; }
...
CString CPreferences::GetVersionCheckURL() {
  return _T("https://github.com/Aishor/eMule-Aishor");
}
...
bool CPreferences::IsDefaultNick(const CString &strCheck) {
  ...
  return strCheck == _T("http://emule-project.net") ||
         strCheck == _T("https://emule-project.net");
}

void CPreferences::SetUserNick(LPCTSTR pszNick) { strNick = pszNick; }
...
bool CPreferences::GetUseReBarToolbar() {
  return GetReBarToolbar() &&
         theApp.m_ullComCtrlVer >= MAKEDLLVERULL(5, 8, 0, 0);
}
...
uint32 CPreferences::GetMaxGraphUploadRate(bool bEstimateIfUnlimited) {
  ...
  return (maxGraphUploadRateEstimated != 0) ? maxGraphUploadRateEstimated + 4
                                            : 16u;
}
...
void CPreferences::EstimateMaxUploadCap(uint32 nCurrentUpload) {
  if (maxGraphUploadRateEstimated + 1 < nCurrentUpload) {
    ...
    if (maxGraphUploadRate == UNLIMITED && theApp.emuledlg->statisticswnd)
      theApp.emuledlg->statisticswnd->SetARange(
          false, thePrefs.GetMaxGraphUploadRate(true));
  }
}
...
bool CPreferences::CanFSHandleLargeFiles(int nForCat) {
  for (INT_PTR i = tempdir.GetCount(); --i >= 0;)
    if (!IsFileOnFATVolume(tempdir[i]))
      return !IsFileOnFATVolume((nForCat > 0)
                                    ? GetCatPath(nForCat)
                                    : GetMuleDirectory(EMULE_INCOMINGDIR));
...
uint16 CPreferences::GetRandomTCPPort() {
  ...
  do {
    ...
    // The port is assumed to be available by default. If we got a table of
    // currently used TCP ports, verify that the port is not in this table.
    bPortIsFree = true;
    if (pTCPTab) {
      uint16 nPortBE = htons(nPort);
      for (DWORD e = pTCPTab->dwNumEntries; e-- > 0;) {
        // If there is a TCP entry in the table (regardless of its state), the
        // port is treated as unavailable.
...
uint16 CPreferences::GetRandomUDPPort() {
  ...
  do {
    ...
    // The port is assumed to be available by default. If we got a table of
    // currently used UDP ports, verify that the port is not in this table.
    bPortIsFree = true;
    if (pUDPTab) {
      uint16 nPortBE = htons(nPort);
...
// Default overridden by Registry value (see below)
// Fallback: ApplicationDir
CString CPreferences::GetDefaultDirectory(EDefaultDirectory eDirectory,
                                          bool bCreate) {
  if (m_astrDefaultDirs[0]
          .IsEmpty()) { // already have all directories fetched and stored?
    ...
    *pszFileName = _T('\0');
    ...
    CString strSelectedConfigBaseDirectory(
        m_astrDefaultDirs[EMULE_EXECUTABLEDIR]);
    CString strSelectedExpansionBaseDirectory(
        m_astrDefaultDirs[EMULE_EXECUTABLEDIR]);
    m_nCurrentUserDirMode = 2; // To let us know which "mode" we are using in
                               // case we want to switch per options

    // check if preferences.ini exists already in our default / fallback dir
    bool bConfigAvailableExecutable =
        (::PathFileExists(strSelectedConfigBaseDirectory + CONFIGFOLDER
                          _T("preferences.ini")) != FALSE);

    // check if our registry setting is present which forces the single or
    // multiuser directories and lets us ignore other defaults 0 = Multiuser, 1
    // = Publicuser, 2 = ExecutableDir. (on Winver < Vista 1 has the same effect
    // as 2)
    DWORD nRegistrySetting = _UI32_MAX;
    CRegKey rkEMuleRegKey;
    if (rkEMuleRegKey.Open(HKEY_CURRENT_USER, _T("Software\\eMule"),
                           KEY_READ) == ERROR_SUCCESS) {
      rkEMuleRegKey.QueryDWORDValue(_T("UsePublicUserDirectories"),
                                    nRegistrySetting);
      rkEMuleRegKey.Close();
    }
    ...
    // Do we need to get SystemFolders, or do we use our old Default anyway?
    // (Executable Dir)
    if (nRegistrySetting == 0 ||
      ...
      if (hShell32) {
        if (GetWindowsVersion() >= _WINVER_VISTA_) {
          ...
          HRESULT(WINAPI * pfnSHGetKnownFolderPath)(REFKNOWNFOLDERID, DWORD,
                                                    HANDLE, PWSTR *);
          (FARPROC &)pfnSHGetKnownFolderPath =
              GetProcAddress(hShell32, "SHGetKnownFolderPath");

          if (pfnSHGetKnownFolderPath != NULL &&
              (*pfnSHGetKnownFolderPath)(FOLDERID_LocalAppData, 0, NULL,
                                         &pszLocalAppData) == S_OK &&
              (*pfnSHGetKnownFolderPath)(FOLDERID_Downloads, 0, NULL,
                                         &pszPersonalDownloads) == S_OK &&
              (*pfnSHGetKnownFolderPath)(FOLDERID_PublicDownloads, 0, NULL,
                                         &pszPublicDownloads) == S_OK &&
              (*pfnSHGetKnownFolderPath)(FOLDERID_ProgramData, 0, NULL,
                                         &pszProgramData) == S_OK) {
            if (_tcsclen(pszLocalAppData) < MAX_PATH - 30 &&
              ...
              if (nRegistrySetting == _UI32_MAX) {
                // no registry default, check if we find a preferences.ini to
                // use
                if (::PathFileExists(strLocalAppData +
                                     _T("eMule\\") CONFIGFOLDER
                                     _T("preferences.ini")))
                  m_nCurrentUserDirMode = 0;
                else if (::PathFileExists(strProgramData +
                                          _T("eMule\\") CONFIGFOLDER
                                          _T("preferences.ini")))
                  ...
                  m_nCurrentUserDirMode =
                      0; // no preferences.ini found, use the default
              } else
                m_nCurrentUserDirMode = nRegistrySetting;

              switch (m_nCurrentUserDirMode) {
              case 0: // multiuser
                strSelectedDataBaseDirectory =
                    strPersonalDownloads + _T("eMule\\");
                strSelectedConfigBaseDirectory =
                    strLocalAppData + _T("eMule\\");
                strSelectedExpansionBaseDirectory =
                    strProgramData + _T("eMule\\");
                break;
              case 1: // public user
                strSelectedDataBaseDirectory =
                    strPublicDownloads + _T("eMule\\");
          ...
          if (!strAppData.IsEmpty() && !strPersonal.IsEmpty()) {
            if (strAppData.GetLength() < MAX_PATH - 30 &&
                strPersonal.GetLength() < MAX_PATH - 40) {
              slosh(strPersonal);
              slosh(strAppData);
              if (nRegistrySetting == 0) {
                // registry setting overwrites, use these folders
                strSelectedDataBaseDirectory =
                    strPersonal + _T("eMule Downloads\\");
                strSelectedConfigBaseDirectory = strAppData + _T("eMule\\");
                m_nCurrentUserDirMode = 0;
                // strSelectedExpansionBaseDirectory stays default
              } else if (nRegistrySetting == _UI32_MAX &&
                         !bConfigAvailableExecutable) {
                if (::PathFileExists(strAppData + _T("eMule\\") CONFIGFOLDER
                                                  _T("preferences.ini"))) {
        ...
        DebugLogError(_T("Unable to load shell32.dll to retrieve the system ")
                      _T("folder locations, using fallbacks"));
        ASSERT(0);
      }
    }

    // All the directories (categories also) should have a trailing backslash
    m_astrDefaultDirs[EMULE_CONFIGDIR] =
        strSelectedConfigBaseDirectory + CONFIGFOLDER;
    m_astrDefaultDirs[EMULE_TEMPDIR] =
        strSelectedDataBaseDirectory + _T("Temp\\");
    m_astrDefaultDirs[EMULE_INCOMINGDIR] =
        strSelectedDataBaseDirectory + _T("Incoming\\");
    m_astrDefaultDirs[EMULE_LOGDIR] =
        strSelectedConfigBaseDirectory + _T("logs\\");
    m_astrDefaultDirs[EMULE_ADDLANGDIR] =
        strSelectedExpansionBaseDirectory + _T("lang\\");
    m_astrDefaultDirs[EMULE_INSTLANGDIR] =
        m_astrDefaultDirs[EMULE_EXECUTABLEDIR] + _T("lang\\");
    m_astrDefaultDirs[EMULE_WEBSERVERDIR] =
        m_astrDefaultDirs[EMULE_EXECUTABLEDIR] + _T("webserver\\");
    m_astrDefaultDirs[EMULE_SKINDIR] =
        strSelectedExpansionBaseDirectory + _T("skins\\");
  ...
  if (bCreate && !m_abDefaultDirsCreated[eDirectory]) {
    switch (eDirectory) { // create the underlying directory first - be sure to
                          // adjust this if changing default directories
...
CString CPreferences::GetMuleDirectory(EDefaultDirectory eDirectory,
                                       bool bCreate) {
  switch (eDirectory) {
  case EMULE_INCOMINGDIR:
    return m_strIncomingDir;
  case EMULE_TEMPDIR:
    ASSERT(0); // This function can return only the first temp. directory. Use
               // GetTempDir() instead!
    return GetTempDir(0);
  case EMULE_SKINDIR:
...
void CPreferences::SetMuleDirectory(EDefaultDirectory eDirectory,
                                    const CString &strNewDir) {
  switch (eDirectory) {
  case EMULE_INCOMINGDIR:
...
void CPreferences::ChangeUserDirMode(int nNewMode) {
  ...
  // check if our registry setting is present which forces the single or
  // multiuser directories and lets us ignore other defaults 0 = Multiuser, 1 =
  // Public user, 2 = ExecutableDir.
  CRegKey rkEMuleRegKey;
  if (rkEMuleRegKey.Create(HKEY_CURRENT_USER, _T("Software\\eMule")) ==
      ERROR_SUCCESS) {
    if (rkEMuleRegKey.SetDWORDValue(_T("UsePublicUserDirectories"), nNewMode) !=
        ERROR_SUCCESS)
...
bool CPreferences::GetSparsePartFiles() {
  ...
  // If a sparse file exceeds a given limit of write I/O operations in a certain
  // order (or i.e. end to beginning) in its lifetime, it will at some point
  // throw out a FILE_SYSTEM_LIMITATION error and deny any writing to this file.
  // It was suggested that Vista might limit the data runs, which would lead to
  // such behaviour, but wouldn't make much sense for a sparse file
  // implementation nevertheless. Due to the fact that eMule writes a lot of
  // small blocks into sparse files and flushes them every 6 seconds, this
  // problem pops up sooner or later for all big files. I don't see any way to
  // walk around this for now Update: This problem seems to be fixed on Win7,
  // possibly on earlier Vista ServicePacks too
  //		   In any case, we allow sparse files for versions earlier and
  // later than Vista
  return m_bSparsePartFiles && (GetWindowsVersion() != _WINVER_VISTA_);
}

bool CPreferences::IsRunningAeroGlassTheme() {
  // This is important for all functions which need to draw in the NC-Area
  // (glass style) Aero by default does not allow this, any drawing will not be
  // visible. This can be turned off, but Vista will not deliver the Glass style
  // then as background when calling the default draw function. In other words,
  // draw all or nothing yourself - eMule chooses currently nothing
