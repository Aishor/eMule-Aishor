...
int eMuleAllocHook(int mode, void *pUserData, size_t nSize, int nBlockUse,
                   long lRequest, const unsigned char *pszFileName,
                   int nLine) noexcept;

// Cannot use a CString for that memory - it will be unavailable on application
// termination!
...
//
// VS2003:	DEP must be enabled dynamically because the linker does not
// support
//			the "/NXCOMPAT" command line option.
// VS2005:	DEP can get enabled at link time by using the "/NXCOMPAT"
// command
//			line option.
// VS2008:	DEP can get enabled at link time by using the "DEP" option
// within
...
void InitDEP() {
  BOOL(WINAPI * pfnGetProcessDEPPolicy)(HANDLE hProcess, LPDWORD lpFlags,
                                        PBOOL lpPermanent);
  BOOL(WINAPI * pfnSetProcessDEPPolicy)(DWORD dwFlags);
  (FARPROC &)pfnGetProcessDEPPolicy =
      GetProcAddress(GetModuleHandle(_T("kernel32")), "GetProcessDEPPolicy");
  (FARPROC &)pfnSetProcessDEPPolicy =
      GetProcAddress(GetModuleHandle(_T("kernel32")), "SetProcessDEPPolicy");
  if (pfnGetProcessDEPPolicy && pfnSetProcessDEPPolicy) {
    ...
    if ((*pfnGetProcessDEPPolicy)(GetCurrentProcess(), &dwFlags, &bPermanent)) {
      ...
      // ----------------------------------------------------
      // DEP flags: 00000003 (PROCESS_DEP_ENABLE |
      // *PROCESS_DEP_DISABLE_ATL_THUNK_EMULATION*) Permanent: *1*
      // ---
      // There is no way to remove the PROCESS_DEP_DISABLE_ATL_THUNK_EMULATION
      // flag at runtime, because the DEP policy is already permanent due to the
      // NXCOMPAT flag.
      //
      ...
      // PROCESS_DEP_DISABLE_ATL_THUNK_EMULATION) Permanent: *1*
      //
      // NOTE: It is ultimately important to explicitly enable the DEP policy
      // even if the process' DEP policy is already enabled. If the DEP policy
      // is already enabled due to an OptOut system policy, the DEP policy is
      // though not yet permanent. As long as the DEP policy is not permanent it
      // could get changed during runtime...
      //
      // So, if the DEP policy for the current process is already enabled but
      // not permanent, it has to be explicitly enabled by calling
      // 'SetProcessDEPPolicy' to make it permanent.
      //
      if (((dwFlags & PROCESS_DEP_ENABLE) == 0 || !bPermanent)
        ...
        // VS2003:	Enable DEP (with ATL-thunk emulation) if not already set
        // by system policy
...
void InitHeapCorruptionDetection() {
  BOOL(WINAPI * pfnHeapSetInformation)(
      HANDLE HeapHandle, HEAP_INFORMATION_CLASS HeapInformationClass,
      PVOID HeapInformation, SIZE_T HeapInformationLength);
  (FARPROC &)pfnHeapSetInformation =
      GetProcAddress(GetModuleHandle(_T("kernel32")), "HeapSetInformation");
...
CemuleApp::CemuleApp(LPCTSTR lpszAppName)
    : CWinApp(lpszAppName), emuledlg(), m_iDfltImageListColorFlags(ILC_COLOR),
      m_ullComCtrlVer(MAKEDLLVERULL(4, 0, 0, 0)),
      m_app_state(APP_STATE_STARTING), m_hSystemImageList(),
      m_sizSmallSystemIcon(16, 16), m_hBigSystemImageList(),
      m_sizBigSystemIcon(32, 32),
      m_strDefaultFontFaceName(_T("MS Shell Dlg 2")), m_dwPublicIP(),
      m_bGuardClipboardPrompt(), m_bAutoStart(), m_bStandbyOff() {
  ...
  // This does not seem to work well with multithreading, although there is no
  ...
  m_dwProductVersionMS =
      MAKELONG(CemuleApp::m_nVersionMin, CemuleApp::m_nVersionMjr);
  m_dwProductVersionLS =
      MAKELONG(CemuleApp::m_nVersionBld, CemuleApp::m_nVersionUpd);

  // create a string version (e.g. "0.30a")
  ASSERT(CemuleApp::m_nVersionUpd + 'a' <= 'f');
  m_strCurVersionLongDbg.Format(
      _T("%u.%u%c.%u"), CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin,
      _T('a') + CemuleApp::m_nVersionUpd, CemuleApp::m_nVersionBld);
#if defined(_DEBUG) || defined(_DEVBUILD)
  m_strCurVersionLong = m_strCurVersionLongDbg;
#else
  m_strCurVersionLong.Format(_T("%u.%u%c-Build%u-R1.2"),
                             CemuleApp::m_nVersionMjr, CemuleApp::m_nVersionMin,
                             _T('a') + CemuleApp::m_nVersionUpd,
                             CemuleApp::m_nVersionBld);
#endif
  ...
  strTmp.Format(_T("0x%lu%c"), m_dwProductVersionMS,
                _T('A') + CemuleApp::m_nVersionUpd);
...
BOOL CemuleApp::InitInstance() {
#ifdef _DEBUG
  // set Floating Point Processor to throw several exceptions, in particular the
  // 'Floating point divide by zero'
  UINT uEmCtrlWord = _control87(0, 0) & _MCW_EM;
  _control87(uEmCtrlWord & ~(/*_EM_INEXACT |*/ _EM_UNDERFLOW | _EM_OVERFLOW |
  ...
  // Installing that memory debug code works fine in Debug builds when running
  // within VS Debugger, but some other test applications don't like that
  // all....
  ...
  theCrashDumper.uCreateCrashDump =
      GetProfileInt(_T("eMule"), _T("CreateCrashDump"), 0);
  ...
  //
  _tsetlocale(LC_ALL, _T("")); // set all categories of locale to user-default
  ...
  if (!AfxInitRichEdit2() && !AfxInitRichEdit())
    AfxMessageBox(
        _T("Fatal Error: No Rich Edit control library found!")); // should never
                                                                 // happen.

  if (!Kademlia::CKademlia::InitUnicode(AfxGetInstanceHandle())) {
    AfxMessageBox(_T("Fatal Error: Failed to load Unicode character tables ")
                  _T("for Kademlia!")); // should never happen.
  ...
  if (thePrefs.IsRunAsUserEnabled()) {
    ...
    if (res == RES_FAILED)
      ...
      theApp.QueueLogLine(false, GetResString(IDS_RAU_FAILED),
                          (LPCTSTR)rau.GetCurrentUserW());
  }
...
#ifdef _DEBUG
  _sntprintf(s_szCrtDebugReportFilePath,
             _countof(s_szCrtDebugReportFilePath) - 1, _T("%s%s"),
             (LPCTSTR)thePrefs.GetMuleDirectory(EMULE_LOGDIR, false),
             APP_CRT_DEBUG_LOG_FILE);
#endif
  VERIFY(theLog.SetFilePath(
      thePrefs.GetMuleDirectory(EMULE_LOGDIR, thePrefs.GetLog2Disk()) +
      _T("eMule.log")));
  VERIFY(
      theVerboseLog.SetFilePath(thePrefs.GetMuleDirectory(EMULE_LOGDIR, false) +
                                _T("eMule_Verbose.log")));
  ...
  m_pFirewallOpener->Init(
      true); // we need to init it now (even if we may not use it yet) because
             // of CoInitializeSecurity - which kinda ruins the sense of the
             // class interface but ooohh well :P
  ...
  if (thePrefs.IsOpenPortsOnStartupEnabled()) {
    if (m_pFirewallOpener->DoesFWConnectionExist()) {
      ...
      if (m_pFirewallOpener->OpenPort(thePrefs.GetPort(), NAT_PROTOCOL_TCP,
                                      EMULE_DEFAULTRULENAME_TCP, true))
      ...
      if (thePrefs.GetUDPPort()) {
        // open port for this session
        if (m_pFirewallOpener->OpenPort(thePrefs.GetUDPPort(), NAT_PROTOCOL_UDP,
                                        EMULE_DEFAULTRULENAME_UDP, true))
          QueueLogLine(false, GetResString(IDS_FO_TEMPUDP_S),
                       thePrefs.GetUDPPort());
        else
          QueueLogLine(false, GetResString(IDS_FO_TEMPUDP_F),
                       thePrefs.GetUDPPort());
      }
    }
  }
  ...
  // Highres scheduling gives better resolution for Sleep(...) calls, and
  // timeGetTime() calls
  m_wTimerRes = 0;
      ...
      if (m_wTimerRes > 0) {
        ...
        if (thePrefs.GetVerbose()) {
          if (mmResult == TIMERR_NOERROR)
            theApp.QueueDebugLogLine(
                false,
                _T("Succeeded to set timer/scheduler resolution to %i ms."),
                m_wTimerRes);
          else {
            theApp.QueueDebugLogLine(
                false, _T("Failed to set timer/scheduler resolution to %i ms."),
                m_wTimerRes);
            m_wTimerRes = 0;
          }
        }
        ...
        theApp.QueueDebugLogLine(
            false,
            _T("m_wTimerRes == 0. Not setting timer/scheduler resolution."));
    }
  }
...
int CrtDebugReportCB(int reportType, char *message, int *returnValue) noexcept {
  ...
  *returnValue = 0; // avoid invocation of 'AfxDebugBreak' in ASSERT macros
  return TRUE; // avoid further processing of this debug report message by the
               // CRT
}
...
int eMuleAllocHook(int mode, void *pUserData, size_t nSize, int nBlockUse,
  ...
  return g_pfnPrevCrtAllocHook(mode, pUserData, nSize, nBlockUse, lRequest,
                               pszFileName, nLine);
}
#endif

bool CemuleApp::ProcessCommandline() {
  bool bIgnoreRunningInstances =
      (GetProfileInt(_T("eMule"), _T("IgnoreInstances"), 0) != 0);
...
#endif
      bIgnoreRunningInstances |=
          (_tcsicmp(pszParam, _T("ignoreinstances")) == 0);
  ...
  // If we create our TCP listen socket with SO_REUSEADDR, we have to ensure
  // that there are no 2 eMules are running on the same port. NOTE: This will
  // not prevent from some other application using that port!
  ...
  // this code part is to determine special cases when we do add a link to our
  // eMule because in this case it would be nonsense to start another instance!
  bool bAlreadyRunning = false;
  if (bIgnoreRunningInstances &&
      cmdInfo.m_nShellCommand == CCommandLineInfo::FileOpen &&
      (command.Find(_T("://")) > 0 || command.Find(_T("magnet:?")) >= 0 ||
       CCollection::HasCollectionExtention(command))) {
  ...
  if (cmdInfo.m_nShellCommand == CCommandLineInfo::FileOpen) {
    if (command.Find(_T("://")) > 0 || command.Find(_T("magnet:?")) >= 0) {
      sendstruct.cbData =
          static_cast<DWORD>((command.GetLength() + 1) * sizeof(TCHAR));
      sendstruct.dwData = OP_ED2KLINK;
      sendstruct.lpData = const_cast<LPTSTR>((LPCTSTR)command);
      if (maininst) {
        SendMessage(maininst, WM_COPYDATA, (WPARAM)0,
                    (LPARAM)(PCOPYDATASTRUCT)&sendstruct);
        return true;
      }
...
BOOL CALLBACK CemuleApp::SearchEmuleWindow(HWND hWnd, LPARAM lParam) noexcept {
  ...
  LRESULT res = ::SendMessageTimeout(hWnd, UWM_ARE_YOU_EMULE, 0, 0,
                                     SMTO_BLOCK | SMTO_ABORTIFHUNG, SEC2MS(10),
                                     &dwMsgResult);
...
CString CemuleApp::CreateKadSourceLink(const CAbstractFile *f) {
  CString strLink;
  if (Kademlia::CKademlia::IsConnected() && theApp.clientlist->GetBuddy() &&
      theApp.IsFirewalled()) {
    CString KadID;
    Kademlia::CKademlia::GetPrefs()
        ->GetKadID()
        .Xor(Kademlia::CUInt128(true))
        .ToHexString(KadID);
    strLink.Format(
        _T("ed2k://|file|%s|%I64u|%s|/|kadsources,%s:%s|/"),
        (LPCTSTR)EncodeUrlUtf8(StripInvalidFilenameChars(f->GetFileName())),
        (uint64)f->GetFileSize(), (LPCTSTR)EncodeBase16(f->GetFileHash(), 16),
...
bool CemuleApp::CopyTextToClipboard(const CString &strText) {
  ...
  HGLOBAL hGlobalT = ::GlobalAlloc(GHND | GMEM_SHARE,
  ...
  HGLOBAL hGlobalA = ::GlobalAlloc(GHND | GMEM_SHARE,
                                   (strTextA.GetLength() + 1) * sizeof(char));
  if (hGlobalA != NULL) {
  ...
  IgnoreClipboardLinks(strText); // this is so eMule won't think the clipboard
                                 // has ed2k links for adding
  return true;
}
...
void CemuleApp::OnlineSig() // Added By Bouc7
{
  ...
  const CString &strSigPath(thePrefs.GetMuleDirectory(EMULE_CONFIGBASEDIR) +
                            _szFileName);
  ...
  if (!file.Open(strSigPath,
                 CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite |
                     CFile::typeBinary,
                 &fex)) {
    LogError(LOG_STATUSBAR, _T("%s %s%s"),
             (LPCTSTR)GetResString(IDS_ERROR_SAVEFILE), _szFileName,
  ...
  try {
    ...
    snprintf(buffer, _countof(buffer), "%.1f",
             (float)downloadqueue->GetDatarate() / 1024);
    file.Write(buffer, (UINT)strlen(buffer));
    file.Write("|", 1);

    snprintf(buffer, _countof(buffer), "%.1f",
             (float)uploadqueue->GetDatarate() / 1024);
    file.Write(buffer, (UINT)strlen(buffer));
    file.Write("|", 1);
...
bool CemuleApp::ShowWebHelp(UINT uTopic) {
  CString strHelpURL;
  strHelpURL.Format(
      _T("https://onlinehelp.emule-project.net/help.php?language=%u&topic=%u"),
      thePrefs.GetLanguageID(), uTopic);
  BrowserOpen(strHelpURL, thePrefs.GetMuleDirectory(EMULE_EXECUTABLEDIR));
  return true;
}

int CemuleApp::GetFileTypeSystemImageIdx(LPCTSTR pszFilePath,
                                         int iLength /* = -1 */,
                                         bool bNormalsSize) {
  ...
  if (iLength > 0 && (pszFilePath[iLength - 1] == _T('\\') ||
                      pszFilePath[iLength - 1] == _T('/'))) {
    // it's a directory
    pszCacheExt = _T("\\");
  ...
  if (bNormalsSize) {
    if (!m_aBigExtToSysImgIdx.Lookup(pszCacheExt, vData)) {
      ...
      SHFILEINFO sfi;
      HIMAGELIST hResult = (HIMAGELIST)::SHGetFileInfo(
          pszFilePath, dwFileAttributes, &sfi, sizeof(sfi),
...
bool CemuleApp::IsConnected(bool bIgnoreEd2k, bool bIgnoreKad) {
  return (!bIgnoreEd2k && theApp.serverconnect->IsConnected()) ||
         (!bIgnoreKad && Kademlia::CKademlia::IsConnected());
}
...
uint32 CemuleApp::GetID() {
  ...
  return static_cast<uint32>(Kademlia::CKademlia::IsConnected() &&
                             Kademlia::CKademlia::IsFirewalled());
}

uint32 CemuleApp::GetED2KPublicIP() const { return m_dwPublicIP; }
...
void CemuleApp::SetPublicIP(const uint32 dwIP) {
  if (dwIP != 0) {
    ...
    if (GetPublicIP() == 0)
      AddDebugLogLine(DLP_VERYLOW, false, _T("My public IP Address is: %s"),
                      (LPCTSTR)ipstr(dwIP));
    else if (Kademlia::CKademlia::IsConnected() &&
             Kademlia::CKademlia::GetPrefs()->GetIPAddress())
      if (htonl(Kademlia::CKademlia::GetIPAddress()) != dwIP)
        AddDebugLogLine(
            DLP_DEFAULT, false,
            _T("Public IP Address reported by Kademlia (%s) differs from ")
            _T("new-found (%s)"),
            (LPCTSTR)ipstr(htonl(Kademlia::CKademlia::GetIPAddress())),
            (LPCTSTR)ipstr(dwIP));
  } else
...
bool CemuleApp::CanDoCallback(CUpDownClient *client) {
  ...
  if ((ed2k & eLow) != 0) {
    ...
    return (client->GetServerIP() != srv->GetIP() ||
            client->GetServerPort() != srv->GetPort());
  }
  return true;
}
...
HICON CemuleApp::LoadIcon(LPCTSTR lpszResourceName, int cx, int cy,
                          UINT uFlags) const {
  // Test using of 16 color icons. If 'LR_VGACOLOR' is specified _and_ the icon
  // resource contains a 16 color version, that 16 color version will be loaded.
  // If there is no 16 color version available, Windows will use the next
  // (better) color version found.
#ifdef _DEBUG
  ...
  if (!sSkinProfile.IsEmpty()) {
    ...
    GetPrivateProfileString(_T("Icons"), lpszResourceName, NULL, szSkinResource,
                            _countof(szSkinResource), sSkinProfile);
    if (szSkinResource[0] != _T('\0')) {
      // expand any optional available environment strings
      TCHAR szExpSkinRes[MAX_PATH];
      if (::ExpandEnvironmentStrings(szSkinResource, szExpSkinRes,
                                     _countof(szExpSkinRes)) != 0) {
      ...
      if (::PathIsRelative(szSkinResource)) {
        ...
        _tmakepathlimit(szFullResPath, NULL, szSkinResFolder, szSkinResource,
                        NULL);
      } else {
      ...
      // check for optional icon index or resource identifier within the icon
      // resource file
      ...
      if (iComma >= 0) {
        bExtractIcon |= (_stscanf(CPTR(strFullResPath, iComma + 1), _T("%d"),
                                  &iIconIndex) == 1);
      ...
      if (bExtractIcon) {
        if (uFlags != 0 || !(cx == cy && (cx == 16 || cx == 32))) {
          UINT uIconId;
          ::PrivateExtractIcons(strFullResPath, iIconIndex, cx, cy, &hIcon,
                                &uIconId, 1, uFlags);
        }

        if (hIcon == NULL) {
          HICON aIconsLarge[1], aIconsSmall[1];
          int iExtractedIcons = ExtractIconEx(strFullResPath, iIconIndex,
                                              aIconsLarge, aIconsSmall, 1);
          if (iExtractedIcons > 0) { // 'iExtractedIcons' is 2(!) if we get a
        ...
        // WINBUG???: 'ExtractIcon' does not work well on ICO-files when using
        // the color scheme 'Windows-Standard (extragro�)' -> always try to use
        // 'LoadImage'!
        //
        // If the ICO file contains a 16x16 icon, 'LoadImage' will though return
        // a 32x32 icon, if LR_DEFAULTSIZE is specified! -> always specify the
        // requested size!
        hIcon = (HICON)::LoadImage(NULL, szFullResPath, IMAGE_ICON, cx, cy,
                                   uFlags | LR_LOADFROMFILE);
        if (hIcon == NULL &&
            ::GetLastError() !=
                ERROR_PATH_NOT_FOUND /* && g_bGdiPlusInstalled*/) {
          // NOTE: Do *NOT* forget to specify /DELAYLOAD:gdiplus.dll as link
          // parameter.
  ...
  if (hIcon == NULL) {
    if (cx != LR_DEFAULTSIZE || cy != LR_DEFAULTSIZE ||
        uFlags != LR_DEFAULTCOLOR)
      hIcon = (HICON)::LoadImage(AfxGetResourceHandle(), lpszResourceName,
                                 IMAGE_ICON, cx, cy, uFlags);
    if (hIcon == NULL) {
      // TODO: Either do not use that function or copy the returned icon. All
      // the calling code is designed
      //  in a way that the icons returned by this function are to be freed with
      //  'DestroyIcon'. But an icon which was loaded with 'LoadIcon', is not be
      //  freed with 'DestroyIcon'. Right now, we never come here...
      ASSERT(0);
...
HBITMAP CemuleApp::LoadImage(LPCTSTR lpszResourceName,
                             LPCTSTR pszResourceType) const {
  const CString &sSkinProfile(thePrefs.GetSkinProfile());
  if (!sSkinProfile.IsEmpty()) {
    // load resource file specification from skin profile
    TCHAR szSkinResource[MAX_PATH];
    GetPrivateProfileString(_T("Bitmaps"), lpszResourceName, NULL,
                            szSkinResource, _countof(szSkinResource),
                            sSkinProfile);
    if (szSkinResource[0] != _T('\0')) {
      // expand any optional available environment strings
      TCHAR szExpSkinRes[MAX_PATH];
  ...
  CEnBitmap bmp;
  return bmp.LoadImage(lpszResourceName, pszResourceType)
             ? (HBITMAP)bmp.Detach()
             : NULL;
}

CString CemuleApp::GetSkinFileItem(LPCTSTR lpszResourceName,
                                   LPCTSTR pszResourceType) const {
  TCHAR szFullResPath[MAX_PATH];
  *szFullResPath = _T('\0');
  const CString &sSkinProfile(thePrefs.GetSkinProfile());
  if (!sSkinProfile.IsEmpty()) {
    // load resource file specification from skin profile
    TCHAR szSkinResource[MAX_PATH];
    GetPrivateProfileString(pszResourceType, lpszResourceName, NULL,
                            szSkinResource, _countof(szSkinResource),
                            sSkinProfile);
    if (szSkinResource[0] != _T('\0')) {
      // expand any optional available environment strings
...
bool CemuleApp::LoadSkinColor(LPCTSTR pszKey, COLORREF &crColor) const {
  ...
  if (!sSkinProfile.IsEmpty()) {
    ...
    GetPrivateProfileString(_T("Colors"), pszKey, NULL, szColor,
                            _countof(szColor), sSkinProfile);
    if (szColor[0] != _T('\0')) {
      int red, grn, blu;
...
bool CemuleApp::LoadSkinColorAlt(LPCTSTR pszKey, LPCTSTR pszAlternateKey,
                                 COLORREF &crColor) const {
  return LoadSkinColor(pszKey, crColor) ||
         LoadSkinColor(pszAlternateKey, crColor);
}
...
CTempIconLoader::CTempIconLoader(LPCTSTR pszResourceID, int cx, int cy,
                                 UINT uFlags) {
  m_hIcon = theApp.LoadIcon(pszResourceID, cx, cy, uFlags);
}

CTempIconLoader::CTempIconLoader(UINT uResourceID, int /*cx*/, int /*cy*/,
                                 UINT uFlags) {
...
void CemuleApp::AddEd2kLinksToDownload(const CString &strLinks, int cat) {
  for (int iPos = 0; iPos >= 0;) {
    ...
    try {
      ...
      LogError(LOG_STATUSBAR, GetResString(IDS_ERR_LINKERROR),
               (LPCTSTR)sBuffer);
      return;
    }
  }
...
void CemuleApp::SearchClipboard() {
  ...
  // Do not alter (trim) 'strLinks' and then copy back to
  // 'm_strLastClipboardContents'! The next clipboard content compare would fail
  // because of the modified string.
  LPCTSTR pszTrimmedLinks = strLinks;
  while (_istspace(*pszTrimmedLinks)) // Skip leading white space
    ++pszTrimmedLinks;
  m_bGuardClipboardPrompt =
      !_tcsnicmp(pszTrimmedLinks, _T("ed2k://|file|"), 13);
  if (m_bGuardClipboardPrompt) {
    // Don't feed too long strings into the MessageBox function, it may freak
    // out.
    CString strLinksDisplay(GetResString(IDS_ADDDOWNLOADSFROMCB));
    if (strLinks.GetLength() > 512)
      strLinksDisplay.AppendFormat(_T("\r\n%s..."),
                                   (LPCTSTR)strLinks.Left(509));
    else
      strLinksDisplay.AppendFormat(_T("\r\n%s"), (LPCTSTR)strLinks);
    if (AfxMessageBox(strLinksDisplay, MB_YESNO | MB_TOPMOST) == IDYES)
      AddEd2kLinksToDownload(pszTrimmedLinks, 0);
  }
  m_strLastClipboardContents =
      strLinks; // Save the unmodified(!) clipboard contents
...
bool CemuleApp::IsEd2kServerLinkInClipboard() {
  static const char _szEd2kServerLink[] =
      "ed2k://|server|"; // Use the ANSI string
  return IsEd2kLinkInClipboard(_szEd2kServerLink,
                               (sizeof _szEd2kServerLink) - 1);
}
...
void CemuleApp::CreateAllFonts() {
  ...
  // Creating that font with 'SYMBOL_CHARSET' should be safer (seen in ATL/MFC
  // code). Though it seems that it does not solve the problem with '6' and '9'
  // characters which are shown for some ppl.
  ...
  //
  // (*1) Do not use 'GetStockObject(DEFAULT_GUI_FONT)' to get the 'Tahoma'
  // font. It does not work...
  //
  // The documentation in MSDN states that DEFAULT_GUI_FONT returns 'Tahoma' on
  // Win2000/XP systems. Though this is wrong, it may be true for US-English
  // locales, but it is wrong for other locales. Furthermore it is even
  // documented that "MS Shell Dlg" gets mapped to "MS Sans Serif" on Windows XP
  // systems. Only "MS Shell Dlg 2" would get mapped to "Tahoma", but "MS Shell
  // Dlg 2" can not be used on prior Windows systems.
  //
  // The reason why "MS Shell Dlg" is though mapped to "Tahoma" when used within
  // dialog resources is unclear.
  //
  // So, to get the same font which is used within dialogs which were created
  // via dialog resources which have the "MS Shell Dlg, 8" specified (again, in
  // that special case "MS Shell Dlg" gets mapped to "Tahoma" and not to "MS
  // Sans Serif"), we just query the main window (which is also a dialog) for
  // the current font.
  //
  LOGFONT lfDefault;
  ...
  //
  // It would not be an error if that font name does not match our
  // pre-determined font name, I just want to know if that ever happens.
  ...
  // that font is nevertheless set to "MS Sans Serif" because a scaled up
  // "Tahoma" font unfortunately does not look as good as a scaled up "MS Sans
  // Serif" font.
  //
  // No! Do *not* use "MS Sans Serif" (never!). This will give a very old
  // fashioned font on certain Asian Windows systems. So, better use "MS Shell
  // Dlg" or "MS Shell Dlg 2" to let Windows map that font to the proper font on
  // all Windows systems.
  //
  LPLOGFONT plfHyperText = thePrefs.GetHyperTextLogFont();
  if (plfHyperText->lfFaceName[0] == _T('\0') ||
      !m_fontHyperText.CreateFontIndirect(plfHyperText))
  ...
  LPLOGFONT plfList = thePrefs.GetListLogFont();
  if (plfList->lfFaceName[0] != _T('\0'))
    m_fontList.CreateFontIndirect(plfList);
...
void CemuleApp::CreateBackwardDiagonalBrush() {
  static const WORD awBackwardDiagonalBrushPattern[8] = {
...
void CemuleApp::UpdateDesktopColorDepth() {
  ...
  if (!g_bLowColorDesktop)
    g_bLowColorDesktop =
        (GetProfileInt(_T("eMule"), _T("LowColorRes"), 0) != 0);
#endif

  if (g_bLowColorDesktop) {
    // If we have 4- or 8-bit desktop color depth, Windows will (by design) load
    // only the 16 color versions of icons. Thus we force all image lists also
    // to 4-bit format.
    ...
    if (m_iDfltImageListColorFlags == ILC_COLOR32 &&
        m_ullComCtrlVer < MAKEDLLVERULL(6, 0, 0, 0)) {
      // We fall back to 16-bit image lists because we do not provide 24-bit
      // versions of icons any longer (due to resource size restrictions for
      // Win98). We could also fall back to 24-bit image lists here but the
      // difference is minimal and considered not to be worth the additional
      // memory consumption.
      //
      // Though, do not fall back to 8-bit image lists because this would let
      // Windows reduce the color resolution to the standard 256 color window
      // system palette. We need a 16-bit or 24-bit image list to hold all our
      // 256 color icons (which are not pre-quantized to standard 256 color
      // windows system palette) without losing any colors.
...
BOOL WINAPI ConsoleCtrlHandler(DWORD dwCtrlType) noexcept {
  ...
  // will get forcefully terminated by Windows after 20 seconds.
  ...
  if (thePrefs.GetDebug2Disk()) {
    ...
    // Setting 'SHUTDOWN_NORETRY' does not prevent from getting terminated after
    // 20 sec.
    // DWORD dwLevel = 0, dwFlags = 0;
    // GetProcessShutdownParameters(&dwLevel, &dwFlags);
    // theVerboseLog.Logf(_T("%hs: ProcessShutdownParameters #0: Level=0x%08x,
    // Flags=0x%08x"), __FUNCTION__, dwLevel, dwFlags);
  ...
  if (dwCtrlType == CTRL_CLOSE_EVENT || dwCtrlType == CTRL_LOGOFF_EVENT ||
    ...
    if (theApp.emuledlg->m_hWnd) {
      if (thePrefs.GetDebug2Disk())
        theVerboseLog.Logf(
            _T("%hs: Sending TM_CONSOLETHREADEVENT to main window"),
            __FUNCTION__);

      // Use 'SendMessage' to send the message to the (different) main thread.
      // This is done by intention because it lets this thread wait as long as
      // the main thread has called 'ExitProcess' or returns from processing the
      // message. This is needed to not let Windows terminate the process before
      // the 20 sec. timeout.
      if (!theApp.emuledlg->SendMessage(TM_CONSOLETHREADEVENT, dwCtrlType,
                                        (LPARAM)GetCurrentThreadId())) {
        theApp.m_app_state = APP_STATE_SHUTTINGDOWN; // as a last attempt
        if (thePrefs.GetDebug2Disk())
          theVerboseLog.Logf(
              _T("%hs: Error: Failed to send TM_CONSOLETHREADEVENT to main ")
              _T("window - error %u"),
  ...
  // Returning FALSE does not cause Windows to immediately terminate the
  // process. Though, that only depends on the next registered console control
  // handler. The default seems to wait 20 sec. until the process has
  // terminated. After that timeout Windows nevertheless terminates the process.
  //
  // For whatever unknown reason, this is *not* always true!? It may happen that
  // Windows terminates the process *before* the 20 sec. timeout if (and only
  // if) the console control handler thread has already terminated. So, we have
  // to take care that we do not exit this thread before the main thread has
  // called 'ExitProcess' (in a synchronous way) -- see also the 'SendMessage'
  // above.
  if (thePrefs.GetDebug2Disk())
    theVerboseLog.Logf(_T("%hs: returning"), __FUNCTION__);
  return FALSE; // FALSE: Let the system kill the process with the default
                // handler.
}

void CemuleApp::UpdateLargeIconSize() {
  // initialize with system values in case we don't find the Shell's registry
  // key
  m_sizBigSystemIcon.cx = ::GetSystemMetrics(SM_CXICON);
  m_sizBigSystemIcon.cy = ::GetSystemMetrics(SM_CYICON);

  // get the Shell's registry key for the large icon size - the large icons
  // which are returned by the Shell are based on that size rather than on the
  // system icon size
  CRegKey key;
  if (key.Open(HKEY_CURRENT_USER, _T("Control Panel\\desktop\\WindowMetrics"),
               KEY_READ) == ERROR_SUCCESS) {
    TCHAR szShellLargeIconSize[12];
    ULONG ulChars = _countof(szShellLargeIconSize);
    if (key.QueryStringValue(_T("Shell Icon Size"), szShellLargeIconSize,
                             &ulChars) == ERROR_SUCCESS) {
      UINT uIconSize = 0;
      if (_stscanf(szShellLargeIconSize, _T("%u"), &uIconSize) == 1 &&
          uIconSize > 0) {
...
void CemuleApp::ResetStandByIdleTimer() {
  // Prevent system from falling asleep if connected or there are ongoing data
  // transfers (upload or download) Since Windows 11 there is no option to reset
  // the idle timer
  if (IsConnected() ||
...
bool CemuleApp::IsXPThemeActive() const {
  ...
  return theApp.m_ullComCtrlVer < MAKEDLLVERULL(6, 16, 0, 0) &&
         g_xpStyle.IsThemeActive() && g_xpStyle.IsAppThemed();
}
