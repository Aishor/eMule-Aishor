...
void CWebSocket::SetParent(CWebServer *pParent) { m_pParent = pParent; }

void CWebSocket::OnRequestReceived(const char *pHeader, DWORD dwHeaderLen,
                                   const char *pData, DWORD dwDataLen,
                                   const in_addr inad) {
  ...
  if (i >= 0)
    sURL.Truncate(i);
  bool filereq = sURL.GetLength() >= 3 &&
                 sURL.Find("..") <
                     0; // prevent file access in the eMule's webserver folder
  if (filereq) {
    CStringA ext(sURL.Right(5).MakeLower());
    i = ext.ReverseFind('.') + 1;
    ext.Delete(0, i);
    filereq = (i > 0) && ext.GetLength() > 1 &&
...
void CWebSocket::OnReceived(void *pData, DWORD dwSize, const in_addr inad) {
    ...
    for (DWORD dwPos = 0; dwPos < m_dwRecv; ++dwPos)
      if ('\n' == m_pBuf[dwPos]) {
        if (bPrevEndl) {
          ...
          // Now write the message's position into two first DWORDs of the
          // buffer
  ...
  if (m_dwHttpHeaderLen && m_dwHttpContentLen < m_dwRecv &&
      (!m_dwHttpContentLen ||
       (m_dwHttpHeaderLen + m_dwHttpContentLen <= m_dwRecv))) {
    OnRequestReceived(m_pBuf, m_dwHttpHeaderLen, m_pBuf + m_dwHttpHeaderLen,
                      m_dwHttpContentLen, inad);

    if (m_bCanRecv && (m_dwRecv > m_dwHttpHeaderLen + m_dwHttpContentLen)) {
      // move our data
      m_dwRecv -= m_dwHttpHeaderLen + m_dwHttpContentLen;
      memmove(m_pBuf, m_pBuf + m_dwHttpHeaderLen + m_dwHttpContentLen,
              m_dwRecv);
    } else
      m_dwRecv = 0;
  ...
  if (m_bValid && m_bCanSend) {
    if (!m_pHead) {
      if (thePrefs.GetWebUseHttps()) {
        for (;;) {
          int nRes =
              mbedtls_ssl_write(m_ssl, (unsigned char *)pData, dwDataSize);
          if (nRes > 0) {
          ...
          if (nRes == MBEDTLS_ERR_NET_CONN_RESET ||
              (nRes != MBEDTLS_ERR_SSL_WANT_READ &&
               nRes != MBEDTLS_ERR_SSL_WANT_WRITE)) {
            m_bValid = false;
            break;
          }
        }
...
void CWebSocket::SendContent(LPCSTR szStdResponse, const void *pContent,
                             DWORD dwContentSize) {
  CStringA sBuf;
  ...
  if (hEvent) {
    if (!WSAEventSelect(hSocket, hEvent, FD_READ | FD_CLOSE | FD_WRITE)) {
      ...
      if (thePrefs.GetWebUseHttps()) {
        ...
        if (ret)
          goto thread_exit;
        mbedtls_ssl_set_bio(&ssl, (void *)&hSocket, mbedtls_net_send,
                            mbedtls_net_recv, NULL);
        while ((ret = mbedtls_ssl_handshake(&ssl)) != 0)
          if (ret != MBEDTLS_ERR_SSL_WANT_READ &&
              ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            DebugLogWarning(_T("Web Interface handshake failed: %s"),
      ...
      while (WAIT_OBJECT_0 == ::WaitForMultipleObjects(DWORD(_countof(pWait)),
                                                       pWait, FALSE,
                                                       INFINITE)) {
          ...
          if (WSAEnumNetworkEvents(hSocket, NULL, &stEvents))
            ...
            if (FD_READ & stEvents.lNetworkEvents)
              for (;;) {
                ...
                int nRes;
                if (thePrefs.GetWebUseHttps())
                  nRes = mbedtls_ssl_read(stWebSocket.m_ssl,
                                          (unsigned char *)pBuf, sizeof pBuf);
                else
                  nRes = recv(hSocket, pBuf, sizeof pBuf, 0);
                if (nRes == MBEDTLS_ERR_SSL_WANT_READ ||
                    nRes == MBEDTLS_ERR_SSL_WANT_WRITE)
                  continue;
                if (nRes <= 0) {
                  if (!nRes) {
                  ...
                  if (thePrefs.GetWebUseHttps()) {
                    for (;;) {
                      int nRes = mbedtls_ssl_write(
                          stWebSocket.m_ssl,
                          (unsigned char *)stWebSocket.m_pHead->m_pToSend,
                          stWebSocket.m_pHead->m_dwSize);
                      if (nRes > 0) {
                    ...
                    int nRes = send(hSocket, stWebSocket.m_pHead->m_pToSend,
                                    stWebSocket.m_pHead->m_dwSize, 0);
                    if (nRes != (int)stWebSocket.m_pHead->m_dwSize) {
                      if (nRes)
                        if ((nRes > 0) &&
                            (nRes < (int)stWebSocket.m_pHead->m_dwSize)) {
                          stWebSocket.m_pHead->m_pToSend += nRes;
                          stWebSocket.m_pHead->m_dwSize -= nRes;
        ...
        if (!stWebSocket.m_bValid ||
            (!stWebSocket.m_bCanRecv && !stWebSocket.m_pHead))
          break;
      }
    thread_exit:
      stWebSocket.m_bValid = false;
...
UINT AFX_CDECL WebSocketListeningFunc(LPVOID pThis) {
  ...
  if (INVALID_SOCKET != hSocket) {
    ...
    stAddr.sin_addr.s_addr = thePrefs.GetBindAddrA()
                                 ? inet_addr(thePrefs.GetBindAddrA())
                                 : INADDR_ANY;
    ...
    if (!bind(hSocket, (LPSOCKADDR)&stAddr, sizeof stAddr) &&
      ...
      if (hEvent) {
        if (!WSAEventSelect(hSocket, hEvent, FD_ACCEPT)) {
          HANDLE pWait[] = {hEvent, s_hTerminate};
          while (WAIT_OBJECT_0 ==
                 ::WaitForMultipleObjects(2, pWait, FALSE, INFINITE)) {
            for (;;) {
              SOCKADDR_IN their_addr;
              int sin_size = (int)sizeof(SOCKADDR_IN);

              SOCKET hAccepted =
                  accept(hSocket, (LPSOCKADDR)&their_addr, &sin_size);
              ...
              if (!bAllowedIP) {
                for (INT_PTR i =
                         thePrefs.GetAllowedRemoteAccessIPs().GetCount();
                     --i >= 0;)
                  if (their_addr.sin_addr.s_addr ==
                      thePrefs.GetAllowedRemoteAccessIPs()[i]) {
                    bAllowedIP = true;
                    break;
                  }
                ...
                if (!bAllowedIP) {
                  LogWarning(
                      _T("Web Interface: Rejected connection attempt from %s"),
                      (LPCTSTR)ipstr(their_addr.sin_addr.s_addr));
              ...
              if (thePrefs.GetWSIsEnabled()) {
                ...
                // - do NOT use Windows API 'CreateThread' to create a thread
                // which uses MFC/CRT -> lot of mem leaks!
                // - 'AfxBeginThread' is excessive for our needs.
                CWinThread *pAcceptThread =
                    new CWinThread(WebSocketAcceptedFunc, (LPVOID)pData);
...
int StartSSL() {
  ...
  if (!ret) { // PSA_SUCCESS is 0
    ...
    if (!ret) {
      ret = mbedtls_x509_crt_parse_file(
          &srvcert, (const char *)CStringA(thePrefs.GetWebCertPath()));
      if (!ret) {
        ret = mbedtls_pk_parse_keyfile(
            &pkey, (const char *)CStringA(thePrefs.GetWebKeyPath()), NULL,
        ...
        if (!ret) {
          ...
          if (!ret) {
            ...
            if (!ret) {
              ...
              // mbedtls_ssl_ticket_write, mbedtls_ssl_ticket_parse,
              // &ticket_ctx); mbedtls_ssl_conf_new_session_tickets(&conf, 1);
              // mbedtls_ssl_conf_tls13_key_exchange_modes(&conf,
              // MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_ALL);
              // mbedtls_ssl_conf_legacy_renegotiation(&conf,
              // MBEDTLS_SSL_LEGACY_NO_RENEGOTIATION); //default
              // mbedtls_ssl_conf_renegotiation(&conf,
              // MBEDTLS_SSL_RENEGOTIATION_DISABLED); //default
  ...
  if (ret)
    ...
    DebugLog(_T("Loaded certificate: %s"),
             (LPCTSTR)GetCertHash(fingerprint, (int)(sizeof fingerprint)));
  }
  return ret;
}
...
void StartSockets(CWebServer *pThis) {
  ...
  if (s_hTerminate != NULL) {
    // - do NOT use Windows API 'CreateThread' to create a thread which uses
    // MFC/CRT -> lot of mem leaks!
    // - because we want to wait on the thread handle,
    //   we have to disable 'CWinThread::m_AutoDelete' -> can't use
    //   'AfxBeginThread'
...
void StopSockets() {
  ...
  if (s_pSocketThread) {
    ...
    if (s_pSocketThread->m_hThread) {
      // because we want to wait on the thread handle we must not use
      // 'CWinThread::m_AutoDelete'. otherwise we may run into the situation
      // that the CWinThread was already auto-deleted and the
      // CWinThread::m_hThread is invalid.
      ...
      if (dwWaitRes == WAIT_TIMEOUT) {
        TRACE(
            "*** Failed to wait for websocket thread termination - Timeout\n");
        ...
        TRACE(
            "*** Failed to wait for websocket thread termination - Error %d\n",
