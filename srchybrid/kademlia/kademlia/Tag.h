...
There is going to be a new forum created just for the Kademlia side of the
client. If you feel there is an error or a way to improve something, please post
it in the forum first and let us look at it. If it is a real improvement, it
will be added to the official client. Changing something without knowing what
all it does, can cause great harm to the network if released in mass form. Any
mod that changes anything within the Kademlia side will not be allowed to
advertise their client on the eMule forum.
*/
...
public:
  ...
  explicit CKadTagNameString(LPCSTR psz) : CStringA(psz) {}

  CKadTagNameString(LPCSTR psz, int len) : CStringA(psz, len) {}

  // A tag name may include character values >= 0xD0 and also >= 0xF0. to
  // prevent those characters to be interpreted as multi-byte character
  // sequences we have to ensure that a binary string compare is performed.
  int Compare(LPCSTR psz) const noexcept {
    ATLASSERT(AfxIsValidString(psz));
    // Do a binary string compare. (independent from any codepage and/or
    // LC_CTYPE setting.)
  ...
  friend bool operator==(const CKadTagNameString &str1,
                         LPCSTR const psz2) noexcept {
  ...
  operator PCXSTR() const noexcept { return CStringA::operator PCXSTR(); }

  XCHAR operator[](int iChar) const noexcept {
    return CStringA::operator[](iChar);
  }

  PXSTR GetBuffer() { return CStringA::GetBuffer(); }

  PXSTR GetBuffer(int nMinBufferLength) {
    return CStringA::GetBuffer(nMinBufferLength);
  }

  int GetLength() const noexcept { return CStringA::GetLength(); }
};

class CKadTagValueString : public CStringW {
public:
  CKadTagValueString() = default;
  explicit CKadTagValueString(const CStringW &rstr) : CStringW(rstr) {}

  explicit CKadTagValueString(const wchar_t *psz) : CStringW(psz) {}

  CKadTagValueString(const wchar_t *psz, int iLen) : CStringW(psz, iLen) {}
...
public:
  ...
  CKadTag(byte type, LPCSTR name) : m_name(name), m_type(type) {}
  ...
  bool IsBool() const { return m_type == TAGTYPE_BOOL; }
...
public:
  CKadTagStr(LPCSTR name, LPCWSTR value, int len)
      : CKadTag(TAGTYPE_STRING, name), m_value(value, len) {}

  CKadTagStr(LPCSTR name, LPCWSTR value)
      : CKadTag(TAGTYPE_STRING, name), m_value(value) {}

  CKadTagStr(LPCSTR name, const CStringW &rstr)
      : CKadTag(TAGTYPE_STRING, name), m_value(rstr) {}

  virtual CKadTagStr *Copy() const { return new CKadTagStr(m_name, m_value); }
...
public:
  CKadTagUInt(LPCSTR name, uint64 value)
      : CKadTag(TAGTYPE_UINT, name), m_value(value) {}

  virtual CKadTagUInt *Copy() const { return new CKadTagUInt(m_name, m_value); }

  virtual uint64 GetInt() const { return m_value; }
...
public:
  CKadTagUInt64(LPCSTR name, uint64 value)
      : CKadTag(TAGTYPE_UINT64, name), m_value(value) {}
...
public:
  CKadTagUInt32(LPCSTR name, uint32 value)
      : CKadTag(TAGTYPE_UINT32, name), m_value(value) {}
...
public:
  CKadTagFloat(LPCSTR name, float value)
      : CKadTag(TAGTYPE_FLOAT32, name), m_value(value) {}
...
public:
  CKadTagBool(LPCSTR name, bool value)
      : CKadTag(TAGTYPE_BOOL, name), m_value(value) {}

  virtual CKadTagBool *Copy() const { return new CKadTagBool(m_name, m_value); }
...
public:
  CKadTagUInt16(LPCSTR name, uint16 value)
      : CKadTag(TAGTYPE_UINT16, name), m_value(value) {}
...
public:
  CKadTagUInt8(LPCSTR name, uint8 value)
      : CKadTag(TAGTYPE_UINT8, name), m_value(value) {}
...
public:
  CKadTagHash(LPCSTR name, const BYTE *value) : CKadTag(TAGTYPE_HASH, name) {
    m_value = new BYTE[MDX_DIGEST_SIZE];
    md4cpy(m_value, value);
  }

  ~CKadTagHash() { delete[] m_value; }

  CKadTagHash(const CKadTagHash &) = delete;
  CKadTagHash &operator=(const CKadTagHash &) = delete;

  virtual CKadTagHash *Copy() const { return new CKadTagHash(m_name, m_value); }
