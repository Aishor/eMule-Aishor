...
// Be careful with these offsets, they are supposed to match *exactly* the
// Windows built-in metric. If it does not match that value, column auto-sizing
// (double clicking on header divider) will give inaccurate results.
const int CMuleListCtrl::sm_iIconOffset =
    4; // Offset from left window border to icon (of 1st column)
const int CMuleListCtrl::sm_iLabelOffset =
    2; // Offset between right icon border and item text (of 1st column)
const int CMuleListCtrl::sm_iSubItemInset =
    4; // Offset from left and right column border to item text
...
CMuleListCtrl::CMuleListCtrl(PFNLVCOMPARE pfnCompare, LPARAM iParamSort)
    : m_SortProc(pfnCompare), m_dwParamSort(iParamSort), m_crWindow(),
      m_crWindowText(), m_crWindowTextBk(m_crWindow), m_crHighlight(),
      m_crHighlightText(m_crWindowText), m_crGlow(), m_crFocusLine(),
      m_crNoHighlight(), m_crNoFocusLine(), m_lvcd(), m_bCustomDraw(),
      m_uIDAccel(IDR_LISTVIEW), m_hAccel(), m_eUpdateMode(lazy),
      m_iAutoSizeWidth(LVSCW_AUTOSIZE), m_iFindDirection(1), m_iFindColumn(),
      m_bGeneralPurposeFind(), m_bCanSearchInAllColumns(), m_bFindMatchCase(),
      m_aColumns(), m_iColumnsTracked(), m_iCurrentSortItem(-1),
      m_atSortArrow(), m_iRedrawCount() {}

int CALLBACK CMuleListCtrl::SortProc(LPARAM /*lParam1*/, LPARAM /*lParam2*/,
                                     LPARAM /*lParamSort*/) {
  return 0;
}
...
void CMuleListCtrl::PreSubclassWindow() {
  ...
  // If we want to handle the VK_RETURN key, we have to do that via
  // accelerators!
  if (m_uIDAccel != UINT_MAX) {
    m_hAccel =
        ::LoadAccelerators(AfxGetResourceHandle(), MAKEINTRESOURCE(m_uIDAccel));
    ASSERT(m_hAccel);
  }

  // DEFAULT_GUI_FONT: Vista: "MS Shell Dlg" with 8 pts (regardless of system
  // applet settings !!!) SYSTEM_FONT:		 Vista: Good old Windows 3.11
  // System Font NULL				 Vista: Font ('Symbol') with the
  // face and size which is configured in System applet.
  if (thePrefs.GetUseSystemFontForMainControls())
    SendMessage(WM_SETFONT, NULL, FALSE);
  else if (theApp.m_fontList.m_hObject)
    SetFont(&theApp.m_fontList);
  else {
    // Intentar crear la fuente si no existe aún (por ejemplo, primer arranque
    // con configuración cargada)
    if (theApp.m_fontList.CreateFontIndirect(thePrefs.GetListLogFont()))
      SetFont(&theApp.m_fontList);
  }
}
...
void CMuleListCtrl::HideColumn(int iColumn) {
  ...
  for (int i = 0; i < m_iColumnsTracked; ++i)
    iFrom += static_cast<int>(m_aColumns[i].iLocation >
                                  m_aColumns[iColumn].iLocation &&
                              m_aColumns[i].bHidden);
...
void CMuleListCtrl::ShowColumn(int iColumn) {
  ...
  for (; iCurrent < m_iColumnsTracked - 1 &&
         iCurrent < IndexToOrder(pHeaderCtrl, 0);
       ++iCurrent)
    piArray[iCurrent] = piArray[iCurrent + 1];
  for (; iCurrent < m_iColumnsTracked - 1 &&
         m_aColumns[iColumn].iLocation >
             m_aColumns[pHeaderCtrl->OrderToIndex(iCurrent + 1)].iLocation;
       ++iCurrent) {
    piArray[iCurrent] = piArray[iCurrent + 1];
  }
...
void CMuleListCtrl::SaveSettings() {
  ...
  ini.SerGet(false, piColWidths, m_iColumnsTracked,
             m_Name + _T("ColumnWidths"));
  ini.SerGet(false, piColHidden, m_iColumnsTracked,
             m_Name + _T("ColumnHidden"));
  ini.SerGet(false, piColOrders, m_iColumnsTracked,
             m_Name + _T("ColumnOrders"));
...
CMuleListCtrl::ArrowType CMuleListCtrl::GetArrowType(int iat) {
  static const CMuleListCtrl::ArrowType art[] = {
      arrowDown, arrowUp, arrowDoubleDown, arrowDoubleUp};
...
void CMuleListCtrl::LoadSettings() {
  ...
  m_atSortArrow =
      GetArrowType(ini.GetInt(m_Name + _T("TableSortAscending"), 1));
  ...
  for (int i = 0; i < m_iColumnsTracked; ++i) {
    ...
    if (piColWidths[i] >= 2) // don't allow column widths of 0 and 1 -- just
                             // because it looks very confusing in GUI
  ...
  for (int i = 1; i < m_iColumnsTracked; ++i)
    if (piColHidden[i] > 0 ||
        (piColHidden[i] < 0 && m_liDefaultHiddenColumns.Find(i) != NULL))
      HideColumn(i);
...
void CMuleListCtrl::SetColors() {
  ...
  if (!sSkinProfile.IsEmpty()) {
    ...
    GetPrivateProfileString(_T("Colors"), strKey + _T("BkImg"), NULL, szColor,
                            _countof(szColor), sSkinProfile);
    if (*szColor == _T('\0'))
      GetPrivateProfileString(_T("Colors"), _T("DefLvBkImg"), NULL, szColor,
                              _countof(szColor), sSkinProfile);
    if (*szColor != _T('\0'))
      strBkImage = szColor;
  }
  ...
  // Must explicitly set a NULL watermark bitmap, to clear any already set
  // watermark bitmap.
  LVBKIMAGE lvimg = {};
  ...
  if (!strBkImage.IsEmpty() && !g_bLowColorDesktop) {
    ...
    if (::ExpandEnvironmentStrings(strBkImage, szExpSkinRes,
...
void CMuleListCtrl::SetSortArrow(int iColumn, ArrowType atType) {
  ...
  if (iColumn >= 0 && pHeaderCtrl->GetItem(iColumn, &headerItem)) {
    ...
    HINSTANCE hInstRes =
        AfxFindResourceHandle(MAKEINTRESOURCE(m_atSortArrow), RT_BITMAP);
    if (hInstRes != NULL) {
      HBITMAP hbmSortStates = (HBITMAP)::LoadImage(
          hInstRes, MAKEINTRESOURCE(m_atSortArrow), IMAGE_BITMAP, 0, 0,
      ...
      if (hbmSortStates != NULL) {
        ...
        if (imlSortStates.Create(
                14, 14, theApp.m_iDfltImageListColorFlags | ILC_MASK, 1, 0)) {
          VERIFY(imlSortStates.Add(&bmSortStates, RGB(255, 0, 255)) >= 0);

          // To avoid drawing problems (which occur only with an image list
          // *with* a mask) while resizing list view columns which have the
          // header control bitmap right aligned, set the background color of
          // the image list.
          if (theApp.m_ullComCtrlVer < MAKEDLLVERULL(6, 0, 0, 0))
            imlSortStates.SetBkColor(::GetSysColor(COLOR_BTNFACE));

          // When setting the image list for the header control for the first
          // time we'll get the image list of the listview control!! So, better
          // store the header control image list separate.
          (void)pHeaderCtrl->SetImageList(&imlSortStates);
          m_imlHeaderCtrl.DeleteImageList();
          m_imlHeaderCtrl.Attach(imlSortStates.Detach());

          // Use smaller bitmap margins -- this saves some pixels which may be
          // required for rather small column titles.
          if (theApp.m_ullComCtrlVer >= MAKEDLLVERULL(5, 8, 0, 0)) {
            int iBmpMargin = pHeaderCtrl->GetBitmapMargin();
            int iNewBmpMargin = ::GetSystemMetrics(SM_CXEDGE) +
...
int CMuleListCtrl::UpdateLocation(int iItem) {
  ...
  if (notFirst) {
    ...
    int iResult =
        m_SortProc(dwpItemData, GetParamAt(pos, iNewIndex), m_dwParamSort);
    if (iResult < 0) {
      ...
      while (iDist > 1) {
        ...
        if (m_SortProc(dwpItemData, GetParamAt(posPrev, iNewIndex - iDist),
                       m_dwParamSort) < 0) {
      ...
      while (--iNewIndex >= 0) {
        ...
        if (m_SortProc(dwpItemData, GetParamAt(pos, iNewIndex),
                       m_dwParamSort) >= 0)
          break;
      }
  ...
  if (notLast) {
    ...
    if (iResult > 0) {
      ...
      while (iDist > 1) {
        ...
        if (m_SortProc(dwpItemData, GetParamAt(posNext, iNewIndex + iDist),
                       m_dwParamSort) > 0) {
          iNewIndex = iNewIndex + iDist;
          pos = posNext;
        } else
...
BOOL CMuleListCtrl::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam,
                             LRESULT *pResult) {
  ...
  case WM_NOTIFY:
    ...
    case NM_RCLICK: // handle right click on headers and show column menu
    {
      ...
      for (int iCurrent = 1; iCurrent < iCount; ++iCurrent) {
        ...
        tmColumnMenu.AppendMenu(
            MF_STRING | (m_aColumns[iCurrent].bHidden ? 0 : MF_CHECKED),
            MLC_IDC_MENU + iCurrent, item.pszText);
      }
      tmColumnMenu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, point.x,
                                  point.y, this);
      VERIFY(tmColumnMenu.DestroyMenu());
    }
      return (BOOL)(*pResult = TRUE);
    case HDN_BEGINTRACKA: // forbid changing the size of anything "before" the
                          // first column
        ...
        if (iNewLoc > 0) {
          ...
          if (iOldLoc != iNewLoc) {
            if (iOldLoc > iNewLoc) {
              ...
              int iMin = iNewLoc;
              for (int i = 0; i < m_iColumnsTracked; ++i) {
                if (m_aColumns[i].iLocation >= iMin &&
                    m_aColumns[i].iLocation < iMax)
    ...
    case HDN_DIVIDERDBLCLICKW:
      ...
      //	If the listview control can query for all the items in a column,
      // it is 	capable of computing the minimal width needed to display the
      // item with 	the largest width. However, if the width of the header
      // label is larger 	then the largest width of the items in a column,
      // the width of the header label 	will overrule the width which would be
      // needed for the items in the column. 	In practice this means, that the
      // column could get larger than really needed 	for the items in the
      // column (just because the width gets adjusted for 	showing the
      // header label also). 	This is a good solution for some of our
      // listviews which do not (yet) provide 	the according functions which
      // would give the listview control the chance to 	query for all items in a
      //column. This flag will thus lead to sizing the 	column at least to the
      // width of the header label. That's at least better 	than resizing
      // the column to zero width (which would be the alternative).
      //
      // Though, a few of our listviews are already capable of providing all the
      // information which is needed by the listview control to properly auto
      // size a column. Those listviews can set the 'm_iAutoSizeWidth' to
      // 'LVSCW_AUTOSIZE' which will lead to standard Windows behaviour.
      //
      if (GetStyle() & LVS_OWNERDRAWFIXED) {
        LPNMHEADER pHeader = reinterpret_cast<LPNMHEADER>(lParam);
        // If the listview is empty, the LVSCW_AUTOSIZE_USEHEADER is more
        // appropriate, even if some listview has requested LVSCW_AUTOSIZE.
        SetColumnWidth(pHeader->iItem, GetItemCount() == 0
  ...
  case LVM_SETITEM: {
    POSITION pos =
        m_Params.FindIndex(reinterpret_cast<LPLVITEM>(lParam)->iItem);
    if (pos) {
  ...
  case LVM_SORTITEMS:
    ...
    for (POSITION pos = m_Params.GetHeadPosition(); pos != NULL;
         m_Params.GetNext(pos))
      m_Params.SetAt(pos, MLC_MAGIC);
    break;
  case LVM_DELETEALLITEMS:
    if (!CListCtrl::OnWndMsg(message, wParam, lParam, pResult) &&
        DefWindowProc(message, wParam, lParam))
      m_Params.RemoveAll();
    return (BOOL)(*pResult = TRUE);
  case LVM_DELETEITEM:
    MLC_ASSERT(m_Params.GetAt(m_Params.FindIndex(wParam)) ==
               CListCtrl::GetItemData(wParam));
    if (!CListCtrl::OnWndMsg(message, wParam, lParam, pResult) &&
        DefWindowProc(message, wParam, lParam))
  ...
  case LVM_INSERTITEMW:
    // try to fix position of inserted items
    {
      ...
      if (notFirst) {
        int iNewIndex = iItem - 1;
        POSITION pos = m_Params.FindIndex(iNewIndex);
        int iResult = m_SortProc(pItem->lParam, GetParamAt(pos, iNewIndex),
                                 m_dwParamSort);
        if (iResult < 0) {
          POSITION posPrev = pos;
          for (int iDist = iNewIndex / 2; iDist > 1;) {
            ...
            if (m_SortProc(pItem->lParam,
                           GetParamAt(posPrev, iNewIndex - iDist),
                           m_dwParamSort) < 0) {
              iNewIndex = iNewIndex - iDist;
              pos = posPrev;
            } else
          ...
          while (--iNewIndex >= 0) {
            m_Params.GetPrev(pos);
            if (m_SortProc(pItem->lParam, GetParamAt(pos, iNewIndex),
                           m_dwParamSort) >= 0)
              break;
          }
      ...
      if (notLast) {
        ...
        if (iResult > 0) {
          ...
          while (iDist > 1) {
            ...
            if (m_SortProc(pItem->lParam,
                           GetParamAt(posNext, iNewIndex + iDist),
                           m_dwParamSort) > 0) {
              iNewIndex = iNewIndex + iDist;
              pos = posNext;
            } else
  ...
  case WM_DESTROY:
    ...
    break;
  case LVM_UPDATE:
    // better fix for old problem... normally Update(int) causes entire list to
    // redraw
    if ((int)wParam ==
        UpdateLocation(
            (int)wParam)) { // no need to invalidate rect if item moved
      RECT rcItem;
    ...
    return (BOOL)(*pResult = TRUE);
  case WM_CONTEXTMENU:
    // If the context menu is opened with the _mouse_ and if it was opened
    // _outside_ the client area of the list view, let Windows handle that
    // message. Otherwise we would prevent the context menu for e.g. scrollbars
    // to be invoked.
...
BOOL CMuleListCtrl::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam,
                                  LRESULT *pResult) {
  if (message != WM_DRAWITEM) {
    // catch the prepaint and copy struct
    if (message == WM_NOTIFY &&
        reinterpret_cast<LPNMHDR>(lParam)->code == NM_CUSTOMDRAW &&
        reinterpret_cast<LPNMLVCUSTOMDRAW>(lParam)->nmcd.dwDrawStage ==
            CDDS_ITEMPREPAINT) {
      m_bCustomDraw =
          CListCtrl::OnChildNotify(message, wParam, lParam, pResult);
      if (m_bCustomDraw)
...
// comctl32.dll's "smooth scroll" consistently leaves garbage below the bottom
// item Until a better way was found, clear the mess on EndScroll.
...
void CMuleListCtrl::InitItemMemDC(CMemoryDC *dc,
...
void CMuleListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  if (m_bCustomDraw)
    clr =
        (bHighlight && g_bLowColorDesktop) ? m_crHighlightText : m_lvcd.clrText;
  else
  ...
  // labels are offset by a certain amount related to the width of a space
  // character
  ...
  DrawFocusRect(pDC, &rcBounds, lvi.state & LVIS_FOCUSED, bCtrlFocused,
...
void CMuleListCtrl::DrawFocusRect(CDC *pDC, LPCRECT rcItem, BOOL bItemFocused,
                                  BOOL bCtrlFocused, BOOL bItemSelected) {
  // draw focus rectangle if the item has focus
  if (bItemFocused && (bCtrlFocused || bItemSelected)) {
    CBrush brush(bCtrlFocused && bItemSelected ? m_crFocusLine
                                               : m_crNoFocusLine);
...
BOOL CMuleListCtrl::OnEraseBkgnd(CDC *pDC) {
  // if (m_crWindowTextBk == CLR_NONE) // this creates a lot of screen
  // flickering 	return CListCtrl::OnEraseBkgnd(pDC);
...
void CMuleListCtrl::OnSysColorChange() {
  ...
  if (thePrefs.GetUseSystemFontForMainControls()) {
    ...
    //
    // Though it does not work as expected. Although we get the WM_MEASUREITEM
    // and although we return the correct (new) item height, the listview
    // control does not redraw the items with the new height until the control
    // really gets resized.
    CRect rc;
    GetWindowRect(rc);
    WINDOWPOS wp;
...
HIMAGELIST CMuleListCtrl::ApplyImageList(HIMAGELIST himl) {
  HIMAGELIST himlOld =
      (HIMAGELIST)SendMessage(LVM_SETIMAGELIST, LVSIL_SMALL, (LPARAM)himl);
  if (m_imlHeaderCtrl.m_hImageList != NULL) {
    // Must *again* set the image list for the header control, because
    // LVM_SETIMAGELIST always resets any already specified header control image
    // lists!
...
void CMuleListCtrl::DoFind(int iStartItem, int iDirection /*1 = down, -1 = up*/,
                           BOOL bShowError) {
  if (iStartItem < 0) {
    MessageBeep(MB_OK);
    return;
  ...
  for (int iItem = iStartItem;
       ((iDirection > 0) ? iItem < iNumItems : iItem >= 0);) {
    const CString &strItemText(GetItemText(iItem, m_iFindColumn));
    if (!strItemText.IsEmpty()) {
      if ((m_bFindMatchCase ? _tcsstr(strItemText, m_strFindText)
...
BOOL CMuleListCtrl::PreTranslateMessage(MSG *pMsg) {
  ...
  if (m_hAccel != NULL && pMsg->message >= WM_KEYFIRST &&
  ...
  // Catch the "Ctrl+<NumPad_Plus_Key>" shortcut. CMuleListCtrl can not handle
  // this.
...
CMuleListCtrl::EUpdateMode
CMuleListCtrl::SetUpdateMode(EUpdateMode eUpdateMode) {
...
void CMuleListCtrl::OnLvnGetInfoTip(LPNMHDR pNMHDR, LRESULT *pResult) {
  // NOTE: Using 'Info Tips' for owner drawn list view controls (like almost all
  // instances of the CMuleListCtrl) gives potentially *wrong* results. One may
  // and will experience several situations where a tooltip should be shown and
  // none will be shown. This is because the Windows list view control code does
  // not know anything about what the owner drawn list view control was actually
  // drawing. So, the Windows list view control code is just *assuming* that the
  // owner drawn list view control instance is using the same drawing metrics as
  // the Windows control. Because our owner drawn list view controls almost
  // always draw an additional icon before the actual item text and because the
  // Windows control does not know that, the calculations performed by the
  // Windows control regarding folded/unfolded items are in couple of cases
  // wrong. E.g. because the Windows control does not know about the additional
  // icon and thus about the reduced space used for drawing the item text, we
  // may show folded item texts while the Windows control is still assuming that
  // we show the full text -> thus we will not receive a precomputed info tip
  // which contains the unfolded item text. Result: We would have to implement
  // our own info tip processing.
  LPNMLVGETINFOTIP pGetInfoTip = reinterpret_cast<LPNMLVGETINFOTIP>(pNMHDR);
  LVHITTESTINFO hti;
  if (pGetInfoTip->iSubItem == 0 && ::GetCursorPos(&hti.pt)) {
    ScreenToClient(&hti.pt);
    if (SubItemHitTest(&hti) < 0 || hti.iItem != pGetInfoTip->iItem ||
        hti.iSubItem != 0) {
      // Don't show the default label tip for the main item, if the mouse is not
      // over the main item.
      if ((pGetInfoTip->dwFlags & LVGIT_UNFOLDED) == 0 &&
          pGetInfoTip->cchTextMax > 0 && pGetInfoTip->pszText[0] != _T('\0')) {
        // For any reason this does not work with Win98 (COMCTL32 v5.8). Even
        // when the info tip text is explicitly set to empty, the list view
        // control may display the unfolded text for the 1st item. It works for
        // WinXP though.
...
int CMuleListCtrl::InsertColumn(int nCol, LPCTSTR lpszColumnHeading,
