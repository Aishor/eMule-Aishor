...
static const byte base16Lookup[BASE16_LOOKUP_MAX][2] = {
    ...
    {'6', 0x6}, {'7', 0x7}, {'8', 0x8}, {'9', 0x9}, {':', 0x9}, {';', 0x9},
    {'<', 0x9}, {'=', 0x9}, {'>', 0x9}, {'?', 0x9}, {'@', 0x9}, {'A', 0xA},
    {'B', 0xB}, {'C', 0xC}, {'D', 0xD}, {'E', 0xE}, {'F', 0xF}};
LPCTSTR const sBadFileNameChar(
    _T("\"*<>?|\\/:")); // lots of invalid chars for filenames in Windows :=)
LPCTSTR const sHiddenPassword =
    _T("\012\010\011\015\033"); // tough to enter from the keyboard
const struct LinkScheme s_apszSchemes[] = {
    {_T("ed2k://"), 7}, {_T("http://"), 7},  {_T("https://"), 8},
    {_T("ftp://"), 6},  {_T("www."), 4},     {_T("ftp."), 4},
    {_T("mailto:"), 7}, {_T("magnet:?"), 8}, {NULL, 0}};
...
CString CastItoXBytes(EMFileSize count, bool isK, bool isPerSec,
                      uint32 decimal) {
  return CastItoXBytes((double)count, isK, isPerSec, decimal);
}
#endif
...
CString CastItoXBytes(double count, bool isK, bool isPerSec, uint32 decimal) {
  ...
  if (isPerSec) {
    if (thePrefs.GetForceSpeedsToKB())
      buffer.Format(_T("%.*f %s"), decimal, count / 1024.0,
                    (LPCTSTR)GetResString(IDS_KBYTESPERSEC));
    else if (count < 1024.0)
      buffer.Format(_T("%.0f %s"), count,
                    (LPCTSTR)GetResString(IDS_BYTESPERSEC));
    else if (count < 1024000.0)
      buffer.Format(_T("%.*f %s"), decimal, count / 1024.0,
                    (LPCTSTR)GetResString(IDS_KBYTESPERSEC));
    else if (count < 1048576000.0)
      buffer.Format(_T("%.*f %s"), decimal, count / 1048576.0,
                    (LPCTSTR)GetResString(IDS_MBYTESPERSEC));
    else if (count < 1073741824000.0)
      buffer.Format(_T("%.*f %s"), decimal, count / 1073741824.0,
                    (LPCTSTR)GetResString(IDS_GBYTESPERSEC));
    else
      buffer.Format(_T("%.*f %s"), decimal, count / 1099511627776.0,
                    (LPCTSTR)GetResString(IDS_TBYTESPERSEC));
  } else {
    if (count < 1024.0)
      buffer.Format(_T("%.0f %s"), count, (LPCTSTR)GetResString(IDS_BYTES));
    else if (count < 1024000.0)
      buffer.Format(_T("%.*f %s"), decimal, count / 1024.0,
                    (LPCTSTR)GetResString(IDS_KBYTES));
    else if (count < 1048576000.0)
      buffer.Format(_T("%.*f %s"), decimal, count / 1048576.0,
                    (LPCTSTR)GetResString(IDS_MBYTES));
    else if (count < 1073741824000.0)
      buffer.Format(_T("%.*f %s"), decimal, count / 1073741824.0,
                    (LPCTSTR)GetResString(IDS_GBYTES));
    else
      buffer.Format(_T("%.*f %s"), decimal, count / 1099511627776.0,
                    (LPCTSTR)GetResString(IDS_TBYTES));
  }
  return buffer;
}
...
CString CastItoIShort(double count, bool isK, uint32 decimal) {
  ...
  if (count < 1.0e15) {
    if (count < 1.0e3)
      ...
      output.Format(_T("%.*f %s"), decimal, count / div,
...
CString CastSecondsToHM(time_t tSeconds) {
  ...
  if (count < MIN2S(1))
    ...
    buffer.Format(_T("%u:%02u %s"), count / MIN2S(1),
                  count - MIN2S(count / MIN2S(1)),
                  (LPCTSTR)GetResString(IDS_MINS));
  else if (count < DAY2S(1))
    buffer.Format(_T("%u:%02u %s"), count / HR2S(1),
                  (count - HR2S(count / HR2S(1))) / MIN2S(1),
                  (LPCTSTR)GetResString(IDS_HOURS));
  else {
    UINT cntDays = count / DAY2S(1);
    UINT cntHrs = (count - DAY2S(cntDays)) / HR2S(1);
    buffer.Format(_T("%u %s %u %s"), cntDays, (LPCTSTR)GetResString(IDS_DAYS),
                  cntHrs, (LPCTSTR)GetResString(IDS_HOURS));
  }
  return buffer;
}
...
CString CastSecondsToLngHM(time_t tSeconds) {
  ...
  if (count < MIN2S(1))
    ...
    buffer.Format(_T("%u:%02u %s"), count / MIN2S(1),
                  count - MIN2S(count / MIN2S(1)),
                  (LPCTSTR)GetResString(IDS_LONGMINS));
  else if (count < DAY2S(1))
    buffer.Format(_T("%u:%02u %s"), count / HR2S(1),
                  (count - HR2S(count / HR2S(1))) / MIN2S(1),
                  (LPCTSTR)GetResString(IDS_LONGHRS));
  else {
    UINT cntDays = count / DAY2S(1);
    UINT cntHrs = (count - DAY2S(cntDays)) / HR2S(1);
    if (cntHrs)
      buffer.Format(_T("%u %s %u:%02u %s"), cntDays,
                    (LPCTSTR)GetResString(IDS_DAYS2), cntHrs,
                    (count - DAY2S(cntDays) - HR2S(cntHrs)) / MIN2S(1),
                    (LPCTSTR)GetResString(IDS_LONGHRS));
    else
      buffer.Format(_T("%u %s %u %s"), cntDays,
                    (LPCTSTR)GetResString(IDS_DAYS2),
                    (count - DAY2S(cntDays) - HR2S(cntHrs)) / MIN2S(1),
                    (LPCTSTR)GetResString(IDS_LONGMINS));
  }
  return buffer;
...
bool CheckFileOpen(LPCTSTR pszFilePath, LPCTSTR pszFileTitle) {
  if (thePrefs.GetCheckFileOpen() && !PathIsURL(pszFilePath) &&
      GetDRM(pszFilePath)) {
    CString strWarning;
    strWarning.Format(GetResString(IDS_FILE_WARNING_DRM),
                      pszFileTitle != NULL ? pszFileTitle : pszFilePath);
...
bool CFileOpen(CFile &file, LPCTSTR lpszFileName, UINT nOpenFlags,
               LPCTSTR lpszMsg) {
  CFileException ex;
  if (!file.Open(lpszFileName, nOpenFlags, &ex)) {
    if (ex.m_cause != CFileException::fileNotFound)
      LogError(LOG_STATUSBAR, _T("%s%s"), lpszMsg,
...
void CommitAndClose(CStdioFile &file) {
  if (thePrefs.GetCommitFiles() >= 2 ||
      (thePrefs.GetCommitFiles() >= 1 && theApp.IsClosing())) {
    file.Flush(); // flush file stream buffers to disk buffers
    if (_commit(_fileno(file.m_pStream)) != 0) // commit disk buffers to disk
      AfxThrowFileException(CFileException::hardIO, ::GetLastError(),
                            file.GetFileName());
  }
  file.Close();
}
...
void ShellOpenFile(LPCTSTR lpName) { ShellOpen(lpName, NULL); }
...
bool ShellDeleteFile(LPCTSTR pszFilePath) {
  ...
  if (thePrefs.GetRemoveToBin()) {
    ...
    fp.pFrom = todel;
    fp.fFlags =
        FOF_ALLOWUNDO | FOF_NOCONFIRMATION | FOF_SILENT | FOF_NORECURSION;
    __try {
...
CString ShellGetFolderPath(int iCSIDL) {
  ...
  // Try the Unicode version from "shell32" *and* examine the function result -
  // just the presence of that function does not mean that it returns the
  // requested path.
  //
  // Win98: 'SHGetFolderPathW' is available in 'shell32.dll', but it does not
  // support all of the CSIDL values.
  ...
  if (pfnSHGetFolderPathW) {
    ...
    if ((*pfnSHGetFolderPathW)(NULL, iCSIDL, NULL, SHGFP_TYPE_CURRENT,
                               wszPath) == S_OK)
  ...
  if (strFolderPath.IsEmpty()) {
    ...
    if (hLibShFolder) {
      (FARPROC &)pfnSHGetFolderPathW =
          GetProcAddress(hLibShFolder, "SHGetFolderPathW");
      if (pfnSHGetFolderPathW) {
        WCHAR wszPath[MAX_PATH];
        if ((*pfnSHGetFolderPathW)(NULL, iCSIDL, NULL, SHGFP_TYPE_CURRENT,
                                   wszPath) == S_OK)
      ...
      if (strFolderPath.IsEmpty()) {
        ...
        (FARPROC &)pfnSHGetFolderPathA =
            GetProcAddress(hLibShFolder, "SHGetFolderPathA");
        if (pfnSHGetFolderPathA) {
          CHAR aszPath[MAX_PATH];
          if ((*pfnSHGetFolderPathA)(NULL, iCSIDL, NULL, SHGFP_TYPE_CURRENT,
                                     aszPath) == S_OK)
...
CString URLDecode(const CString &inStr, bool bKeepNewLine) {
  ...
  for (int x = 0; x < inStr.GetLength(); ++x) {
    if (inStr[x] == _T('%') && x + 2 < inStr.GetLength() &&
        _istxdigit(inStr[x + 1]) && _istxdigit(inStr[x + 2])) {
      TCHAR hexstr[3];
...
void DupAmpersand(CString &rstr) { rstr.Replace(_T("&"), _T("&&")); }
/*
...
bool Ask4RegFix(bool checkOnly, bool dontAsk, bool bAutoTakeCollections) {
  ...
  // first check if the registry keys are already set (either by installer in
  // HKLM or by user in HKCU)
  result = regkey.Open(HKEY_CLASSES_ROOT, _T("ed2k\\shell\\open\\command"),
                       KEY_READ);
  ...
  if (!bGlobalSet) {
    ...
    if (checkOnly)
      return true;
    HKEY hkeyCR = thePrefs.GetWindowsVersion() < _WINVER_2K_
                      ? HKEY_LOCAL_MACHINE
                      : HKEY_CURRENT_USER;
    if (regkey.Create(hkeyCR,
                      _T("Software\\Classes\\ed2k\\shell\\open\\command")) ==
        ERROR_SUCCESS) {
      if (dontAsk || (LocMessageBox(IDS_ASSIGNED2K, MB_ICONQUESTION | MB_YESNO,
                                    0) == IDYES)) {
        VERIFY(regkey.SetStringValue(NULL, regbuffer) == ERROR_SUCCESS);

        VERIFY(
            regkey.Create(hkeyCR, _T("Software\\Classes\\ed2k\\DefaultIcon")) ==
            ERROR_SUCCESS);
        VERIFY(regkey.SetStringValue(NULL, modbuffer) == ERROR_SUCCESS);

        VERIFY(regkey.Create(hkeyCR, _T("Software\\Classes\\ed2k")) ==
               ERROR_SUCCESS);
        VERIFY(regkey.SetStringValue(NULL, _T("URL: ed2k Protocol")) ==
               ERROR_SUCCESS);
        VERIFY(regkey.SetStringValue(_T("URL Protocol"), _T("")) ==
               ERROR_SUCCESS);

        VERIFY(
            regkey.Open(hkeyCR, _T("Software\\Classes\\ed2k\\shell\\open")) ==
            ERROR_SUCCESS);
...
void BackupReg() {
  ...
  //	1)	It must follow the rules: reading from HKCR and writing into
  // HKCU. What we are currently doing 		is not consistent with the other
  // registry function which are dealing with the same keys.
  //
  //	2)	It behaves quite(!) differently under Win98 due to an obvious
  // change in the Windows API. 		WinXP: Reading a non existent
  // value returns 'key not found' error. 		Win98: Reading a non
  // existent value returns an empty string (which gets saved and restored by
  // our code). 		This means that saving/restoring existent
  // registry keys works completely differently in Win98/XP. 		Actually
  // it works correctly under Win98 and is broken in WinXP+. Though, did someone
  // notice it at all?

  HKEY hkeyCR = thePrefs.GetWindowsVersion() < _WINVER_2K_ ? HKEY_LOCAL_MACHINE
                                                           : HKEY_CURRENT_USER;
  ...
  if (regkey.Create(hkeyCR,
    ...
    TCHAR rbuffer[MAX_PATH + 100];
    ULONG maxsize = _countof(rbuffer);
    if (regkey.QueryStringValue(_T("OldDefault"), rbuffer, &maxsize) !=
            ERROR_SUCCESS ||
        maxsize == 0) {
      maxsize = _countof(rbuffer);
      if (regkey.QueryStringValue(NULL, rbuffer, &maxsize) == ERROR_SUCCESS)
        VERIFY(regkey.SetStringValue(_T("OldDefault"), rbuffer) ==
               ERROR_SUCCESS);
  ...
  if (regkey.Create(hkeyCR,
    ...
    if (regkey.QueryStringValue(_T("OldDefault"), rbuffer, &maxsize) ==
      ...
      VERIFY(
          ...
          ERROR_SUCCESS);
      maxsize = _countof(rbuffer);
      if (regkey.QueryStringValue(_T("OldIcon"), rbuffer, &maxsize) ==
          ERROR_SUCCESS) {
        VERIFY(regkey.SetStringValue(NULL, rbuffer) == ERROR_SUCCESS);
...
UINT GetMaxWindowsTCPConnections() {
  ...
  /*	OSVERSIONINFOEX osvi;
          ...
          if (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT) // Windows NT product
     family return UNLIMITED;  //no limits

          if (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) { // Windows 95
     product family

                  HKEY hKey;
                  if (osvi.dwMajorVersion == 4 && osvi.dwMinorVersion == 0) {
     //old school 95 DWORD dwValue; DWORD dwLength = (DWORD)sizeof dwValue;

                          RegOpenKeyEx(HKEY_LOCAL_MACHINE,
     _T("System\\CurrentControlSet\\Services\\VxD\\MSTCP") , 0, KEY_QUERY_VALUE,
     &hKey); LONG lResult = RegQueryValueEx(hKey, _T("MaxConnections"), NULL,
     NULL , (LPBYTE)&dwValue, &dwLength); RegCloseKey(hKey);
                  ...
                  RegOpenKeyEx(HKEY_LOCAL_MACHINE,
     _T("System\\CurrentControlSet\\Services\\VxD\\MSTCP") , 0, KEY_QUERY_VALUE,
     &hKey); lResult = RegQueryValueEx(hKey, TEXT("MaxConnections"), NULL, NULL,
     (LPBYTE)szValue, &dwLength); RegCloseKey(hKey);

                  LONG lMaxConnections;
                  if (lResult != ERROR_SUCCESS || (lMaxConnections =
     _tstoi(szValue)) < 1) return 100;  //the default for 98/ME is 100

                  return lMaxConnections;
          }

          return UNLIMITED;  //give the user the benefit of the doubt, most use
     NT+ anyway
  */
}
...
WORD DetectWinVersion() {
  ...
  case VER_PLATFORM_WIN32_NT:
    ...
    return _WINVER_7_;    // never return Win95 if we get the info about a NT
                          // system
...
uint64 GetFreeDiskSpaceX(LPCTSTR pDirectory) {
  ...
  return ::GetDiskFreeSpaceEx(pDirectory, &nFreeDiskSpace, NULL, NULL)
             ? nFreeDiskSpace.QuadPart
             : 0;
}

CString GetRateString(UINT rate) {
  static const UINT ids[6] = {IDS_CMT_NOTRATED, IDS_CMT_FAKE,
                              IDS_CMT_POOR,     IDS_CMT_FAIR,
                              IDS_CMT_GOOD,     IDS_CMT_EXCELLENT};
...
bool DecodeBase16(const TCHAR *base16Buffer, unsigned int base16BufLen,
                  byte *buffer, unsigned int bufflen) {
...
unsigned DecodeLengthBase16(unsigned base16Length) { return base16Length / 2; }
...
CWebServices::CWebServices() { m_tDefServicesFileLastModified = 0; }
...
INT_PTR CWebServices::ReadAllServices() {
  ...
  if (readFile != NULL) {
    ...
    while (!feof(readFile)) {
      ...
      if (sbuffer[0] == _T('#') || sbuffer[0] == _T('/') ||
          sbuffer.GetLength() < 5)
        continue;

      int iPos = sbuffer.Find(_T(','));
      if (iPos > 0) {
        const CString strUrlTemplate(
            sbuffer.Right(sbuffer.GetLength() - iPos - 1).Trim());
        if (!strUrlTemplate.IsEmpty()) {
          static LPCTSTR const _apszMacros[] = {
              _T("#hashid"), _T("#filesize"),      _T("#filename"),
...
int CWebServices::GetAllMenuEntries(CTitleMenu *pMenu, DWORD dwFlags) {
  struct _stat64 st;
  if (m_aServices.IsEmpty() || (statUTC(GetDefaultServicesFile(), st) == 0 &&
                                st.st_mtime > m_tDefServicesFileLastModified))
    ReadAllServices();
  ...
  for (INT_PTR i = 0; i < m_aServices.GetCount(); ++i) {
    ...
    if (rSvc.uMenuID == uMenuID) {
      ...
      if (file != NULL) {
        ...
        strUrlTemplate.Replace(_T("#filename"),
                               EncodeURLQueryParam(file->GetFileName()));
        ...
        strUrlTemplate.Replace(
            _T("#cleanfilename"),
            EncodeURLQueryParam(CleanupFilename(file->GetFileName())));

        // add cleaned up basename to the url
        strUrlTemplate.Replace(
            _T("#cleanname"),
            EncodeURLQueryParam(CleanupFilename(strBaseName, false)));
      }

      // Open URL
      TRACE(_T("Starting URL: %s\n"), (LPCTSTR)strUrlTemplate);
      return BrowserOpen(strUrlTemplate,
...
void CWebServices::Edit() {
  CString sDat;
  sDat.Format(_T("\"%swebservices.dat\""),
...
extern "C" int CALLBACK BrowseCallbackProc(HWND hWnd, UINT uMsg, LPARAM,
                                           LPARAM lpData) {
  if (uMsg == BFFM_INITIALIZED) {
    // Set initial directory
    if (((LPBROWSEINIT)lpData)->pszInitialDir != NULL)
      SendMessage(hWnd, BFFM_SETSELECTION, TRUE,
                  (LPARAM)((LPBROWSEINIT)lpData)->pszInitialDir);

    // Set dialog's window title
    if (((LPBROWSEINIT)lpData)->pszDlgTitle != NULL)
      SendMessage(hWnd, WM_SETTEXT, 0,
                  (LPARAM)((LPBROWSEINIT)lpData)->pszDlgTitle);
  }

  return 0;
}

bool SelectDir(HWND hWnd, LPTSTR pszPath, LPCTSTR pszTitle,
               LPCTSTR pszDlgTitle) {
  ASSERT(pszPath != NULL);
  bool bResult = false;
  ...
  if (SHGetMalloc(&pShlMalloc) == NOERROR) {
    ...
    BrsInfo.ulFlags = BIF_VALIDATE | BIF_NEWDIALOGSTYLE | BIF_RETURNONLYFSDIRS |
                      BIF_SHAREABLE | BIF_DONTGOBELOWDOMAIN;
    ...
    if (pszTitle != NULL || pszDlgTitle != NULL) {
      ...
      BrsInit.pszDlgTitle =
          (pszDlgTitle != NULL) ? pszDlgTitle : NULL /*pszTitle*/;
...
void MakeFoldername(CString &rstrPath) {
  if (!rstrPath
           .IsEmpty()) { // don't canonicalize an empty path, we would get a "\"
    canonical(rstrPath);
    slosh(rstrPath);
  }
}
...
BOOL DialogBrowseFile(CString &rstrPath, LPCTSTR pszFilters,
                      LPCTSTR pszDefaultFileName, DWORD dwFlags,
                      bool openfilestyle) {
  CFileDialog myFileDialog(
      openfilestyle, NULL, pszDefaultFileName,
      dwFlags | OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, pszFilters,
      NULL // parent CWnd
      ,
...
CString md4str(const byte *hash) { return EncodeBase16(hash, MDX_DIGEST_SIZE); }
...
CString CleanupFilename(const CString &filename, bool bExtension) {
  ...
  for (int i = 0; i < extpos; ++i) {
    if (sClean[i] != _T('.'))
      continue;
    if (i <= 0 || i >= sClean.GetLength() - 1 || !_istdigit(sClean[i - 1]) ||
        !_istdigit(sClean[i + 1]))
      sClean.SetAt(i, _T(' '));
  }
...
int __cdecl CompareE2DKFileType(const void *p1, const void *p2) noexcept {
  return _tcscmp(reinterpret_cast<const SED2KFileType *>(p1)->pszExt,
                 reinterpret_cast<const SED2KFileType *>(p2)->pszExt);
}
...
EED2KFileType GetED2KFileTypeID(LPCTSTR pszFileName) {
  ...
  const SED2KFileType *pFound = (SED2KFileType *)bsearch(
      &ft, g_aED2KFileTypes, _countof(g_aED2KFileTypes),
      sizeof g_aED2KFileTypes[0], CompareE2DKFileType);
  return pFound ? pFound->iFileType : ED2KFT_ANY;
}

// Returns the ed2k file type string ID which is to be used for
...
LPCTSTR GetED2KFileTypeSearchTerm(EED2KFileType iFileID) {
  ...
  // NOTE: If this gets changed, the function 'GetED2KFileTypeSearchID' also
...
// Returns the ed2k file type integer ID which is to be used for
// publishing+searching
EED2KFileType GetED2KFileTypeSearchID(EED2KFileType iFileID) {
  ...
  case ED2KFT_DOCUMENT:
    return iFileID;
  // NOTE: Archives and CD-Images are published+searched with file type "Pro"
  // NOTE: If this gets changed, the function 'GetED2KFileTypeSearchTerm' also
  // needs to get updated!
  case ED2KFT_ARCHIVE:
  case ED2KFT_CDIMAGE:
...
// Returns a file type which is used eMule internally only, examining the
...
public:
  CED2KFileTypes() {
    qsort(g_aED2KFileTypes, _countof(g_aED2KFileTypes),
          sizeof g_aED2KFileTypes[0], CompareE2DKFileType);
#ifdef _DEBUG
...
CED2KFileTypes
    theED2KFileTypes; // get the list sorted *before* any code is accessing it

const BYTE *FindPattern(const BYTE *pucBuff, int iBuffSize,
                        const BYTE *pucPattern, int iPatternSize) {
  for (int iSearchRange = iBuffSize - iPatternSize; iSearchRange >= 0;
       --iSearchRange) {
...
int GetModuleErrorString(DWORD dwError, CString &rstrError, LPCTSTR pszModule) {
  ...
  LPCVOID lpSource;
  if (pszModule) {
    dwFlags = FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_ALLOCATE_BUFFER |
...
int GetErrorMessage(DWORD dwError, CString &rstrErrorMsg, DWORD dwFlags) {
  if (GetSystemErrorString(dwError, rstrErrorMsg) == 0) // 0 if no error string
    rstrErrorMsg.Format(
        ((long)dwError >= 0) ? _T("Error %u") : _T("Error 0x%08x"), dwError);
  else if (dwFlags & 1) { // show numeric error code
    const CString sMsg(rstrErrorMsg);
    rstrErrorMsg.Format(
        ((long)dwError >= 0 ? _T("Error %u: %s") : _T("Error 0x%08x: %s")),
        dwError, (LPCTSTR)sMsg);
  }
...
BOOL GetExceptionMessage(const CException &ex, LPTSTR lpszErrorMsg,
                         UINT nMaxError) {
  BOOL ret = ex.GetErrorMessage(lpszErrorMsg, nMaxError, NULL);
  if (lpszErrorMsg)
    lpszErrorMsg[(ret && nMaxError) ? nMaxError - 1 : 0] = 0; // terminate
                                                              // string
  return ret;
}
...
LPCTSTR GetShellExecuteErrMsg(DWORD dwShellExecError) {
   ...
   * ----------------------------------------------------------------------------
   * 0						0	"Out of memory"
   * Yes ERROR_FILE_NOT_FOUND		2	(== SE_ERR_FNF)
   * Yes SE_ERR_FNF				2	(==
   * ERROR_FILE_NOT_FOUND)	Yes ERROR_PATH_NOT_FOUND		3
   * (== SE_ERR_PNF)				Yes SE_ERR_PNF
   * 3	(== ERROR_PATH_NOT_FOUND)	Yes SE_ERR_ACCESSDENIED		5
   * Yes SE_ERR_OOM				8
   * Yes ERROR_BAD_FORMAT			11
   * Yes SE_ERR_SHARE				26
   * No SE_ERR_ASSOCINCOMPLETE	27
   * No SE_ERR_DDETIMEOUT		28
   * No SE_ERR_DDEFAIL			29
   * No SE_ERR_DDEBUSY			30
   * No SE_ERR_NOASSOC			31
   * No SE_ERR_DLLNOTFOUND		32
   * No
   *
   * Using "FormatMessage(FORMAT_MESSAGE_FROM_HMODULE, <shell32.dll>" does *not*
   * work!
   */
  static const struct _tagERRMSG {
    ...
    LPCTSTR pszMsg;
      ...
      {1, _T("Call to MS-DOS Int 21 Function 4B00h was invalid.")},
      {/* 2*/ SE_ERR_FNF, _T("The specified file was not found.")},
      {/* 3*/ SE_ERR_PNF, _T("The specified path was not found.")},
      {4, _T("Too many files were open.")},
      {/* 5*/ SE_ERR_ACCESSDENIED,
       _T("The operating system denied access to the specified file.")},
      {6, _T("Library requires separate data segments for each task.")},
      {7, _T("There were MS-DOS Memory block problems.")},
      {/* 8*/ SE_ERR_OOM,
       _T("There was insufficient memory to start the application.")},
      {9, _T("There were MS-DOS Memory block problems.")},
      {10, _T("Windows version was incorrect.")},
      {11, _T("Executable file was invalid. Either it was not a Windows ")
           _T("application or there was an error in the .EXE image.")},
      {12, _T("Application was designed for a different operating system.")},
      {13, _T("Application was designed for MS-DOS 4.0.")},
      {14, _T("Type of executable file was unknown.")},
      {15, _T("Attempt was made to load a real-mode application (developed ")
           _T("for an earlier version of Windows).")},
      {16,
       _T("Attempt was made to load a second instance of an executable file ")
       _T("containing multiple data segments that were not marked read-only.")},
      {17, _T("Attempt was made in large-frame EMS mode to load a second ")
           _T("instance of an application that links to certain non-shareable ")
           _T("DLLs that are already in use.")},
      {18, _T("Attempt was made in real mode to load an application marked ")
           _T("for use in protected mode only.")},
      {19, _T("Attempt was made to load a compressed executable file. The ")
           _T("file must be decompressed before it can be loaded.")},
      {20, _T("Dynamic-link library (DLL) file was invalid. One of the DLLs ")
           _T("required to run this application was corrupt.")},
      {21, _T("Application requires Microsoft Windows 32-bit extensions.")},
      /*22-31	 RESERVED FOR FUTURE USE. NOT RETURNED BY VERSION 3.0.*/
      {24,
       _T("Command line too long.")}, // 30.03.99 []: Seen under WinNT 4.0/Win98
                                      // for a very long command line!

      /*non-WinExec error codes*/
      {/*26*/ SE_ERR_SHARE, _T("A sharing violation occurred.")},
      {/*27*/ SE_ERR_ASSOCINCOMPLETE,
       _T("The file name association is incomplete or invalid.")},
      {/*28*/ SE_ERR_DDETIMEOUT,
       _T("The DDE transaction could not be completed because the request ")
       _T("timed out.")},
      {/*29*/ SE_ERR_DDEFAIL, _T("The DDE transaction failed.")},
      {/*30*/ SE_ERR_DDEBUSY,
       _T("The DDE transaction could not be completed because other DDE ")
       _T("transactions were being processed.")},
      {/*31*/ SE_ERR_NOASSOC, _T("There is no application associated with the ")
                              _T("given file name extension.")},
...
int GetDesktopColorDepth() {
  ...
  int iColorBits = ::GetDeviceCaps(hdcScreen, BITSPIXEL) *
...
bool IsLANIP(uint32 nIP) {
  ...
  // -------------------------------------------
  //	0.*								"This"
  // Network 	10.0.0.0 - 10.255.255.255		Class A
  // 172.16.0.0 - 172.31.255.255		Class B 	192.168.0.0 -
  ...
  if (nFirst == 192 &&
      nSecond == 168) // check this 1st, because those LANs IPs are most common
    return true;
...
CString GetFormatedUInt(ULONG ulVal) {
  ...
  if (nf.Grouping == 0) {
    ...
    nf.LeadingZero = 0;
    nf.Grouping = 3;
    // we are hardcoding the following two format chars intentionally because
    // the C-RTL also has the decimal sep hardcoded to '.'
    nf.lpDecimalSep = _T(".");
    nf.lpThousandSep = _T(",");
    nf.NegativeOrder = 0;
  }
  ...
  int iResult = GetNumberFormat(LOCALE_SYSTEM_DEFAULT, 0, szVal, &nf,
                                strVal.GetBuffer(iBuffSize), iBuffSize);
...
// this function creates the content for the "length" ed2k meta tag which was
// introduced by eDonkeyHybrid with the data type 'string' :/  to save some
// bytes, omit leading zeros Though it got handy to display media properties
...
void Debug(LPCTSTR pszFmtMsg, ...) {
  ...
  int iTimeLen = (int)_tcsftime(strBuff.GetBuffer(40), 40, _T("%H:%M:%S "),
                                localtime(&tNow));
  strBuff.ReleaseBuffer(iTimeLen);
#endif
  strBuff.AppendFormatV(pszFmtMsg, pArgs);

  // get around a bug in the debug device which is not capable of dumping long
  // strings
...
CString DbgGetFileInfo(const uchar *hash) {
  ...
  if (hash != NULL) {
    ...
    strInfo.Format(_T("File=%s"),
                   (pszName != NULL ? pszName : (LPCTSTR)md4str(hash)));
  }
  return strInfo;
}
...
CString DbgGetFileStatus(UINT nPartCount, CSafeMemFile &data) {
  ...
  if (nPartCount == 0)
    ...
    strFileStatus.Format(_T("Parts=%u  Avail=%u  %s"), nPartCount,
                         nAvailableParts, (LPCTSTR)strPartStatus);
  }
...
CString DbgGetBlockInfo(uint64 StartOffset, uint64 EndOffset) {
  CString strInfo;
  strInfo.Format(_T("%I64u-%I64u (%I64u bytes)"), StartOffset, EndOffset,
  ...
  if (so == eo)
    ...
    strInfo.AppendFormat(_T(", Block %I64u-%I64u%s"), so, eo,
                         (eo - so > 1) ? _T("(**)") : _T(""));

  return strInfo;
}

CString DbgGetBlockFileInfo(const Requested_Block_Struct *block,
                            const CPartFile *partfile) {
  CString strInfo(DbgGetBlockInfo(block));
  strInfo.AppendFormat(
      _T("; %s"),
      (LPCTSTR)DbgGetFileInfo(partfile ? partfile->GetFileHash() : NULL));
  return strInfo;
}
...
CString DbgGetMuleClientTCPOpcode(UINT opcode) {
  static const struct {
    LPCTSTR pszOpcode;
    UINT uOpcode;
  } _aOpcodes[] = {_STRVAL(OP_EMULEINFO),
...
CString DbgGetClientTCPPacket(UINT protocol, UINT opcode, UINT size) {
  ...
  if (protocol == OP_EDONKEYPROT)
    str.Format(_T("protocol=eDonkey  opcode=%s"),
               (LPCTSTR)DbgGetDonkeyClientTCPOpcode(opcode));
  else if (protocol == OP_PACKEDPROT)
    str.Format(_T("protocol=Packed  opcode=%s"),
               (LPCTSTR)DbgGetMuleClientTCPOpcode(opcode));
  else if (protocol == OP_EMULEPROT)
    str.Format(_T("protocol=eMule  opcode=%s"),
               (LPCTSTR)DbgGetMuleClientTCPOpcode(opcode));
  else
...
void DebugRecv(LPCSTR pszMsg, const CUpDownClient *client, const uchar *packet,
               uint32 nIP) {
  // 111.222.333.444 = 15 chars
  if (client) {
    if (packet != NULL)
      Debug(_T("%-24hs from %s; %s\n"), pszMsg,
            (LPCTSTR)client->DbgGetClientInfo(true),
            (LPCTSTR)DbgGetFileInfo(packet));
    else
...
void DebugSend(LPCSTR pszMsg, const CUpDownClient *client,
               const uchar *packet) {
  if (client) {
    if (packet != NULL)
...
void throwCStr(LPCTSTR pStr) { throw CString(pStr); }

ULONGLONG GetDiskFileSize(LPCTSTR pszFilePath) {
  // If the file is not compressed nor sparse, 'GetCompressedFileSize' returns
  // the 'normal' file size.
  ULONGLONG ullCompFileSize;
  ((LPDWORD)&ullCompFileSize)[0] =
      ::GetCompressedFileSize(pszFilePath, &((LPDWORD)&ullCompFileSize)[1]);
  if (((LPDWORD)&ullCompFileSize)[0] != INVALID_FILE_SIZE ||
      ::GetLastError() == NO_ERROR)
    return ullCompFileSize;

  // If 'GetCompressedFileSize' failed or is not available, use the default
  // function
  WIN32_FIND_DATA fd;
  ...
  if (point.x == -1 && point.y == -1) {
    ...
    if (iSel >= 0) {
      ...
      if (lv.GetItemRect(iSel, &rc, LVIR_BOUNDS)) {
        point.SetPoint(rc.left + lv.GetColumnWidth(0) / 2,
                       (rc.top + rc.bottom) / 2);
...
time_t safe_mktime(struct tm *ptm) { return ptm ? mktime(ptm) : -1; }
...
CString StripInvalidFilenameChars(const CString &strText) {
  ...
  static LPCTSTR const apszReservedFilenames[] = {
      _T("NUL"),  _T("CON"),  _T("PRN"),  _T("AUX"),  _T("CLOCK$"), _T("COM1"),
      _T("COM2"), _T("COM3"), _T("COM4"), _T("COM5"), _T("COM6"),   _T("COM7"),
      _T("COM8"), _T("COM9"), _T("LPT1"), _T("LPT2"), _T("LPT3"),   _T("LPT4"),
      _T("LPT5"), _T("LPT6"), _T("LPT7"), _T("LPT8"), _T("LPT9")};
  for (unsigned i = 0; i < _countof(apszReservedFilenames); ++i) {
    ...
    if (_tcsnicmp(strDest, apszReservedFilenames[i], nPrefixLen) == 0) {
      ...
      if (strDest[nPrefixLen] == _T('.')) {
        // Filename starts with a reserved file name followed by a '.'
        // character: Replace that ',' character with an '_' character.
...
CString ipstr(uint32 nIP) { return CString(inet_ntoa(*(in_addr *)&nIP)); }

CStringA ipstrA(uint32 nIP) { return CStringA(inet_ntoa(*(in_addr *)&nIP)); }

void ipstrA(CHAR *pszAddress, int iMaxAddress, uint32 nIP) {
  const BYTE *pucIP = (BYTE *)&nIP;
  snprintf(pszAddress, iMaxAddress, "%u.%u.%u.%u", pucIP[0], pucIP[1], pucIP[2],
           pucIP[3]);
}
...
public:
  ...
  static const CString &GetVolumeInfo(LPCTSTR pszVolume) {
    ...
    // 'GetVolumeInformation' may cause a noticeable delay - depending on the
    // type of volume which is queried. As we are using that function for almost
    // every file (for compensating the NTFS file time issues), we need to cash
    // this information.
    //
    // The cache gets cleared when the user manually hits the 'Reload' button in
    // the 'Shared Files' window and when Windows broadcasts a message about
    // that a volume was mounted/unmounted.
    //
    DWORD dwMaximumComponentLength = 0;
    DWORD dwFileSystemFlags = 0;
    TCHAR szFileSystemNameBuffer[MAX_PATH + 1];
    if (!GetVolumeInformation(pszVolume, NULL, 0, NULL,
...
void ClearVolumeInfoCache(int iDrive) { g_VolumeInfo.ClearCache(iDrive); }
...
// ignore real(!) thumbs.db files -- seems that lot of ppl have 'thumbs.db'
// files without the 'System' file attribute
bool IsThumbsDb(const CString &sFilePath, const CString &sFileName) {
  if (sFileName.CompareNoCase(_T("thumbs.db")) == 0) {
    ...
    if (StgOpenStorage(sFilePath, NULL, STGM_READ | STGM_SHARE_DENY_WRITE, NULL,
                       0, &pStorage) == S_OK) {
...
bool IsAutoDaylightTimeSetActive() {
  ...
  if (key.Open(HKEY_LOCAL_MACHINE,
    ...
    if (key.QueryDWORDValue(_T("DisableAutoDaylightTimeSet"),
                            dwDisableAutoDaylightTimeSet) == ERROR_SUCCESS)
      if (dwDisableAutoDaylightTimeSet)
        return false;
  }
  return true; // default to 'Automatically adjust clock for daylight saving
               // changes'
}
...
bool AdjustNTFSDaylightFileTime(time_t &ruFileDate, LPCTSTR pszFilePath) {
  ...
  if (IsDaylightSavingTimeActive(lDaylightBias)) {
    if (IsAutoDaylightTimeSetActive()) {
      ...
      // and if the file's date is within DST period, we get again a wrong file
      // date.
      //
      // If 'Automatically adjust clock for daylight saving changes' is
      // disabled, Windows always reports 'Active DST'(!!) with a bias of '0'
      // although there is no DST specified. This means also, that there is no
      // chance to determine if a date is within any DST.

      // Following code might be correct, but because we don't have a DST and
      // because we don't have a bias, the code won't do anything useful.
...
uint16 GetRandomUInt16() {
  ...
  uint32 uRand0 = rand();
  uint32 uRand1 = rand();
  // NOTE: if the assert fires, most likely this function was called *without*
  // calling 'srand' first. NOTE: each spawned thread HAS to call 'srand' for
...
uint32 GetRandomUInt32() {
  ...
  uint32 uRand1 = rand();
  ASSERT(uRand0 != 41 || uRand1 != 18467);
  // NOTE: if this assert fires, most likely this function was called *without*
  // calling 'srand' first. NOTE: each spawned thread HAS to call 'srand' for
  // itself to get real random numbers.
...
HWND ReplaceRichEditCtrl(CWnd *pwndRE, CWnd *pwndParent, CFont *pFont) {
  ...
  if (pwndRE) {
    ...
    if (GetClassName(pwndRE->m_hWnd, szClassName, _countof(szClassName)) &&
        _tcsicmp(szClassName, _T("RichEdit20W")) == 0)
      return NULL;
    ...
    hwndNewRE =
        ::CreateWindowEx(dwExStyle, RICHEDIT_CLASS, strText, dwStyle,
                         rcWnd.left, rcWnd.top, rcWnd.Width(), rcWnd.Height(),
                         pwndParent->m_hWnd, (HMENU)uCtrlID, NULL, NULL);
...
void InstallSkin(LPCTSTR pszSkinPackage) {
  if (thePrefs.GetMuleDirectory(EMULE_SKINDIR).IsEmpty() ||
      _taccess(thePrefs.GetMuleDirectory(EMULE_SKINDIR), 0) != 0) {
  ...
  if (szExt == _T(".zip")) {
    ...
    if (zip.Open(pszSkinPackage)) {
      ...
      for (int i = zip.GetCount(); --i >= 0;) {
        ...
        if (zf && zf->m_sName.Right(_countof(_szSkinSuffix) - 1)
                          .CompareNoCase(_szSkinSuffix) == 0) {
          zfIniFile = zf;
          break;
        }
      }
      ...
      if (zfIniFile) {
        for (int i = 0; i < zip.GetCount(); ++i) {
          CZIPFile::File *zf = zip.GetFile(i);
          if (!zf || zf->m_sName.IsEmpty() || zf->m_sName[0] == _T('\\') ||
              zf->m_sName[0] == _T('/') || zf->m_sName.Find(_T(':')) >= 0 ||
          ...
          if (zf->m_sName[zf->m_sName.GetLength() - 1] == _T('/')) {
            ...
            if (!::CreateDirectory(strDstDirPath, NULL)) {
              ...
              CString strError;
              strError.Format(GetResString(IDS_INSTALL_SKIN_DIR_ERROR),
                              (LPCTSTR)strDstDirPath,
            ...
            if (!zf->Extract(strDstFilePath)) {
              DWORD dwError = ::GetLastError();
              CString strError;
              strError.Format(GetResString(IDS_INSTALL_SKIN_FILE_ERROR),
                              (LPCTSTR)zf->m_sName, (LPCTSTR)strDstFilePath,
                              (LPCTSTR)GetErrorMessage(dwError));
              AfxMessageBox(strError, MB_ICONERROR);
              break;
            }
    ...
    if (rar.Open(pszSkinPackage)) {
      ...
      while (rar.GetNextFile(strFileName)) {
        if (strFileName.IsEmpty() || strFileName[0] == _T('\\') ||
            strFileName[0] == _T('/') || strFileName.Find(_T(':')) >= 0 ||
        ...
        if (!bFoundSkinINIFile && strFileName.Right(_countof(_szSkinSuffix) - 1)
                                          .CompareNoCase(_szSkinSuffix) == 0)
          bFoundSkinINIFile = true;

        // No need to care about possible available sub-directories. UnRAR.DLL
        // cares about that automatically.
        CString strDstFilePath(thePrefs.GetMuleDirectory(EMULE_SKINDIR));
        ...
        if (!rar.Extract(strDstFilePath)) {
          ...
          CString strError;
          strError.Format(GetResString(IDS_INSTALL_SKIN_FILE_ERROR),
                          (LPCTSTR)strFileName, (LPCTSTR)strDstFilePath,
                          (LPCTSTR)GetErrorMessage(dwError));
          AfxMessageBox(strError, MB_ICONERROR);
          bError = true;
          break;
      ...
      strError.Format(_T("%s\r\n\r\n%s"),
                      (LPCTSTR)GetResString(IDS_INSTALL_SKIN_PKG_ERROR),
                      CRARFile::sUnrar_download);
      AfxMessageBox(strError, MB_ICONERROR);
    }
  }
  if (uid)
...
void TriggerPortTest(uint16 tcp, uint16 udp) {
  ...
  // the port check will need to do an obfuscated callback too if obfuscation is
  // requested, hence we have to provide our userhash so it can create the key
  if (thePrefs.IsCryptLayerPreferred())
    m_sTestURL.AppendFormat(_T("&obf=1&clienthash=%s"),
                            (LPCTSTR)md4str(thePrefs.GetUserHash()));
  else
    m_sTestURL += _T("&obf=0");
...
int CompareLocaleStringNoCaseA(LPCSTR psz1, LPCSTR psz2) {
  // SDK says: The 'CompareString' function is optimized to run at the highest
  // speed when 'dwCmpFlags' is set to 0 or NORM_IGNORECASE, and 'cchCount1' and
  // 'cchCount2' have the value -1.
  int iResult =
      CompareStringA(GetThreadLocale(), NORM_IGNORECASE, psz1, -1, psz2, -1);
...
int CompareLocaleStringNoCaseW(LPCWSTR psz1, LPCWSTR psz2) {
  ...
  // 'cchCount2' have the value -1.
  int iResult =
      CompareStringW(GetThreadLocale(), NORM_IGNORECASE, psz1, -1, psz2, -1);
  return iResult ? iResult - CSTR_EQUAL : 0;
}
...
int __cdecl CompareCStringPtrLocaleStringNoCase(const void *p1,
                                                const void *p2) {
...
void Sort(CSimpleArray<const CString *> &apstr,
          int(__cdecl *pfnCompare)(const void *, const void *)) {
...
void AddAutoStart() {
  ...
  DWORD dwModPathLen =
      ::GetModuleFileName(NULL, sExeFilePath, _countof(sExeFilePath));
  ...
  mKey.Create(HKEY_CURRENT_USER,
              _T("Software\\Microsoft\\Windows\\CurrentVersion\\Run"), REG_NONE,
...
int FontPointSizeToLogUnits(int nPointSize) {
  ...
  if (hDC) {
    POINT pt;
		...
		pt.y /= 720;
#else
    // This math accounts for proper rounding and thus we will get the correct
    // results.
    //
    // PointSize	Result
    // -------------------
    // 8*10			10.666 -> 11 (rounded up and thus correct
    // result)
...
bool IsUnicodeFile(LPCTSTR pszFilePath) {
  ...
  bool bResult = (fp != NULL) && fread(&wBOM, sizeof wBOM, 1, fp) == 1 &&
                 wBOM == u'\xFEFF';
  if (fp != NULL)
    fclose(fp);
  return bResult;
...
ULONGLONG GetModuleVersion(LPCTSTR pszFilePath) {
  ...
  DWORD dwUnused;
  DWORD dwVerInfSize =
      GetFileVersionInfoSize(const_cast<LPTSTR>(pszFilePath), &dwUnused);
  if (dwVerInfSize != 0) {
    LPBYTE pucVerInf = (LPBYTE)malloc(dwVerInfSize);
    if (pucVerInf) {
      if (GetFileVersionInfo(const_cast<LPTSTR>(pszFilePath), 0, dwVerInfSize,
                             pucVerInf)) {
        VS_FIXEDFILEINFO *pFileInf = NULL;
        UINT uLen = 0;
        if (VerQueryValue(pucVerInf, _T("\\"), (LPVOID *)&pFileInf, &uLen) &&
            pFileInf && uLen) {
          ullVersion = MAKEDLLVERULL(HIWORD(pFileInf->dwFileVersionMS),
                                     LOWORD(pFileInf->dwFileVersionMS),
                                     HIWORD(pFileInf->dwFileVersionLS),
                                     LOWORD(pFileInf->dwFileVersionLS));
        }
      }
      free(pucVerInf);
    }
...
ULONGLONG GetModuleVersion(HMODULE hModule) {
  ...
  DWORD dwModPathLen =
      ::GetModuleFileName(hModule, szFilePath, _countof(szFilePath));
...
CString GetShareName(const CString &path) {
  // Should be a drive '\\?\volume\' or '\\.\volume\', or a UNC share
  // '\\server\name\'
...
bool DoCollectionRegFix(bool checkOnly) {
  ...
  bool bGlobalSet = false;
  CRegKey regkey;
  LONG result = regkey.Open(HKEY_CLASSES_ROOT,
                            _T("eMule\\shell\\open\\command"), KEY_READ);
  if (result == ERROR_SUCCESS) {
    TCHAR rbuffer[MAX_PATH + 100];
    ULONG maxsize = _countof(rbuffer);
    regkey.QueryStringValue(NULL, rbuffer, &maxsize);
  ...
  if (!bGlobalSet) {
    ...
    HKEY hkeyCR = thePrefs.GetWindowsVersion() < _WINVER_2K_
                      ...
                      : HKEY_CURRENT_USER;
    if (regkey.Create(hkeyCR,
                      _T("Software\\Classes\\eMule\\shell\\open\\command")) ==
        ERROR_SUCCESS) {
      VERIFY(regkey.SetStringValue(NULL, regbuffer) == ERROR_SUCCESS);

      VERIFY(
          regkey.Create(hkeyCR, _T("Software\\Classes\\eMule\\DefaultIcon")) ==
          ERROR_SUCCESS);
      VERIFY(regkey.SetStringValue(NULL, CString(modbuffer) + _T(",1")) ==
             ERROR_SUCCESS);

      VERIFY(regkey.Create(hkeyCR, _T("Software\\Classes\\eMule")) ==
             ERROR_SUCCESS);
      VERIFY(regkey.SetStringValue(NULL, _T("eMule Collection File")) ==
             ERROR_SUCCESS);

      VERIFY(regkey.Open(hkeyCR, _T("Software\\Classes\\eMule\\shell\\open")) ==
             ERROR_SUCCESS);
      regkey.RecurseDeleteKey(_T("ddexec"));
      regkey.RecurseDeleteKey(_T("ddeexec"));

      VERIFY(regkey.Create(
                 hkeyCR, _T("Software\\Classes\\") COLLECTION_FILEEXTENSION) ==
             ERROR_SUCCESS);
...
bool gotostring(CFile &file, const uchar *find, LONGLONG plen) {
  ...
  for (ULONGLONG i = file.GetPosition(); i < len && file.Read(&temp, 1) == 1;
       ++i) {
    if (temp == find[j])
      ++j;
    else
...
RC4_Key_Struct *RC4CreateKey(const uchar *pachKeyData, uint32 nLen,
                             RC4_Key_Struct *key, bool bSkipDiscard) {
  if (key == NULL)
...
void RC4Crypt(const uchar *pachIn, uchar *pachOut, uint32 nLen,
              RC4_Key_Struct *key) {
...
static const SFileExts s_fileexts[] = {
    ...
    {FILETYPE_EXECUTABLE, _T("WIN/DOS EXE"),
     _T("|EXE|COM|DLL|SYS|CPL|FON|OCX|SCR|VBX|")},
    {FILETYPE_UNKNOWN, _T(""), _T("")}};

static const byte FILEHEADER_7Z_ID[] = {0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C};
static const byte FILEHEADER_ACE_ID[] = {0x2A, 0x2A, 0x41, 0x43,
                                         0x45, 0x2A, 0x2A};
...
static const byte FILEHEADER_MP4_ID[] = {
    0x66, 0x74, 0x79, 0x70}; // Enig123::please note that the header starts at
                             // the position 4, not 0
...
static const byte FILEHEADER_PNG_ID[] = {0x89, 0x50, 0x4e, 0x47,
                                         0x0d, 0x0a, 0x1a, 0x0a};
static const byte FILEHEADER_RAR_ID[] = {0x52, 0x61, 0x72, 0x21};
static const byte FILEHEADER_WM_ID[] = {0x30, 0x26, 0xb2, 0x75, 0x8e, 0x66,
...
bool GetDRM(LPCTSTR pszFilePath) {
  ...
  if (fd != -1) {
    ...
    if ((size_t)iRead > sizeof FILEHEADER_WM_ID &&
        memcmp(aucBuff, FILEHEADER_WM_ID, sizeof FILEHEADER_WM_ID) == 0) {
      iRead -= sizeof FILEHEADER_WM_ID;
      if (iRead > 0) {
        static const WCHAR s_wszWrmHdr[] = L"<WRMHEADER";
        if (FindPattern(aucBuff + sizeof FILEHEADER_WM_ID, iRead,
                        (BYTE *)s_wszWrmHdr,
                        sizeof s_wszWrmHdr - sizeof s_wszWrmHdr[0]))
          return true;
      }
    }
  }
...
EFileType GetFileTypeEx(CShareableFile *kfile, bool checkextention,
                        bool checkfileheader, bool nocached) {
  ...
  if (checkfileheader &&
      (!kfile->IsPartFile() || pfile->IsCompleteBDSafe(0, HEADERCHECKSIZE) ||
       test4iso)) {
    try {
      CFile inFile;
      if (inFile.Open(kfile->GetFilePath(),
                      CFile::modeRead | CFile::shareDenyNone)) {
        unsigned char headerbuf[HEADERCHECKSIZE];
        if (!kfile->IsPartFile() ||
            pfile->IsCompleteBDSafe(0, HEADERCHECKSIZE)) {
          int read = inFile.Read(headerbuf, HEADERCHECKSIZE);
          if (read == HEADERCHECKSIZE) {
            if (memcmp(headerbuf, FILEHEADER_ZIP_ID,
                       sizeof FILEHEADER_ZIP_ID) == 0)
              res = ARCHIVE_ZIP;
            else if (memcmp(headerbuf, FILEHEADER_RAR_ID,
                            sizeof FILEHEADER_RAR_ID) == 0)
              res = ARCHIVE_RAR;
            else if (memcmp(headerbuf + 7, FILEHEADER_ACE_ID,
                            sizeof FILEHEADER_ACE_ID) == 0)
              res = ARCHIVE_ACE;
            else if (memcmp(headerbuf, FILEHEADER_7Z_ID,
                            sizeof FILEHEADER_7Z_ID) == 0)
              res = ARCHIVE_7Z;
            else if (memcmp(headerbuf, FILEHEADER_WM_ID,
                            sizeof FILEHEADER_WM_ID) == 0)
              res = WM;
            else if (memcmp(headerbuf, FILEHEADER_AVI_ID,
                            sizeof FILEHEADER_AVI_ID) == 0 &&
                     strncmp((char *)headerbuf + 8, "AVI", 3) == 0)
              res = VIDEO_AVI;
            else if (memcmp(headerbuf, FILEHEADER_MP3_ID,
                            sizeof FILEHEADER_MP3_ID) == 0 ||
                     memcmp(headerbuf, FILEHEADER_MP3_ID2,
                            sizeof FILEHEADER_MP3_ID2) == 0)
              res = AUDIO_MPEG;
            else if (memcmp(headerbuf, FILEHEADER_MPG_ID,
                            sizeof FILEHEADER_MPG_ID) == 0)
              res = VIDEO_MPG;
            else if (memcmp(headerbuf + 4, FILEHEADER_MP4_ID,
                            sizeof(FILEHEADER_MP4_ID)) == 0)
              res = VIDEO_MP4;
            else if (memcmp(headerbuf, FILEHEADER_MKV_ID,
                            sizeof(FILEHEADER_MKV_ID)) == 0)
              res = VIDEO_MKV;
            else if (memcmp(headerbuf, FILEHEADER_OGG_ID,
                            sizeof(FILEHEADER_OGG_ID)) == 0)
              res = VIDEO_OGG;
            else if (memcmp(headerbuf, FILEHEADER_PDF_ID,
                            sizeof FILEHEADER_PDF_ID) == 0)
              res = DOCUMENT_PDF;
            else if (memcmp(headerbuf, FILEHEADER_PNG_ID,
                            sizeof FILEHEADER_PNG_ID) == 0)
              res = PIC_PNG;
            else if (memcmp(headerbuf, FILEHEADER_JPG_ID,
                            sizeof FILEHEADER_JPG_ID) == 0 &&
                     (headerbuf[3] == 0xe1 || headerbuf[3] == 0xe0))
              res = PIC_JPG;
            else if (memcmp(headerbuf, FILEHEADER_GIF_ID,
                            sizeof FILEHEADER_GIF_ID) == 0 &&
                     headerbuf[5] == 0x61 &&
                     (headerbuf[4] == 0x37 || headerbuf[4] == 0x39))
              res = PIC_GIF;
            else if (memcmp(headerbuf, FILEHEADER_EXE_ID,
                            sizeof FILEHEADER_EXE_ID) == 0) {
              // This could be a self extracting RAR archive. If the ED2K name
              // of the file has the RAR extension we treat this 'EXE' file as
              // an 'unverified' RAR file.
              res = ExtensionIs(kfile->GetFileName(), _T(".rar"))
                        ? FILETYPE_UNKNOWN
                        : FILETYPE_EXECUTABLE;
            } else if ((headerbuf[0] & 0xFF) == 0xFF &&
                       (headerbuf[1] & 0xE0) == 0xE0)
              res = AUDIO_MPEG;
          }
        }
        if (res == FILETYPE_UNKNOWN && test4iso) {
          ...
          int read = inFile.Read(headerbuf, HEADERCHECKSIZE);
          if (read == HEADERCHECKSIZE) {
            if (memcmp(headerbuf, FILEHEADER_ISO_ID,
                       sizeof FILEHEADER_ISO_ID) == 0)
              res = IMAGE_ISO;
          }
        }
        inFile.Close();
  ...
  for (const SFileExts *ext = s_fileexts; ext->ftype != FILETYPE_UNKNOWN;
       ++ext) {
  ...
  if (extLC.GetLength() == 3 && extLC[0] == _T('R') && _istdigit(extLC[1]) &&
      _istdigit(extLC[2]))
    return ARCHIVE_RAR;
  return FILETYPE_UNKNOWN;
}
...
int IsExtensionTypeOf(EFileType ftype, const CString &ext) {
  ...
  for (const SFileExts *pext = s_fileexts; pext->ftype != FILETYPE_UNKNOWN;
       ++pext) {
...
// Wrapper for _tmakepath which ensures that the output buffer does not exceed
// MAX_PATH using a smaller buffer without checking the sizes prior calling this
// function is not safe If the resulting path would be longer than MAX_PATH-1,
// it will be empty and return false (similar to PathCombine)
bool _tmakepathlimit(LPTSTR path, LPCTSTR drive, LPCTSTR dir, LPCTSTR fname,
                     LPCTSTR ext) {
  if (path == NULL) {
    ASSERT(0);
    return false;
...
bool HasSubdirectories(const CString &strDir) {
  // Never try to enumerate the files of a drive and thus physically access the
  // drive, just for the information whether the drive has subdirectories in the
  // root folder. Depending on the physical drive type (floppy disk, CD-ROM
  // drive, etc.) this creates an annoying physical access to that drive - which
  // is to be avoided in each case. Even Windows Explorer shows all drives by
  // default with a '+' sign (which means that the user has to explicitly open
  // the drive to really get the content) - and that approach will be fine for
  // eMule as well. Since the restriction for drives 'A:' and 'B:' was removed,
  // this gets more important now.
  CString sDir(strDir);
...
uint8 GetMyConnectOptions(bool bEncryption, bool bCallback) {
  ...
  const uint8 uSupportsCryptLayer =
      static_cast<uint8>(thePrefs.IsCryptLayerEnabled() && bEncryption);
  const uint8 uRequestsCryptLayer =
      static_cast<uint8>(thePrefs.IsCryptLayerPreferred() && bEncryption);
  const uint8 uRequiresCryptLayer =
      static_cast<uint8>(thePrefs.IsCryptLayerRequired() && bEncryption);
  // direct callback is only possible if connected to kad, TCP firewalled and
  // verified UDP open (for example on a full cone NAT)
  const uint8 uDirectUDPCallback = static_cast<uint8>(
      bCallback && theApp.IsFirewalled() && Kademlia::CKademlia::IsRunning() &&
      !Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) &&
      Kademlia::CUDPFirewallTester::IsVerified());

  const uint8 byCryptOptions =
      (uDirectUDPCallback << 3) | (uRequiresCryptLayer << 2) |
      (uRequestsCryptLayer << 1) | (uSupportsCryptLayer << 0);
...
bool AddIconGrayscaledToImageList(CImageList &rList, HICON hIcon) {
  ...
  if (::GetIconInfo(hIcon, &iinfo)) {
    ...
    if (cxGray.CreateFromHBITMAP(iinfo.hbmColor)) {
      ...
      bResult = rList.Add(CBitmap::FromHandle(hGrayBmp),
                          CBitmap::FromHandle(iinfo.hbmMask)) != -1;
...
extern "C" int mbedtls_rsa_gen_key() { return 0; }
extern "C" int psa_crypto_init() { return 0; }
extern "C" void mbedtls_psa_crypto_free() {}
extern "C" void mbedtls_ssl_ticket_init() {}
extern "C" int mbedtls_ssl_ticket_setup() { return 0; }
extern "C" void mbedtls_ssl_ticket_free() {}
