...
CSplashScreen::~CSplashScreen() { m_imgSplash.DeleteObject(); }
...
void CSplashScreen::OnPaint() {
  ...
  if (m_imgSplash.GetSafeHandle()) {
    ...
    if (dcMem.CreateCompatibleDC(&dc)) {
      ...
      dc.FillSolidRect(rc.left + 1, rc.top + 1, rc.Width() - 2, rc.Height() - 2,
                       RGB(255, 255, 255));
      ...
      dc.DrawText(_T("eMule-Aishor Mod | Based on eMule (C) 2002-2024 Merkur"),
                  &rc, DT_CENTER | DT_NOPREFIX);
      if (pOldFont)
        dc.SelectObject(pOldFont);
      font.DeleteObject();
    }
