...
void LogCertificate(PCCERT_CONTEXT pCertContext) {
  ...
  if (pCertInfo) {
    if (CertNameToStr(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                      &pCertInfo->Subject, CERT_X500_NAME_STR, szString,
                      _countof(szString)))
    ...
    if (pCertInfo->SerialNumber.cbData && pCertInfo->SerialNumber.pbData)
      DebugLog(LOG_DONTNOTIFY, _T("Email Encryption: Serial nr.: %s"),
               (LPCTSTR)GetCertInteger(pCertInfo->SerialNumber.pbData,
                                       pCertInfo->SerialNumber.cbData));

    if (CertNameToStr(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                      &pCertInfo->Issuer, CERT_X500_NAME_STR, szString,
                      _countof(szString)))
    ...
    if (CertGetNameString(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE,
                          CERT_NAME_DISABLE_IE4_UTF8_FLAG, szOID_COMMON_NAME,
                          szString, _countof(szString)))
      DebugLog(LOG_DONTNOTIFY, _T("Email Encryption: Name: %s"), szString);

    BYTE md5[16];
    DWORD cb = (DWORD)sizeof md5;
    if (CertGetCertificateContextProperty(pCertContext, CERT_MD5_HASH_PROP_ID,
                                          md5, &cb) &&
        cb == sizeof md5)
      DebugLog(LOG_DONTNOTIFY, _T("Email Encryption: MD5 hash: %s"),
               (LPCTSTR)GetCertHash(md5, cb));
    ...
    if (CertGetCertificateContextProperty(pCertContext, CERT_SHA1_HASH_PROP_ID,
                                          sha1, &cb) &&
        cb == sizeof sha1)
      DebugLog(LOG_DONTNOTIFY, _T("Email Encryption: SHA1 hash: %s"),
               (LPCTSTR)GetCertHash(sha1, cb));
  }
}

bool Encrypt(const CStringA &rstrContentA, CByteArray &raEncrypted,
             LPCWSTR pwszCertSubject) {
  LPCTSTR pszContainer = AfxGetAppName();
  HCRYPTPROV hCryptProv;
  if (!CryptAcquireContext(&hCryptProv, pszContainer, NULL, PROV_RSA_FULL,
                           NULL)) {
    DWORD dwError = ::GetLastError();
    if (dwError != (DWORD)NTE_BAD_KEYSET) {
      DebugLogWarning(LOG_DONTNOTIFY,
                      _T("Email Encryption: Failed to acquire certificate ")
                      _T("context container '%s' - %s"),
                      pszContainer, (LPCTSTR)GetErrorMessage(dwError, 1));
      return false;
    }
    if (!CryptAcquireContext(&hCryptProv, pszContainer, NULL, PROV_RSA_FULL,
                             CRYPT_NEWKEYSET)) {
      DebugLogWarning(LOG_DONTNOTIFY,
                      _T("Email Encryption: Failed to create certificate ")
                      _T("context container '%s' - %s"),
                      pszContainer,
  ...
  if (hStoreHandle) {
    PCCERT_CONTEXT pRecipientCert = CertFindCertificateInStore(
        hStoreHandle, PKCS_7_ASN_ENCODING | X509_ASN_ENCODING, 0,
        CERT_FIND_SUBJECT_STR, pwszCertSubject, NULL);
    if (pRecipientCert) {
      ...
      if (CryptEncryptMessage(
              &EncryptParams, 1, &pRecipientCert, (BYTE *)(LPCSTR)rstrContentA,
              rstrContentA.GetLength(), NULL, &cbEncryptedBlob)) {
        try {
          raEncrypted.SetSize(cbEncryptedBlob);
          if (!CryptEncryptMessage(&EncryptParams, 1, &pRecipientCert,
                                   (BYTE *)(LPCSTR)rstrContentA,
                                   rstrContentA.GetLength(),
                                   raEncrypted.GetData(), &cbEncryptedBlob)) {
            DebugLogWarning(
                LOG_DONTNOTIFY,
                _T("Email Encryption: Failed to encrypt message - %s"),
                (LPCTSTR)GetErrorMessage(::GetLastError(), 1));
        ...
        DebugLogWarning(LOG_DONTNOTIFY,
                        _T("Email Encryption: Failed to get length of ")
                        _T("encrypted message - %s"),
                        (LPCTSTR)GetErrorMessage(::GetLastError(), 1));

      VERIFY(CertFreeCertificateContext(pRecipientCert));
    } else
      DebugLogWarning(LOG_DONTNOTIFY,
                      _T("Email Encryption: Failed to find certificate with ")
                      _T("subject '%ls' - %s"),
                      pwszCertSubject,
                      (LPCTSTR)GetErrorMessage(::GetLastError(), 1));
    ...
    DebugLogWarning(
        LOG_DONTNOTIFY,
        _T("Email Encryption: Failed to open certificate store '%s' - %s"),
        pszCertStore, (LPCTSTR)GetErrorMessage(::GetLastError(), 1));
...
bool encoded_word(const CString &src, CStringA &dst) {
  ...
  int iLength =
      Base64EncodeGetRequiredLength(srcA.GetLength(), ATL_BASE64_FLAG_NOCRLF);

  if (!Base64Encode(reinterpret_cast<const BYTE *>((LPCSTR)srcA),
                    srcA.GetLength(), dst.GetBuffer(iLength), &iLength,
...
static int do_handshake(mbedtls_ssl_context *ssl, LPCTSTR *pmsg) {
  ...
  if (flags != 0)
    DebugLogWarning(LOG_DONTNOTIFY,
                    _T("Mail server certificate has issues: %u"), flags);
  return 0;
}
...
static int write_ssl(mbedtls_ssl_context *ssl, const char *buf, size_t len) {
  if (len > 0)
    for (int ret;
         (ret = mbedtls_ssl_write(ssl, (unsigned char *)buf, len)) <= 0;)
...
static int write_txt(mbedtls_net_context *sock_fd, const char *buf,
                     size_t len) {
  if (len > 0) {
...
protected:
  CNotifierMailThread() =
      default; // protected constructor used by dynamic creation
...
void CNotifierMailThread::sendmail() {
  ...
  ret = mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, pers,
                              sizeof pers - 1);
  if (ret != 0) {
  ...
  if (ret != 0) {
    DebugLogWarning(LOG_DONTNOTIFY, _T("Connect to %s:%hu failed: %d"),
                    (LPCTSTR)m_mail.sServer, m_mail.uPort, ret);
    goto exit;
  }
  ...
  ret = mbedtls_ssl_ticket_setup(&ticket_ctx, mbedtls_ctr_drbg_random,
                                 &ctr_drbg, MBEDTLS_CIPHER_AES_256_GCM, 86400);
  if (ret != 0) {
    pmsg = _T("mbedtls_ssl_ticket_setup");
    goto exit;
  }
  // Session tickets disabled for mbedTLS 3.x compatibility
  // mbedtls_ssl_conf_session_tickets_cb(&m_ssl_conf, mbedtls_ssl_ticket_write,
  // mbedtls_ssl_ticket_parse, &m_ssl_ticket_ctx);
  // mbedtls_ssl_conf_new_session_tickets(&m_ssl_conf,
  // MBEDTLS_SSL_SESSION_TICKETS_ENABLED);
  // mbedtls_ssl_conf_tls13_key_exchange_modes(&m_ssl_conf,
  ...
  switch (m_mail.uAuth) {
  case AUTH_PLAIN:
    sTmpA.Format("%c%s%c%s", '\0', (LPCSTR)((CStringA)m_mail.sUser), '\0',
                 (LPCSTR)(CStringA(m_mail.sPass)));
    ret = mbedtls_base64_encode(base, sizeof base, &n,
                                (unsigned char *)(LPCSTR)sTmpA,
                                sTmpA.GetLength());
    if (ret != 0) {
  ...
  sBodyA.Format("Content-Type: text/plain;\r\n"
                ...
                (LPCSTR)(NeedUTF8String(m_strBody) ? wc2utf8(m_strBody)
                                                   : (CStringA)m_strBody));
  ...
  if (bEncrypt) {
    ...
    if (!Base64Encode(aEncrypted.GetData(), (int)aEncrypted.GetSize(),
                      sBodyA.GetBuffer(iLength), &iLength)) {
  ...
  sBufA.Format("From: eMule <%s>\r\n"
               "Subject: %s\r\n"
               "To: <%s>\r\n"
               "MIME-Version: 1.0\r\n"
               "%s\r\n"
               "\r\n.\r\n",
               (LPCSTR)sSenderA, (LPCSTR)sTmpA, (LPCSTR)sReceiverA,
               (LPCSTR)sBodyA);
...
exit:
  ...
  if (pmsg)
    DebugLogError(_T("Error: %s returned -0x%04x - %s"), pmsg, -ret,
                  (LPCTSTR)SSLerror(ret));
}
...
int CNotifierMailThread::write_data(mbedtls_ssl_context *ssl, const char *buf,
  ...
  return m_mail.uTLS == MODE_NONE
             ? write_txt((mbedtls_net_context *)ssl->MBEDTLS_PRIVATE(p_bio),
                         buf, len)
...
void CemuleDlg::SendNotificationMail(TbnMsg nMsgType, LPCTSTR pszText) {
  ...
  if (mail.sServer.Trim().IsEmpty() || mail.sTo.Trim().IsEmpty() ||
      mail.sFrom.Trim().IsEmpty())
    return;

  CNotifierMailThread *pThread = static_cast<CNotifierMailThread *>(
      AfxBeginThread(RUNTIME_CLASS(CNotifierMailThread), THREAD_PRIORITY_LOWEST,
                     0, CREATE_SUSPENDED));
  if (pThread) {
    mail.sEncryptCertName =
        theApp.GetProfileString(_T("eMule"), _T("NotifierMailEncryptCertName"))
            .Trim();
    pThread->m_mail = mail;
    ...
    if (uid)
      pThread->m_strSubject.AppendFormat(_T(": %s"),
