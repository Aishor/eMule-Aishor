...
public:
  ...
  //CxImage::Load(imageFileName,CXIMAGE_FORMAT_PNG);} 	bool Save(const TCHAR *
  //imageFileName){ return CxImage::Save(imageFileName,CXIMAGE_FORMAT_PNG);}
...
protected:
  ...
  static void PNGAPI user_read_data(png_structp png_ptr, png_bytep data,
                                    png_size_t length) {
  ...
  static void PNGAPI user_write_data(png_structp png_ptr, png_bytep data,
                                     png_size_t length) {
    CxFile *hFile = (CxFile *)png_get_io_ptr(png_ptr);
  ...
  static void PNGAPI user_error_fn(png_structp png_ptr,
